...
\usepackage{xcolor}
\definecolor{SectionColor}{RGB}{200,60,0}
\definecolor{SubSectionColor}{RGB}{200,60,0}
\usepackage{titlesec}
...
Mon parcours s'est construit à l'interface entre l'ingénierie logicielle, les systèmes multi-agents et la cybersécurité. Après ma formation d'ingénieur à l'INSA Rennes, j'ai rejoint le \textbf{Laboratoire de Conception et d'Intégration des Systèmes} (LCIS) pour une thèse CIFRE avec Thales, mêlant problématiques industrielles et recherche académique autour de l'automatisation de la conception de systèmes multi-agents pour assurer la cyberdéfense.

Mes travaux portent sur la conception et l'étude de systèmes multi-agents pour la Cyberdéfense, combinant apprentissage par renforcement multi-agent, modélisation organisationnelle et analyse de comportements. J'ai développé plusieurs contributions méthodologiques, dont la méthode MAMAD et la plateforme CybMASDE, en conservant un lien direct avec les besoins opérationnels industriels.
...
\begin{itemize}
    ...
    \item \textbf{Spécialités} : Systèmes multi-agents (thématique 86) et Cyberdéfense (thématique E4)
    \item \textbf{Encadrement} :
          \begin{itemize}
              \item \textbf{Jean-Paul Jamont} (Professeur, UGA, LCIS) -- Directeur de thèse
              \item \textbf{Michel Occello} (Professeur, UGA) -- Co-directeur de thèse
              \item \textbf{Louis-Marie Traonouez} (Ingénieur de recherche, Thales LAS) -- Accompagnement industriel
...
%
Ma thèse, intitulée \textit{De l'Organisation d'un Système Multi-Agent de Cyberdéfense}, porte sur la conception et l’analyse de systèmes multi-agents dédiés à la détection et à la réponse à des comportements hostiles dans des environnements distribués. Elle a été menée sous la direction de Jean-Paul Jamont (UGA) et Michel Occello (UGA), avec la co-encadrement de Louis-Marie Traonouez (Thales LAS) et l’accompagnement scientifique de Paul Théron (AICA~IWG).
%
Elle a été réalisée au sein de l’École Doctorale MSTII (Mathématiques, Sciences et Technologies de l’Information, Informatique) de l’Université Grenoble Alpes. Les travaux issus de cette thèse ont donné lieu à plusieurs publications composées de deux journaux, quatre conférences internationales et six conférences nationales.
...
\begin{landscape}
    ...
    \begin{longtable}{%
        ...
        \caption{Enseignements en tant que \textit{ATER} (Statut) à
            Grenoble INP -- Esisar (Établissement), devant un public
        d'étudiants ingénieurs (Public).} \label{tab:enseignement-long}  \\
        \toprule
