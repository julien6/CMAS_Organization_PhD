...
\setbeamertemplate{footline}{
  ...
  \scriptsize%
  \ifnum\value{framenumber}>42%
    Appendix \number\numexpr\value{framenumber}-51\relax/51%
  \else%
    \ifnum\value{framenumber}>39%
      %
    \else
      \number\numexpr\value{framenumber}\relax/39%
    \fi
    ...
    \begin{column}{0.7\textwidth}
      ...
      \begin{itemize}
        \item \textit{\textbf{\textquote{Autonomous Intelligent Cyberdefense Agent} (AICA)}} \parencite{Kott2023}
              \begin{itemize}
                \item[$\Longrightarrow$] \textit{\textbf{\textquote{Multi-Agent System Centric AICA Reference Architecture}}} \parencite{Kott2023}
              \end{itemize}
...
\begin{frame}{État de l'art}{Recommendations : décomposition problème \& hypothèses}
  ...
  \resizebox{\linewidth}{!}{
    \begin{tabular}{lp{6.5cm}p{5cm}}
      ...
      \textbf{Analyse -- ANL}      & \textbf{H-AANL} : Inférence automatique des rôles et objectifs organisationnels à partir des trajectoires d’agents.  & C4 Contrôle, C5 Explicabilité \\
...
\begin{frame}{}
  ...
  \noindent {\tiny \begin{spacing}{0.8}
      \textit{Soulé, J., Jamont, J.-P., Occello, M., Traonouez L.-M., Théron, P. (2025). \textquote{Assisting Multi-Agent System Design with MOISE+ and MARL : The MAMAD Method}. In : Journal of Autonomous Agents and Multi-Agent Systems (JAAMAS) (2025). Under revision.}
    \end{spacing}}
...
\begin{frame}{Couverture globale des critères C1--C5}
  ...
  \begin{itemize}\scriptsize
    ...
    \item \textbf{Explicabilité} : alignement moyen $\approx 0.8$ entre rôles inférés et spécifications MOISE+ $\Rightarrow$ compréhension post-hoc automatisée.
    ...
    \item \textbf{Non-sationarité} : convergence plus rapide, politiques stables et meilleures récompenses cumulées sur l’ensemble des cas d’usage.
  \end{itemize}
\end{frame}
...
\begin{frame}{Synthèse et contributions principales}
  \small
  ...
  \begin{itemize}
    ...
    \item \textbf{Contributions principales :}
          \begin{itemize}
            \item Une extension \textbf{multi-agent des \textit{World Models}}
...
\begin{frame}{Synthèse : Apports et limites}

  \begin{block}{\faCheckCircle \; Points forts}
    \begin{itemize}
      \item Cadre MAMAD : pipeline complet MOD--TRN--ANL--TRF.
      \item Couplage $\mathcal{M}OISE^+$ / MARL : performance, robustesse, autonomie.
      \item Explicabilité organisationnelle (TEMM / Auto-TEMM).
      \item Jumeau numérique adaptatif pour le transfert continu.
      \item Plateforme CybMASDE : reproductibilité et généricité.
    \end{itemize}
  \end{block}
  ...
  \begin{alertblock}{\faExclamationTriangle \; Limitations}
    \begin{itemize}
      \item Modélisation \& validation experte nécessaire pour environnement et roles / objectifs.
      \item Explicabilité reste complexe ; interpretation humaine nécéssaire.
      \item Validation principalement simulée.
      \item Coût computationnel élevé et passage à l’échelle limitant.
    \end{itemize}
  \end{alertblock}
...
\begin{frame}{Publications et communications}{}
  ...
  \begin{itemize}
    \item Soulé, J., Jamont, J.-P., Occello, M., Traonouez L.-M., Théron, P. \textit{Assisting Multi-Agent System Design with MOISE+ and MARL: The MAMAD Method},
  ...
  \begin{itemize}
    ...
    \item \textit{Outil pour la Conception de SMA par apprentissage par renforcement multi-agent}, JFSMA 2024.
