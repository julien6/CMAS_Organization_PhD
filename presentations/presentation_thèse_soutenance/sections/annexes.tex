...
\begin{frame}{Explicabilité (TEMM / Auto-TEMM)}
    \textbf{Différences TEMM / Auto-TEMM :}
    \begin{itemize}
        \item TEMM : analyse semi-manuelle (expert).
        \item Auto-TEMM : tout automatique (clustering + extraction).
    \end{itemize}

    \textbf{Indice de Jaccard :}
    \begin{itemize}
        \item Simplicité.
        \item Robuste pour comparer actions/états visités.
        \item Adapté aux trajectoires partielles.
    \end{itemize}

    \textbf{Convergence :}
    \begin{itemize}
        \item Stabilisation des récompenses.
        \item Faible variance.
        \item Stabilité des rôles/trajectoires.
    \end{itemize}
\end{frame}

%===========================================================
\begin{frame}{Explicabilité en MARL — Vue d'ensemble}
    \small
    \textbf{Trois familles principales :}
    \begin{itemize}
        \item \textbf{Approches intrinsèques} : modèles interprétables par construction (arbres, concepts, décomposition).
        \item \textbf{Analyse post-hoc} : explications produites \emph{après} l'entraînement (SHAP, concepts latents, saillance).
        \item \textbf{Inférence de rôles / organisation} : extraction de structures globales, rôles et dépendances.
    \end{itemize}
\end{frame}
%===========================================================
...
%===========================================================
\begin{frame}{I. Approches intrinsèques (\textit{Interpretable-by-Design})}
    \small
    \textbf{I.1. Politiques arborescentes}
    \begin{itemize}
        \item Tree-Extraction MARL~\cite{zhang2024advancing}
        \item MAVIPER Trees~\cite{milani2022maviper}
        \item MAVIPER+~\cite{milani2024interpretable}
        \item Soft-Tree Decomposition (MIXRTs)~\cite{liu2025}
    \end{itemize}

    \vspace{2mm}
    \textbf{I.2. Concepts, hybridation et symbolique}
    \begin{itemize}
        \item Concept-MARL~\cite{zabounidis2023concept}
        \item XR-Explainable~\cite{iturria2024explainable}
        \item Shapley-MARL~\cite{li2025from}
        \item Neuro-Symbolic MARL~\cite{subramanian2024neurosymbolic}
    \end{itemize}

    \vspace{2mm}
    \textbf{I.3. Décomposition de récompenses}
    \begin{itemize}
        \item Reward-Decomposition XRL~\cite{juozapaitis2019explainable}
        \item VDN~\cite{Sunehag2018}, QMIX~\cite{Tabish2018}
    \end{itemize}
\end{frame}
%===========================================================
...
%===========================================================
\begin{frame}{II. Analyse post-hoc (\textit{Post-hoc Explainability})}
    \small
    \textbf{II.1. Contribution et importance}
    \begin{itemize}
        \item Shapley Collective-MARL~\cite{heuillet2021collective}
        \item Agent-Importance Metrics~\cite{khlifi2023agentimportance}
        \item Shapley-Attribution~\cite{li2025from}
    \end{itemize}

    \vspace{2mm}
    \textbf{II.2. Concepts latents et probing}
    \begin{itemize}
        \item Latent-Concept Analysis~\cite{grupen2022concept}
        \item Layer Probing~\cite{alain2018understanding}
    \end{itemize}

    \vspace{2mm}
    \textbf{II.3. Extraction / vérification}
    \begin{itemize}
        \item Policy-Extraction Verification~\cite{bastani2018verifiable}
    \end{itemize}

    \vspace{2mm}
    \textbf{II.4. Méthodes réflexives et saillance}
    \begin{itemize}
        \item Interpretability Perspectives~\cite{poupart2025perspectives}
        \item Post-hoc Interpretability Survey~\cite{madsen2023posthoc}
        \item Sanity Checks for Saliency~\cite{adebayo2018sanity}
        \item Concept Relevance Propagation~\cite{achtibat2022from}
        \item Intrinsic vs Post-hoc Evaluation~\cite{amorim2023evaluating}
    \end{itemize}
\end{frame}
%===========================================================
...
%===========================================================
\begin{frame}{III. Inférence de rôles et structures sociales}
    \small
    \textbf{III.1. Rôles via information mutuelle}
    \begin{itemize}
        \item Role-MI Learning~\cite{Wang2020}
    \end{itemize}

    \vspace{2mm}
    \textbf{III.2. Dépendances sociales et interactives}
    \begin{itemize}
        \item Inferential-Roles~\cite{yusuf2020inferential}
        \item Dependency-Graphing~\cite{serrino2019finding}
    \end{itemize}

    \vspace{2mm}
    \textbf{III.3. Travaux fondateurs}
    \begin{itemize}
        \item Early-Coordination~\cite{berenji2000learning}
    \end{itemize}
\end{frame}
%===========================================================
...
\begin{frame}
    {Annexes: RL/MARL taxonomy}{Aperçu}

    \begin{figure}
        \centering
        \resizebox{1\linewidth}{!}{
            \input{figures/marl_algo_taxonomy.tex}
        }
    \end{figure}
\end{frame}
...
\section{Cyber-Dec-POMDP}

\begin{frame}{Annexes: Modèle Dec-POMDP pré-spécialisé}
    {An overview}
...
\begin{frame}{Annexes: Formal model}
    {An overview}
...
\begin{frame}{Annexes: Simulation implementation}
...
\begin{frame}{Annexes: Attack/Defense scenarios integration}
...
\begin{frame}{Annexes}{Challenges in Defining Constraint Guides (RAG, RRG, GRG)}
...
\begin{frame}{Modèles organisationnels}
    \textbf{Principaux modèles :}
    \begin{itemize}
        \item AGR : simple (agent--groupe--rôle), pas de couche fonctionnelle ni déontique.
        \item OperA : très normatif, difficile à intégrer dans un pipeline MARL.
        \item Gaia / Tropos : méthodologies statiques, peu adaptées aux environnements dynamiques.
        \item MOISE+ : structure + missions + déontique, exploitable algorithmiquement.
    \end{itemize}

    \textbf{Pourquoi MOISE+ ?}
    \begin{itemize}
        \item Combine structure, fonction, déontique.
        \item Intégrable dans une fonction de récompense (rôles/objets $\rightarrow$ contraintes).
        \item Favorise l'explicabilité organisationnelle.
    \end{itemize}
\end{frame}

\begin{frame}{Méthodes de conception SMA}
    \textbf{Alternatives :}
    \begin{itemize}
        \item GAIA : structuré mais statique.
        \item ADELFE : adaptable, mais pas focalisée contrainte organisationnelle.
        \item DIAMOND, INGENIAS : orientés ingénierie, peu compatibles MARL.
    \end{itemize}

    \textbf{Limites :}
    \begin{itemize}
        \item Peu ou pas compatibles avec Dec-POMDP / MARL.
        \item Design one-shot plutôt qu'itératif.
    \end{itemize}

    \textbf{Atouts MAMAD :}
    \begin{itemize}
        \item Conception automatisée par apprentissage.
        \item Analyse organisationnelle intégrée.
    \end{itemize}
\end{frame}

%-------------------------------------------
\begin{frame}{Stackelberg Security Games}
    \textbf{Définition :}
    \begin{itemize}
        \item Leader (défenseur) choisit une stratégie.
        \item Follower (attaquant) réagit après observation.
    \end{itemize}

    \textbf{Pourquoi non utilisé ?}
    \begin{itemize}
        \item Hypothèse d’attaquant rationnel.
        \item Modèle stratégique, non séquentiel.
        \item Pas adapté à l'observabilité partielle (sauf extension).
        \item Pas exploitable dans MARL dynamique ou peu habituel.
    \end{itemize}

    \textbf{Perspective :} possible intégration pour l’allocation tactique.
\end{frame}

%-------------------------------------------
\begin{frame}{Arbres Attaque/Défense (ADTrees)}
    \textbf{Traduction dans MAMAD :}
    \begin{itemize}
        \item Scénario $\rightarrow$ ADTree $\rightarrow$ extraction des propriétés.
        \item Création des nœuds du Dec-POMDP (machines, services).
        \item Règles de transitions (compromission, mitigation).
    \end{itemize}

    \textbf{Lien avec MOISE+ :}
    \begin{itemize}
        \item Nœuds défensifs $\rightarrow$ objectifs.
        \item Branches d’attaque $\rightarrow$ plans d’objectifs.
    \end{itemize}

    \textbf{État actuel :} protocole existant pour environnement ;
    \begin{itemize}
        \item traduction vers MOISE+ = futur travail.
    \end{itemize}
\end{frame}
...
\begin{frame}{Complexité et Passage à l'échelle}
    \textbf{World Model :}
    \begin{itemize}
        \item Coût par transition constant (LSTM + MLP).
        \item Complexité totale proportionnelle aux transitions.
    \end{itemize}

    \textbf{TEMM / Auto-TEMM :}
    \begin{itemize}
        \item Hiérarchique : $O(n^3)$.
        \item K-means : $O(nkd)$.
    \end{itemize}

    \textbf{MARL (ex. PPO) :}
    \[
        O(\text{agents} \times \text{steps} \times \text{taille réseau} \times \text{updates})
    \]

    \textbf{Ajout des règles :} surcoût $O(\text{agents} \times \text{steps} \times \text{règles})$.
\end{frame}

\begin{frame}{Spécificité Cyberdéfense}
    \textbf{Éléments de spécialisation :}
    \begin{itemize}
        \item Dec-POMDP pré-spécialisé (nœuds, propriétés réseau).
        \item ADTrees pour structurer les scénarios.
        \item Rôles MOISE+ adaptés (détection, mitigation).
        \item Objectifs cyber (protéger les données, maintenir la disponibilité).
    \end{itemize}
\end{frame}

\begin{frame}{Contraintes, Multi-objectifs, Déontique}
    \textbf{Multi-objectifs :}
    \begin{itemize}
        \item Intégration via plans d’objectifs MOISE+.
    \end{itemize}

    \textbf{Déontique :}
    \begin{itemize}
        \item Extraction auto : rôles + objectifs atteints.
    \end{itemize}

    \textbf{Contraintes temporelles :}
    \begin{itemize}
        \item Paramètre $v_m$ déjà disponible (validité temporelle).
    \end{itemize}
\end{frame}

%-------------------------------------------
\begin{frame}{Choix des algorithmes MARL}
    \textbf{Algorithmes évalués :}
    \begin{itemize}
        \item MADDPG, MAPPO, QMIX, COMA.
    \end{itemize}

    \textbf{Conclusion :}
    \begin{itemize}
        \item MAPPO = meilleur compromis stabilité/performance.
        \item SAC disponible et perspective intéressante.
    \end{itemize}

    \textbf{Attaquant/Défenseur :}
    \begin{itemize}
        \item Apprentissage simultané pour co-adaptation.
        \item Curriculum learning = piste future.
    \end{itemize}
\end{frame}

%-------------------------------------------
\begin{frame}{World Model et Fidélité}
    \textbf{Validation du WM :}
    \begin{itemize}
        \item Analyse manuelle des observations.
        \item Test de transfert vers environnement réel.
    \end{itemize}

    \textbf{Biais possibles :}
    \begin{itemize}
        \item Dérive si modèle imparfait.
        \item Compounding error si horizon trop long.
    \end{itemize}

    \textbf{Solutions :}
    \begin{itemize}
        \item Cycles courts, réalignement fréquent.
        \item Limiter les rollouts en imagination.
    \end{itemize}
\end{frame}

%-------------------------------------------
\begin{frame}{10. Pipeline MAMAD et utilisation}
    \textbf{Entrées nécessaires :}
    \begin{itemize}
        \item Espaces d’actions/observations.
        \item Fonction de récompense.
        \item Simulation (optionnelle).
        \item API d’interaction.
    \end{itemize}

    \textbf{Sorties :}
    \begin{itemize}
        \item Rôles et objectifs.
        \item Politique conjointe.
        \item Indicateurs d’adéquation.
    \end{itemize}
\end{frame}

%-------------------------------------------
\begin{frame}{11. Limites et perspectives}
    \textbf{Limites :}
    \begin{itemize}
        \item Coût computationnel élevé.
        \item Fidélité imparfaite du World Model.
    \end{itemize}

    \textbf{Perspectives :}
    \begin{itemize}
        \item World Models neuro-symboliques.
        \item Plans d’objectifs MOISE+.
        \item Contraintes temporelles complexes.
    \end{itemize}
\end{frame}
