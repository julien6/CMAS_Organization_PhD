...
\begin{frame}{Explainability (TEMM / Auto-TEMM)}
    \textbf{Differences between TEMM and Auto-TEMM:}
    \begin{itemize}
        \item TEMM: semi-manual expert analysis.
        \item Auto-TEMM: fully automated analysis (clustering + extraction).
    \end{itemize}

    \textbf{Jaccard index:}
    \begin{itemize}
        \item Simplicity.
        \item Robust for comparing visited actions and states.
        \item Well suited to partial trajectories.
    \end{itemize}

    \textbf{Convergence:}
    \begin{itemize}
        \item Reward stabilization.
        \item Low variance.
        \item Stable roles and trajectories.
    \end{itemize}
\end{frame}

%===========================================================
\begin{frame}{Explainability in MARL --- Overview}
    \small
    \textbf{Three main families:}
    \begin{itemize}
        \item \textbf{Intrinsic approaches}: models that are interpretable by design (trees, concepts, decomposition).
        \item \textbf{Post-hoc analysis}: explanations produced \emph{after} training (SHAP, latent concepts, saliency).
        \item \textbf{Role / organization inference}: extraction of global structures, roles, and dependencies.
    \end{itemize}
\end{frame}
%===========================================================
...
%===========================================================
\begin{frame}{I. Intrinsic Approaches (\textit{Interpretable-by-Design})}
    \small
    \textbf{I.1. Tree-based policies}
    \begin{itemize}
    ...
    \vspace{2mm}
    \textbf{I.2. Concepts, hybridization, and symbolic methods}
    \begin{itemize}
    ...
    \vspace{2mm}
    \textbf{I.3. Reward decomposition}
    \begin{itemize}
...
%===========================================================
\begin{frame}{II. Post-hoc Analysis (\textit{Post-hoc Explainability})}
    \small
    \textbf{II.1. Contribution and importance}
    \begin{itemize}
    ...
    \vspace{2mm}
    \textbf{II.2. Latent concepts and probing}
    \begin{itemize}
    ...
    \vspace{2mm}
    \textbf{II.3. Extraction / verification}
    \begin{itemize}
        \item Policy-Extraction Verification~\cite{bastani2018verifiable}
    \end{itemize}

    \vspace{2mm}
    \textbf{II.4. Reflective methods and saliency}
    \begin{itemize}
...
%===========================================================
\begin{frame}{III. Role Inference and Social Structures}
    \small
    \textbf{III.1. Roles via mutual information}
    \begin{itemize}
        \item Role-MI Learning~\cite{Wang2020}
    \end{itemize}

    \vspace{2mm}
    \textbf{III.2. Social and interaction dependencies}
    \begin{itemize}
...
\begin{frame}
    {Appendix: RL/MARL taxonomy}{Overview}
...
\begin{frame}{Appendix: Pre-specialized Dec-POMDP model}
    {An overview}
...
\begin{frame}{Appendix: Attack/Defense scenario integration}
...
\begin{frame}{Organizational models}
    \textbf{Main models:}
    \begin{itemize}
        \item AGR: simple (agent--group--role), with no functional or deontic layer.
        \item OperA: highly normative and difficult to integrate into a MARL pipeline.
        \item Gaia / Tropos: static methodologies, only weakly suited to dynamic environments.
        \item MOISE+: structure + missions + deontics, and directly exploitable algorithmically.
    \end{itemize}

    \textbf{Why MOISE+?}
    \begin{itemize}
        \item Combines structure, function, and deontics.
        \item Can be integrated into a reward function (roles/objects $\rightarrow$ constraints).
        \item Supports organizational explainability.
    \end{itemize}
\end{frame}

\begin{frame}{MAS design methods}
    \textbf{Alternatives:}
    \begin{itemize}
        \item GAIA: structured but static.
        \item ADELFE: adaptable, but not focused on organizational constraints.
        \item DIAMOND, INGENIAS: engineering-oriented, with limited MARL compatibility.
    \end{itemize}

    \textbf{Limitations:}
    \begin{itemize}
        \item Little or no compatibility with Dec-POMDP / MARL.
        \item One-shot design rather than iterative design.
    \end{itemize}

    \textbf{MAMAD strengths:}
    \begin{itemize}
        \item Learning-based automated design.
        \item Integrated organizational analysis.
    \end{itemize}
\end{frame}
...
\begin{frame}{Stackelberg Security Games}
    \textbf{Definition:}
    \begin{itemize}
        \item The leader (defender) selects a strategy.
        \item The follower (attacker) reacts after observing it.
    \end{itemize}

    \textbf{Why not used here?}
    \begin{itemize}
        \item Assumes a rational attacker.
        \item Strategic rather than sequential model.
        \item Not well suited to partial observability, except through extensions.
        \item Not naturally exploitable in dynamic MARL settings.
    \end{itemize}

    \textbf{Outlook:} a possible integration for tactical resource allocation.
\end{frame}

%-------------------------------------------
\begin{frame}{Attack/Defense Trees (ADTrees)}
    \textbf{Translation into MAMAD:}
    \begin{itemize}
        \item Scenario $\rightarrow$ ADTree $\rightarrow$ property extraction.
        \item Creation of Dec-POMDP nodes (machines, services).
        \item Transition rules (compromise, mitigation).
    \end{itemize}

    \textbf{Link with MOISE+:}
    \begin{itemize}
        \item Defensive nodes $\rightarrow$ goals.
        \item Attack branches $\rightarrow$ goal plans.
    \end{itemize}

    \textbf{Current status:} an environment protocol is already available;
    \begin{itemize}
        \item translation into MOISE+ remains future work.
    \end{itemize}
\end{frame}
...
\begin{frame}{Complexity and Scalability}
    \textbf{World Model :}
    \begin{itemize}
        \item Constant cost per transition (LSTM + MLP).
        \item Total complexity proportional to the number of transitions.
    \end{itemize}

    \textbf{TEMM / Auto-TEMM :}
    \begin{itemize}
        \item Hierarchical: $O(n^3)$.
        \item K-means : $O(nkd)$.
    \end{itemize}

    \textbf{MARL (ex. PPO) :}
    \[
        O(\text{agents} \times \text{steps} \times \text{network size} \times \text{updates})
    \]

    \textbf{Adding rules:} overhead of $O(\text{agents} \times \text{steps} \times \text{rules})$.
\end{frame}

\begin{frame}{Cyber-defense specificity}
    \textbf{Specialization elements:}
    \begin{itemize}
        \item A pre-specialized Dec-POMDP (nodes, network properties).
        \item ADTrees to structure scenarios.
        \item MOISE+ roles adapted to cyber defense (detection, mitigation).
        \item Cyber objectives (protecting data, maintaining availability).
    \end{itemize}
\end{frame}

\begin{frame}{Constraints, Multi-objective Optimization, and Deontics}
    \textbf{Multi-objective setting:}
    \begin{itemize}
        \item Integration through MOISE+ goal plans.
    \end{itemize}

    \textbf{Deontics:}
    \begin{itemize}
        \item Automatic extraction: roles + achieved goals.
    \end{itemize}

    \textbf{Temporal constraints:}
    \begin{itemize}
        \item Parameter $v_m$ already available (temporal validity).
    \end{itemize}
\end{frame}

%-------------------------------------------
\begin{frame}{Choice of MARL Algorithms}
    \textbf{Evaluated algorithms:}
    \begin{itemize}
    ...
    \textbf{Conclusion:}
    \begin{itemize}
        \item MAPPO provides the best stability/performance trade-off.
        \item SAC is available and remains an interesting direction.
    \end{itemize}

    \textbf{Attacker/Defender setting:}
    \begin{itemize}
        \item Simultaneous learning for co-adaptation.
        \item Curriculum learning remains a promising future direction.
    \end{itemize}
\end{frame}

%-------------------------------------------
\begin{frame}{World Model and Fidelity}
    \textbf{WM validation:}
    \begin{itemize}
        \item Manual analysis of observations.
        \item Transfer testing on the real environment.
    \end{itemize}

    \textbf{Possible biases:}
    \begin{itemize}
        \item Drift when the model is imperfect.
        \item Compounding error when the horizon is too long.
    \end{itemize}

    \textbf{Mitigations:}
    \begin{itemize}
        \item Short cycles and frequent realignment.
        \item Limit imagination rollouts.
    \end{itemize}
\end{frame}

%-------------------------------------------
\begin{frame}{10. MAMAD Pipeline and Usage}
    \textbf{Required inputs:}
    \begin{itemize}
        \item Action and observation spaces.
        \item Reward function.
        \item Simulation (optional).
        \item Interaction API.
    \end{itemize}

    \textbf{Outputs:}
    \begin{itemize}
        \item Roles and goals.
        \item Joint policy.
        \item Fit indicators.
    \end{itemize}
\end{frame}
...
%-------------------------------------------
\begin{frame}{11. Limitations and Perspectives}
    \textbf{Limitations:}
    \begin{itemize}
        \item High computational cost.
        \item Imperfect World Model fidelity.
    \end{itemize}

    \textbf{Perspectives:}
    \begin{itemize}
        \item Neuro-symbolic World Models.
        \item MOISE+ goal plans.
        \item Complex temporal constraints.
    \end{itemize}
\end{frame}
