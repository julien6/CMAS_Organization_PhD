...
This method mitigates reliance on domain-specific knowledge in defining roles and goals by reframing their design as an optimization problem. Agent policies evolve to maximize cumulative rewards within a policy space, which is initially unconstrained due to loosely defined roles and goals. The analysis phase enables the identification of abstract roles and goals, which progressively refine the policy space by eliminating irrelevant behaviors. Through an iterative process of training, empirical analysis, refinement, and retraining, the search space is incrementally restricted, facilitating convergence toward optimal, domain-agnostic roles and goals with minimal manual intervention. This adaptive approach enhances the KARMA generalizability.
...
\footnotetext[1]{\label{lnk:footnote_1}Source code available with extra details and Jupyter Notebook at: \url{https://anonymous.4open.science/r/KARMA-040B/README.md}}
