...
\begin{abstract}
    In cloud-native critical systems, Kubernetes clusters with interdependent services face challenges to their operational resilience due to poor workload management, leading to issues like resource blocking, bottlenecks, and continuous pod crashes. Such failures are further exacerbated by adversarial scenarios, including Distributed Denial-of-Service (DDoS) attacks. Conventional Horizontal Pod Autoscaling (HPA) approaches often struggle in these dynamic conditions, while reinforcement learning-based methods, though more adaptable, typically optimize single objectives such as latency or resource usage, neglecting broader failure scenarios.
    %
    This paper proposes a Multi-Agent System (MAS) for Kubernetes HPA by decomposing operational resilience into failure-specific sub-objectives, handled by dedicated agents. We present an online framework comprising four phases to automate its design: (1) modeling a digital twin built from the cluster's traces; (2) training agents in simulation using roles and missions informed by failure contexts; (3) analyzing agent behaviors to ensure explainability; and (4) transferring learned policies to the real cluster.
    %
    Experimental evaluations demonstrate that the proposed MAS outperforms eight state-of-the-art HPA systems in sustaining operational resilience under adversarial conditions across two complex Kubernetes cluster scenarios.
\end{abstract}

\begin{IEEEkeywords}
    Adversarial, Horizontal Pod Autoscaling, MARL, Multi-Agent System Design
\end{IEEEkeywords}
...
KARMA addresses the gaps identified in existing work by introducing KARMA, an online automated design framework, that aims to produce efficient HPA MAS specifically designed to handle adversarial scenarios. \autoref{tab:autoscaling_criteria} provides a comparative overview of key Kubernetes HPA systems with our contribution. We position our contribution through following features:
\begin{itemize}
