...
\begin{table}[h!]
    ...
    \hline
    \textbf{Criterion} & \vspace{-0.3cm}\textbf{\cite{gymhpa2022}} & \vspace{-0.3cm}\textbf{\cite{aware2023}} & \vspace{-0.3cm}\textbf{\cite{imam2022}} & \vspace{-0.3cm}\textbf{\cite{Libra}} & \vspace{-0.3cm}\textbf{\cite{QoSRL}} & \vspace{-0.3cm}\textbf{\cite{AHPA}} & \vspace{-0.3cm}\textbf{\cite{KOSMOS}} & \vspace{-0.3cm}\textbf{\cite{COPA}} & \textbf{KARMA} \\
    \hline
    \hline
...
\begin{itemize}
    ...
    \item \textbf{AWARE}~\cite{aware2023} incorporates RL to optimize autoscaling decisions while balancing quality-of-service (QoS) objectives, such as response time and throughput. It partially considers adversarial scenarios but struggles with high automation levels and multi-agent coordination
...
For example, during a DDoS attack, an agent may take precedence to ensuring that the other agents' actions are not uselessly applied. This necessitates understanding how agents implicitly or explicitly negotiate priorities and coordinate~\cite{shoham2008multiagent}.
...
\noindent Hierarchical clustering identifies recurring sequences of actions that define an agent's role. The distance between action sequences is computed using Dynamic Time Warping~\cite{berndt1994using}:
\[
...
\noindent K-means clustering groups trajectories based on similarity in states visited. The cluster centers represent common objectives, and recurrent observations in high-reward trajectories are sampled to define intermediate goals:
    ...
    \]
    Here, $\mathbb{P}(s)$ represents the probability of visiting a state $s$ within successful trajectories. Hyperparameters of K-means are empirically optimized to minimize noise.
...
The evaluation environment consists of a Kubernetes cluster simulating a \textbf{Chained Services} architecture. Each service comprises a set of microservices hosted in pods and managed by deployments. For instance, \autoref{fig:chained_services_graph} illustrates the graph representation of a four services CS cluster. We considered using a cluster characterized by the following specifications:
...
\begin{itemize}
    ...
    \item \textbf{Worker Nodes:} 1 worker node with 8 vCPUs, 32 GB RAM, and 1 Gbps network bandwidth. 
...
The KARMA framework leverages \textit{Cyber Multi-Agent System Development Environment}~\footnote{Source code available with extra detail and Jupyter Notebook at: \url{https://anonymous.4open.science/r/KARMA-040B/README.md}} (CybMASDE) that is a general assisted-design MAS fitting in which the KARMA framework is developed. The framework includes:
\begin{itemize}
    \item \textbf{Digital Twin Modeling:} A simulation environment replicates Kubernetes cluster using real-world traces.
    \item \textbf{MARL Training:} \textit{MADDPG}~\cite{lowe2017multi} os used to train agents in the digital twin environment.
...
\noindent Following to the $\mathcal{M}OISE^+$~\cite{hubner2002moise} and the AICA architectural insights~\cite{kott2018autonomous}, we implemented four roles to address a specific degradation factor in a QoS and defines the permissible actions of agents. Each role is associated with a mission, containing a single sub-objective based on metrics.
...
\begin{enumerate*}[label={}, itemjoin={;\quad }]
    \item \( T_{\text{in}}^i \): Incoming traffic for service \( i \) (requests/s or Kbps).
...
\begin{itemize}
    \item \textbf{Operational Resilience:} Based on the global reward from the success rate (\%), pending request ratio (\%), average latency (ms)~\cite{burns2016borg}.
...
\label{subsec:gap_operational_resilience}

Operational resilience evaluates the ability of the system to handle failures and maintain high Quality of Service (QoS). Metrics such as \textit{success rate}, \textit{latency compliance}, and \textit{pending requests ratio} are used. \autoref{tab:operational_resilience} presents the results across all scenarios.
...
\begin{table}[h!]
    ...
    {\scriptsize
    \begin{tabular}{>{\raggedright\arraybackslash}m{3cm}>{\centering\arraybackslash}m{1.5cm}>{\centering\arraybackslash}m{1.5cm}>{\centering\arraybackslash}m{1.5cm}}
    \toprule
    \textbf{Baseline} & \textbf{Success Rate (\%)} & \textbf{Latency Compliance (\%)} & \textbf{Pending Requests (\%)} \\
    \midrule
    Single-Agent w/o Org. Spec. & 74.3 & 67.1 & 18.7 \\
    Single-Agent w/ Hard Org. Spec. & 83.9 & 75.6 & 12.3 \\
    Multi-Agent w/o Org. Spec. & 86.5 & 79.2 & 10.5 \\
    Multi-Agent w/ Soft Org. Spec. & 90.3 & 84.1 & 6.7 \\
    Multi-Agent w/ Hard Org. Spec. (KARMA) & \textbf{93.7} & \textbf{88.5} & \textbf{3.1} \\
    \bottomrule
    \end{tabular}
    }
\end{table}

\paragraph*{Scenario-Specific Analysis.}
KARMA achieves the highest success rate (93.7\%) and latency compliance (88.5\%) while minimizing pending requests to just 3.1\%, outperforming all baselines across all scenarios. For example:
\begin{itemize}
    \item In the \textbf{Bottleneck Resolution} scenario, KARMA effectively reduces pending requests to 2.8\%, enabling higher throughput compared to the next-best baseline, Multi-Agent w/o Org. Spec. (10.2\%).
    \item In the \textbf{Resource Contention} scenario, KARMA prioritizes critical services, achieving a latency compliance of 90.3\%, compared to 82.1\% for Single-Agent w/ Hard Org. Spec.
\end{itemize}

\paragraph*{Discussion.}
These results highlight the importance of multi-agent coordination and hard organizational specifications in ensuring resilience under diverse workload conditions. Single-agent baselines, even with hard organizational specifications, struggle to match the flexibility and robustness of KARMA's distributed decision-making approach.


\subsection{Gap 2: Adversarial Conditions}
\label{subsec:gap_adversarial_conditions}

Adversarial conditions, such as Distributed Denial-of-Service (DDoS) attacks, evaluate the robustness of the system. The metrics include \textit{recovery time} and \textit{service availability}, as shown in \autoref{tab:adversarial_conditions}.
...
\begin{table}[h!]
    ...
    {\scriptsize
    \begin{tabular}{>{\raggedright\arraybackslash}m{3cm}>{\centering\arraybackslash}m{1.5cm}>{\centering\arraybackslash}m{1.5cm}}
    \toprule
    \textbf{Baseline} & \textbf{Recovery Time (s)} & \textbf{Service Availability (\%)} \\
    \midrule
    Single-Agent w/o Org. Spec. & 60.2 & 72.3 \\
    Single-Agent w/ Hard Org. Spec. & 45.3 & 80.7 \\
    Multi-Agent w/o Org. Spec. & 40.1 & 85.4 \\
    Multi-Agent w/ Soft Org. Spec. & 35.2 & 89.6 \\
    Multi-Agent w/ Hard Org. Spec. (KARMA) & \textbf{28.5} & \textbf{95.2} \\
    \bottomrule
    \end{tabular}
    }
\end{table}
...
\paragraph*{Scenario-Specific Analysis.}
KARMA consistently achieves the best performance in the DDoS scenario:
\begin{itemize}
    \item \textbf{Recovery Time:} KARMA recovers 28.9\% faster than Multi-Agent w/o Org. Spec., demonstrating the effectiveness of collaborative agent coordination under stress.
    \item \textbf{Service Availability:} At 95.2\%, KARMA maintains service availability even under sustained attack, significantly outperforming Single-Agent w/o Org. Spec. (72.3\%).
\end{itemize}

\paragraph*{Discussion.}
The results underscore the importance of multi-agent configurations and hard organizational specifications in managing adversarial conditions. Hard constraints guide agents to prioritize critical actions, enabling faster recovery and higher availability. Comparatively, baselines like IMAM lack the explicit organizational guidance provided by KARMA, leading to slower recovery and lower robustness.


\subsection{Gap 3: Digital Twin Modeling}
\label{subsec:gap_digital_twin}

The accuracy of the digital twin is critical for training agents in realistic scenarios. The transition model accuracy is presented in \autoref{tab:digital_twin_accuracy}, evaluated across all scenarios.
...
\begin{table}[h!]
    ...
    {\scriptsize
    \begin{tabular}{>{\raggedright\arraybackslash}m{3cm}>{\centering\arraybackslash}m{1.5cm}}
    \toprule
...
\paragraph*{Scenario-Specific Analysis.}
KARMA achieves 94.8\% accuracy with its MLP-based transition model, enabling agents to adapt better to real-world cluster dynamics. In contrast, the transition model without MLP struggles to generalize across scenarios, limiting its utility for training robust agents.

\paragraph*{Discussion.}
The use of MLP approximators in KARMA significantly improves digital twin accuracy, providing agents with a more realistic simulation environment. This enhanced modeling capability is crucial for preparing agents to handle dynamic and adversarial scenarios effectively.


\subsection{Gap 4: Automated MAS Generation}
\label{subsec:gap_mas_generation}

The efficiency of MAS generation is assessed by \textit{convergence time} and \textit{training overhead}. \autoref{tab:mas_generation} presents the results, while \autoref{fig:learning_curves} shows learning curves for the main baselines over 2000 episodes.

\begin{table}[h!]
    \centering
    \caption{MAS Generation Efficiency Across All Scenarios}
    \label{tab:mas_generation}
    {\scriptsize
    \begin{tabular}{>{\raggedright\arraybackslash}m{3cm}>{\centering\arraybackslash}m{1.5cm}>{\centering\arraybackslash}m{1.5cm}}
    \toprule
    \textbf{Baseline} & \textbf{Convergence Time (episodes)} & \textbf{Training Overhead (hours)} \\
    \midrule
    Multi-Agent w/o Org. Spec. & 1200 & 24 \\
    Multi-Agent w/ Hard Org. Spec. (KARMA) & \textbf{950} & \textbf{18} \\
    \bottomrule
    \end{tabular}
    }
\end{table}
...
\paragraph*{Scenario-Specific Analysis.}
KARMA achieves a 20.8\% reduction in convergence time and a 25\% reduction in training overhead compared to Multi-Agent w/o Org. Spec. These gains are consistent across all scenarios, reflecting the efficiency of role-guided learning.

The explicit use of roles and missions enables faster convergence by narrowing the search space for optimal policies. This efficiency is particularly valuable in large-scale systems where training overhead can be a bottleneck.

\subsection{Gap 5: Adaptability}
\label{subsec:gap_adaptability}

Adaptability is assessed by analyzing reward variance across dynamic scenarios. \autoref{tab:mas_generation} presents results obtained for the reward variance accross all scenarios.

% TODO: all scenarios including the mixed scenario
\begin{table}[h!]
    \centering
    \caption{Comparison of MAS Generation Efficiency}
    \label{tab:mas_generation}
    {\scriptsize
    \begin{tabular}{>{\raggedright\arraybackslash}m{2.5cm}>{\centering\arraybackslash}m{2cm}>{\centering\arraybackslash}m{2cm}}
    \toprule
    \textbf{Approach} & \textbf{Convergence Time (episodes)} & \textbf{Training Overhead (hours)} \\
    \midrule
    Multi-Agent w/o Org. Spec. & 1200 & 24 \\
    Multi-Agent w/ Hard Org. Spec. (KARMA) & \textbf{950} & \textbf{18} \\
    \bottomrule
...
KARMA's adaptability is attributed to the organizational structure of roles and missions, which ensures agents maintain stability even when workloads fluctuate. This stability is crucial for systems operating in unpredictable environments. KARMA exhibits the lowest reward variance, indicating consistent performance across dynamic workloads.


\subsection{Gap 7: Explainability}
\label{subsec:gap_explainability}

Explainability is assessed through trajectory clustering and alignment with roles and missions. \autoref{fig:trajectory_clustering_hrl} shows the hierarchical clustering for role inference, while \autoref{tab:alignment} presents alignment scores.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.49\textwidth]{figures/role_hierarchical_clustering.pdf}
    \caption{Hierarchical clustering of agent trajectories for role inference.}
...
\begin{table}[h!]
    ...
    \begin{tabular}{>{\raggedright\arraybackslash}m{3cm}>{\centering\arraybackslash}m{1.5cm}}
    \toprule
    \textbf{Baseline} & \textbf{Alignment Score (\%)} \\
    \midrule
    Multi-Agent w/o Org. Spec. & 78.5 \\
    Multi-Agent w/ Hard Org. Spec. (KARMA) & \textbf{96.2} \\
    \bottomrule
    \end{tabular}
    }
\end{table}
...
This paper presented KARMA, a framework aimed at improving the operational resilience of Kubernetes clusters through a MAS-based approach. By integrating reinforcement learning and organizational principles, KARMA provides a structured and explainable methodology for handling dynamic workloads, adversarial scenarios, and failure conditions in cloud-native environments.
