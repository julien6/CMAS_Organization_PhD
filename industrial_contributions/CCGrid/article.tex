...
% TODO :
%  - il faut aussi introduire les actions, transitions d'état pour des "attaquants" (on considère qu'il y en a qu'un seul)
...
\noindent \textbf{Action Representation:} The action spaces for the defender and the attacker are distinct:
\begin{itemize}
    \item \textbf{Defender's Actions (\(A_d\)):} 
    $$
    a_d = (\text{service\_id}, \text{replica\_change}),
    $$
    where $\text{service\_id}$ identifies the target deployment, and $\text{replica\_change}$ indicates the change in replicas, bounded within $[-\alpha, +\alpha]$ (e.g., $\alpha = 3$).
    \item \textbf{Attacker's Actions (\(A_a\)):} 
    $$
    a_a = (\text{entry\_point\_id}, \text{rate\_modification}, \text{data\_modification}),
    $$
    where $\text{entry\_point\_id}$ specifies the point of entry for the attack, $\text{rate\_modification}$ modifies the incoming traffic rate (\(\text{high\_decrease}, \text{low\_decrease}, \text{no\_change}, \text{low\_increase}, \text{high\_increase}\)), and $\text{data\_modification}$ specifies the degree of data alteration (\(\text{no\_alteration}, \text{low\_alteration}, \text{high\_alteration}\)).
\end{itemize}

\

\noindent \textbf{Transition Function:} The state transition function $T: S \times A_d \times A_a \to S$ captures the effects of both defender and attacker actions:
$$
T(s_t, a_d, a_a) = s_{t+1}.
$$
For example:
\begin{itemize}
    \item Defender actions $a_d$ adjust the number of replicas for specific services, impacting resource consumption and availability.
...
\end{itemize}
Observed transitions $(s_t, a_d, a_a, s_{t+1})$ form a partial state transition graph, enriched using an MLP-based approximator for unobserved transitions.

\

\noindent \textbf{Reward Functions:} The reward functions $R_d$ and $R_a$ reflect the conflicting objectives of the defender and the attacker:
\begin{itemize}
    \item \textbf{Defender's Reward ($R_d$):}
    $$
    ...
    \item \textbf{Attacker's Reward ($R_a$):}
    $$
    r_a = -R_d(s_t, a_d, a_a),
    $$
    incentivizing actions that degrade the cluster's performance and availability.
\end{itemize}
