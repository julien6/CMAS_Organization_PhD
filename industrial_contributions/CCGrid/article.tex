...
\section{KARMA: A Framework for Kubernetes }
...
The KARMA framework is designed to address the limitations of conventional HPA by leveraging MARL and modular architectural components. Its goal is to provide robust, explainable, and adaptive scaling decisions for cloud-native critical systems, particularly under adversarial scenarios.
...
% TODO: expliquer plus en détail ce qu'est Kubernetes en partant d'une vue haut niveau, puis en expliquant l'architecture Kubernetes, la terminologie de chacun des composants, leur relations, pourquoi est-ce qu'on a besoin de changer le nombre de replica lorsque le flux entrant varie (spoiler: parce que plusieurs pods peuvent gérer plus de données entrantes en parallèle), expliquer comment on peut techniquement changer le nombre de replica via l'API (et donc le chemin à suivre dans l'architecture de Kubernetes depuis l'API jusqu'aux pods...)
As illustrated in \autoref{fig:karma_architecture}, the KARMA framework is runned along the Kubernetes cluser. The Kubernetes cluster is composed of worker nodes hosting pods, deployments, and services, with metrics server collecting resource metrics and providing them to the KARMA framework.

% TODO: dire que le framework est lancé sur un noeud connecté à la fois à l'API Kubernetes et l'API Prometheus, qu'il se compose de 4 composants utilisés dans un processus de conception automatisé reposant sur ...
The KARMA framework, implemented as a software process, includes four key components: Modeling, Training, Analyzing, and Transfer. These components are interconnected and operate in a loop to iteratively improve the system’s scalability and adaptability.

Here, we picture a global view of the underlying interactions occuring between the Kubernetes cluster and the KARMA framework proceeding as follows:\\

\textbf{1)} \quad Metrics such as CPU, memory usage, or latency from the Kubernetes cluster, as well as custom metrics exposed by the pods, are continuously collected by Prometheus~\cite{prometheus} that is the most common monitoring system and time-series metrics database. Stored states can be queried and sent to the Modeling Component.\\

\textbf{2)} \quad The \textbf{Modeling Component} generates a simulated transition function based on a representative sample of collected states, leveraging a Deep Learning (DL) model (TODO: LSTM or MLP + VAE?) to predict transition from applied scaling actions to states and . This modeled transition function is coupled with the proposed availability maximization reward function to produce an environment with PettingZoo~\cite{Terry2021}, that is the de facto standard library to implement an environment with applicable MARL techniques.\\

\textbf{3)} \quad The \textbf{Training Component}, leveraging the modeled PettingZoo environment, trains the agents policies to maximize the cumulative reward function. As suggested in AOMEA, agents can be optionally associated to roles and missions. A role is a rule-based mapping between a state and the expected set of actions an agent can choose in, aiming to restrict the policy exploration to a relevant or required policy sub-space. Again, to better help agents explore their policy space, a mission can be associated to them as a set of intermediary objectives that agents try to reach as incremental sub-steps to help them reaching the final goal.\\

\textbf{4)} \quad The \textbf{Analyzing Component} evaluates the trained agents behavior from their trajectories using non-supervised machine learning techniques. Hierarchical Clustering is used with  trajectory visualization and explainability analysis to ensure their effectiveness and alignment with the system's objectives.\\

\textbf{5)} \quad The \textbf{Transfer Component} prepares trained agents' policies to be runned in the real Kubernetes cluster by applying chosen scaling actions remotely via the Kubernetes API when receiving the current state from the real cluster.\\

\textbf{6)} \quad Scaling actions that adjust the number of replicas in a deployment, are applied via the Kubernetes API, completing the loop.

\

In the next sub-sections, we will detail each of the phase as for the involved theoretical concepts and their technical implementation.
