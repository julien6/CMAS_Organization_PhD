...
Model-based Reinforcement Learning (MBRL) has emerged as a powerful paradigm for improving data efficiency and enabling planning by learning predictive models of the environment dynamics \cite{ha2018worldmodels, hafner2019learning, hafner2020dreamer}. By learning a latent world model, agents can ``imagine'' future trajectories and optimize their behavior through model-based planning or latent-space rollouts. While these approaches have achieved remarkable success in single-agent settings, extending world models to Multi-Agent Reinforcement Learning (MARL) remains a major open challenge \cite{zhang2021, lowe2017multi, nguyen2020deep}. In particular, multi-agent environments introduce partial observability, non-stationarity induced by concurrent learning agents, and complex interaction dynamics, all of which significantly complicate world model learning.

Recent works have begun to explore \emph{multi-agent world models} (MAWMs) as a foundation for model-based MARL \cite{liu2022mwm, wang2023modelbasedmarl, zhang2024scalablemawm}. These approaches aim to learn predictive models over joint agent observations and actions, typically under a centralized-training–decentralized-execution (CTDE) paradigm \cite{oliehoek2016concise}. Despite promising empirical results, purely neural MAWMs still struggle to produce reliable long-horizon predictions in realistic partially observable environments. In practice, learned models often exhibit \emph{semantic drift}: predicted observations progressively violate basic properties of the environment, such as spatial coherence, object persistence, or interaction constraints, even when short-term prediction error remains low \cite{talvitie2014modelbias, venkatraman2015improving}.

We argue that this failure mode stems from a fundamental mismatch between the structure of the environment and the inductive biases of standard neural world models. To make this precise, we decompose the global problem of learning reliable multi-agent world models into the following \textbf{four gaps} that remain insufficiently addressed in the literature:

\paragraph{(G1) Lack of explicit invariants and equivariances.}
Most MAWMs are trained purely from data using generic neural architectures (such as recurrent models or transformers) \cite{liu2022mwm, wang2023modelbasedmarl}. As a result, they must implicitly learn fundamental environment properties such as object permanence, collision constraints, or action-induced geometric transformations. Prior work in representation learning has shown that failing to encode invariances and equivariances leads to poor generalization and unstable long-horizon predictions \cite{cohen2016group, bronstein2021geometric}. However, existing MAWMs rarely enforce such properties explicitly, especially in agent-centric, partially observable settings.

\paragraph{(G2) Semantic inconsistency under partial observability.}
In MARL, each agent typically observes only a local, partial view of the environment \cite{oliehoek2016concise}. Learning joint dynamics from such fragmented observations is inherently ill-posed, and neural world models often produce predictions that are locally plausible but globally inconsistent. For instance, objects may disappear or reappear without cause, or spatial relationships may become incoherent over time. While prior work has analyzed error compounding in model-based RL \cite{talvitie2014modelbias}, few methods explicitly address \emph{semantic consistency} as a first-class objective in MAWMs.

\paragraph{(G3) Absence of constraint-aware training objectives.}
Most existing MAWMs are trained by minimizing prediction or reconstruction losses (such as negative log-likelihood or mean squared error) \cite{hafner2019learning, zhang2024scalablemawm}. Such objectives do not penalize violations of high-level environment rules and treat all prediction errors uniformly. In contrast, the neuro-symbolic and physics-informed learning literature has demonstrated that incorporating structured constraints as differentiable regularizers can substantially improve robustness and generalization \cite{raissi2019physicsinformed, manhaeve2018deepproblog}. This idea has not yet been systematically applied to multi-agent world modeling.

\paragraph{(G4) Lack of evaluation metrics for semantic consistency.}
Standard evaluation protocols for world models focus on one-step or multi-step prediction error \cite{hafner2020dreamer}, which are often insufficient to capture long-horizon semantic failures. Two models with similar prediction error may differ drastically in terms of logical or physical plausibility. While recent works have advocated for task-level evaluation through downstream control \cite{hafner2020dreamer}, there is currently no widely adopted metric to quantify semantic consistency in MAWMs, making it difficult to diagnose or compare models along this critical dimension.

In this work, we propose \textbf{Neuro-Symbolic Multi-Agent World Models (NS-MAWM)}, a framework designed to explicitly address these gaps. Our approach integrates symbolic \emph{invariants} and \emph{action-conditioned equivariances} into MAWM training as differentiable consistency constraints, thereby injecting structured inductive biases into the learning process. To accommodate uncertainty and stochasticity, constraints are enforced softly, and an optional gating mechanism learns when each rule should be applied. To evaluate semantic consistency, we introduce a novel metric, the \emph{Rule Violation Rate}, which quantifies the frequency of invariant violations over long-horizon rollouts.

Through extensive experiments on multi-agent grid-based environments and a standard MARL benchmark, we demonstrate that NS-MAWM significantly reduces semantic drift, improves long-horizon prediction fidelity, and yields better downstream planning performance compared to purely neural baselines. These results show that explicitly addressing gaps (G1)–(G4) leads to more reliable and robust multi-agent world models, advancing the state of the art in model-based MARL.

The remainder of the paper is organized as follows.
Section~\ref{sec:background} introduces the multi-agent reinforcement learning setting and world model formulation.
Section~\ref{sec:method} presents the proposed neuro-symbolic world model framework and training algorithm.
Section~\ref{sec:analysis} provides an analysis of the effect of consistency constraints on long-horizon prediction.
Section~\ref{sec:eval} reports experimental results and ablation studies.
Finally, Section~\ref{sec:conclusion} concludes and discusses future directions.
...
% - One standard MARL benchmark (Overcooked-like, SMAC-like, VMAS-like)
