...
\]
where $f_{\theta}(\cdot)$ is typically implemented as a Recurrent Neural Network (RNN),
such as an Long-Short-Term Memory (LSTM)~\cite{hochreiter1997long},
and $g_{\theta}(\cdot)$ is a parametric mapping such as Multi-Layer Perceptron (MLP).
%
...
Given a dataset of joint trajectories $\mathcal{D}_{H^j} = \{ h^j \}$, with $h^j = \langle (\omega_t^j, a_t^j) \rangle_{t=0}^{T}$, the objective is to learn a predictive model of joint observation dynamics, as commonly done in model-based MARL\cite{Wang2022modelbasedmultiagentreinforcementlearning,Xu2022mingling}.
%
We define a \textbf{Joint-Observation Prediction Model} (JOPM) as a neuronal model to capture the environment dynamics:
%
\[
  \mathcal{T}_{\theta}^j :
...
\begin{figure}[t]
  ...
  and a determinability mask $\mathcal{M}_d$, while a neural world model $\mathcal{T}^j_\theta$
  predicts the full next observation.
  Three integration strategies are illustrated:
  (\emph{left}) symbolic consistency regularization, which aligns neural predictions with symbolic ones via an auxiliary loss;
  (\emph{center}) symbolic projection, which enforces symbolic features by overwriting neural outputs;
  (\emph{right}) residual symbolic dynamics, where symbolic rules predict determinable features and the neural model predicts only undeterminable ones.
  }
...
We quantify symbolic consistency using the \textit{Rule Violation Rate (RVR)},
which measures how often model predictions violate the symbolic transition rules
defined in \autoref{sec:ns}.
...
The \textbf{Rule Violation Rate} over $T$ transitions is then:
\begin{equation}
  \text{RVR} =
...
RVR is primarily informative for purely neural world models
