...
Il s'agit d'une synthèse de deux processus qui entrent dans le cadre des objectifs de l'OMARL. Le premier processus détérmine les spécifications organisationnelles des politiques des agents, le second processus entraine les politiques conjointes en satisfaisant les spécifications organisationnelle données. Une vue illustrative de \emph{PRAHOM} est donnée dans \autoref{fig:prahom_process}.
...
Dans le cadre de CybMASDE
%
\footnote{\emph{Cyberdefense Multi Agent System Developpment Environment} est un simulateur pour SMA de Cyberdéfense et disponible à \url{https://github.com/julien6/CybMASDE}}
%
...
%
\emph{PRAHOM Wrapper} fournit des moyens d'inferrer des spécifications organisationnelles à partir d'agents entrainés ou contraindre leur entrainement.
...
%
Dans \autoref{lst:wrapper_basic_use}, nous détaillons une utilisation basique du wrapper pour augmenter un environnement \emph{PettingZoo} (ligne 5) avec des relations connues entre les spécifications organisationnelles et les historiques attendus (ligne 3) ainsi que les spécifications organisationelles qui contraignent les politiques possibles des agents (ligne 4).

\emph{PRAHOM Wrapper} est utilisé avec une fonctionalité supplémentaire qui permet l'entrainement avec PPO et la configuration par défaut (ligne 6).
Pendant l'entrainement, \textquote{agent\_0} est contraint au rôle \textquote{follower} de sorte que ses actions doivent être choisies d'après les historiques attendus (exprimables de diverses manières en plus de liste). Le respect de ces contraintes est faisable selon trois modes: i) \textquote{CORRECT} : masque/corrige de façon externe aux politique les observations reçues et actions choisies  ; ii) \textquote{CORRET\_AND\_PENALIZE} : similaire à \textquote{CORRECT} mais les agents apprennent également à respecter les contraintes ; iii) \textquote{CORRECT\_AT\_POLICY} : change en interne les politiques pour integrer les contraintes (par défaut en masquant/corrigeant les observations/actions).

Après entrainement, \emph{PRAHOM Wrapper} infère les spécifications organisationnelles de l'organisation généralisée à partir de cinq historiques conjoints (obtenus en faisant jouer les agents entrainés sur cinq épisodes) ; ainsi que l'instanciation de cette organisation pour chaque historique conjoint (ligne 7).
Ce processus d'inférence utilise d'abord les relations connues entre les historiques et les spécifications organisationnelles (ligne 3) : l'historique d'un agent qui contient l'observation $14$ (\textquote{commande reçue}) et l'action $74$ (\textquote{appliquer ordre}), est un \textquote{follower}. De même, pour les liens entre roles en utilisant des expressions régulières pour les historiques (ligne 3).

Ensuite, en s'appuyant sur la définition donnée de chaque spécification organisationnelle vis-à-vis des historiques, \emph{PRAHOM Wrapper} généralise plusieurs historiques conjoints pour obtenir de nouvelles spécifications organisationnelles.
%
...
À partir des premières spécifications organisationelles obtenues, un algorithme proposée déduit empiriquement d'autres spécifications organisationnelles telles que les compatibilités, les autorisations et les obligations.
Ces résultats peuvent être incomplets ou bruités en raison des limites des techniques et de l'algorithme d'inférence des spécifications organisationelles. Cependant, les résultats étant conformes à $\mathcal{M}OISE^+$, il est possible de les utiliser dans les méthodes de conception MAS à la lumière des spécifications organisationnelles identifiées.
...
env=prahom_wrapper(env,action_to_specs,training_specs, "CORRECT", ["sequence_clustering"], ["role"], ["dendogram", "PCA"])
...
\footnotetext[1]{Des explications supplémentaires et les exemples discutés utilisant le \emph{PRAHOM Wrapper} sont disponibles sur \url{https://github.com/julien6/omarl_experiments?tab=readme-ov-file\#predator-prey-with-communication}}
