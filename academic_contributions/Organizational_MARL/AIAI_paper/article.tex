...
\paragraph{\textbf{Structural Specifications (SS)}}: $SS = \langle \mathcal{R}, \mathcal{SG}, \mathcal{L}^{intra}, \mathcal{L}^{inter}, \mathcal{C}^{intra}, \mathcal{C}^{inter}, \allowbreak np, ng \rangle$, where: 
\begin{enumerate*}[label=$\bullet$,itemjoin={;\quad}]
...
\begin{enumerate*}[label=$\bullet$,itemjoin={;\quad}]
    \item $SCH = \langle\mathcal{G}, \mathcal{M}, \mathcal{P}, mo, nm \rangle$: the set of \textbf{social scheme}, where:
          \begin{enumerate*}[label=$\bullet$,itemjoin={;\quad}]
              \item $\mathcal{G}$ is the set of global goals;
...
\begin{enumerate*}[label=$\bullet$,itemjoin={;\quad}]
    \item $TC$: the set of \textbf{time constraints}. A time constraint $tc \in TC$ specifies a set of periods determining whether a permission or an obligation is valid;
    \item $OBL$: the set of \textbf{obligations}. An obligation $obl(\rho_a,m,tc)$ means an agent playing role $\rho_a \in \mathcal{R}$ is obliged to commit on mission $m \in \mathcal{M}$ for a given time constraint $tc \in TC$;
...
\paragraph{\textbf{Inferring organizational specifications from joint-policies}}
...
\paragraph{\textbf{Constraining joint-policies satisfying organizational specifications}}
...
Finally, after executing one test episode, it returns a suboptimal joint-policy $\pi_{joint}$ satisfying $os_{init}$. We can note the restriction of $OS_init$ in the possible joint-policies might prevent the MARL algorithm from finding a joint-policy that satisfies the minimal score (expected cumulative reward) defined by the designer.
...
\begin{figure}[h!]
    ...
    \caption{Average reward for each iteration in Pistonball environment for PTS}
...
\begin{figure}[h!]
    ...
    \caption{Principal Component Analysis (PCA) of the trained agent histories in Pistonball environment}
