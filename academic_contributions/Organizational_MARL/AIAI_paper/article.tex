...
In section II, after briefly introducing the underlying idea linking $\mathcal{M}OISE^{+}$ with MARL; we show the related works we identified. It shows the originality of our positioning in literature.
...
The premise of our idea is to benefit of a MARL process that would automatically converge to an optimal solution as for establishing the rules in agents (called policies) that drives the MAS to the goal. Yet, unlike human-based design where the agent's logic is clearly specified, trained policies with MARL are approximated by black box functions such as Neural Networks. Therefore, we also need to address the explainability issue not only for individual agents' policies but for the joint-policy in its entirety through a collective point-of-view. We think, this problem can be partially addressed by considering that a joint-policy can be (at least partially) described in terms of the organizational specifications. We chose $\mathcal{M}OISE^+$ as our organizational model for it gives an advanced formal description for an organization without incompatibilities with MARL, especially for formal description of agents' policies. Additionally, it provides a sufficiently detailed vision of organization to be understood at different point of views.
