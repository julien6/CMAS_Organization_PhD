...
\begin{minipage}[t]{0.55\textwidth}
    ...
    \begin{itemize}
        \item \textquote{Predator-prey with communication}~\cite{Lowe2017} (PPY) that consists in predators monitored by a leader to better catch preys;
        \item \textquote{Pistonball} (PBL)~\cite{Terry2021} that consists in a series of piston to bring a ball from right to left side;
        \item \textquote{Knights Archers Zombies}~\cite{Terry2021} (KAZ) that consists in a few knights and archers learning how to kill as much zombies a possible;
        \item \textquote{Drone swarm - 3rd CAGE Challenge}~\cite{cage_challenge_3_announcement} (CYB) that consists in blue agents deployed in drones while trying to tackle embedded malware programs' impact.
    \end{itemize}
...
\begin{minipage}[t]{0.4\textwidth}
    \begin{figure}[H] % Use the 'H' specifier from the 'float' package to force the figure placement
        \centering
        \includegraphics[width=\linewidth]{figures/envs.png} % Replace "example-image" with your figure file name
