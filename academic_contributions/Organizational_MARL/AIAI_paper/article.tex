...
\begin{document}
%
\title{AMOEA, An Assisted Approach for MAS Organization Engineering}
%
...
\begin{abstract}
    ...
    We introduces AMOEA (Assisted MAS Organization Engineering Approach), an novel design approach to assist the MAS design whose underlying idea is to use Multi-Agent Reinforcement Learning (MARL) with organizational specifications for both understanding and constraining the training process regarding design constraints.
    % results
...
%    - Resulting PRAHOM organizational specification are indeed matching the expected ones
%    - Learning in MARL with PRAHOM is indeed impacted by constraining organizational specifications and allows designer to take hand on the way agents' learn at an organization level
...
%    - Most resulting PRAHOM organizational specifications outlines some expected collective strategies
...
% \textquote{Google Research Football Environment}~\cite{Kurach2020},


We selected 4 environments to assess our approach: \textquote{Predator-prey with communication}~\cite{Lowe2017} that consists in predators monitored by a leader to better catch preys; \textquote{Pistonball}~\cite{Terry2021} that consists in a series of piston to bring a ball from right to left side; \textquote{Knights Archers Zombies}~\cite{Terry2021} that consists in a few knights and archers learning how to kill as much zombies a possible; \textquote{Drone swarm - 3rd CAGE Challenge} that consists in blue agents deployed in drones while trying to tackle embedded malware programs' impact.
...
As a general observation, we can notice convergence time is biger for NTS than for PTS which is also bigger than for FTS. That makes sense for the search space is decreasing, hence a shorter convergence time. Global performance indicated by cumulative reward show NTS to outperform PTS and FTS for agents are overfited to solving the problem much more finely than the designer's specifications can do.
