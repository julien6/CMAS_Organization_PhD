...
Whether an OCPV or ACPV, when designing a MAS, the human designer has to define the logic in agents themselves intending the MAS can reach some specific goals. It often takes place as an iterative process where designer are proceeding by trial and error. Despite the designer skills, that process may be long to converge towards

The main idea is to consider a joint-policy can be (at least partially) described in terms of organizational specifications.
