...
\section{Action constraining during training implies to constrain joint-policy set}
    ...
    So, instead, we may observe the \textquote{apparent} behavior of agents to check thanks to their histories to check any mismatch. Actually, we hypothesize it is possible dealing that \textquote{selection} throughout the training by forcing agents to make the expected decision at each step.
