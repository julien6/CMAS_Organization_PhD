...
\begin{relation}\label{rel:def_extended_dmo}
    $\\$ dmo_{ext}: D \times \mathcal{OS}_{cons} \rightarrow (SR_{joint,i,s}[D] \cap rct[osh[\mathcal{OS}_{cons}]]) \times osh^{-1}[obs[SR_{joint,i,s}[D]] \cap osh[\mathcal{OS}_{cons}]]
\end{relation}
...
\begin{algorithm}[hbt!]
    ...
    \KwData{$os_{cons} \in \mathcal{OS}_{cons}$, the constraining org. specs.}
    \KwData{$marl-alg: \Pi_{joint} \times H_{joint} \times \mathbb{R} \rightarrow \Pi_{joint}$, the MARL algorithm approach for training a joint policy from explored histories and associated rewards}

    \KwResult{$(s\pi_{joint,i,s} \in S\Pi_{joint,i,s}, os_{i,s} \in \mathcal{OS}_{i,s})$, the sub-optimal policies and associated org. specs.}
...
\begin{figure*}
    \centering
    \input{figures/gosi_illustrative_view.tex}
...
\begin{algorithm}[hbt!]
    ...
    \KwData{$h_{joint}$, the joint-policies to be analyzed}
...
\begin{algorithm}[hbt!]
    ...
    \KwData{$solv: \Pi \times A \times O \times \mathbb{R} \rightarrow \Pi$, the MARL algorithm approach for updating a single policy in order to solve the Dec-POMDP}
    \KwResult{$s\pi_{joint} \in S\Pi_{joint,i,s}$, the sub-optimal joint-policies that satisfy the design specifications}
