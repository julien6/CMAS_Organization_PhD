...
\begin{algorithm}[hbt!]
    ...
    \KwData{$marl-alg: \Pi_{joint} \times H_{joint} \times \mathbb{R} \rightarrow \Pi_{joint}$, the MARL algorithm for training a joint policy from explored histories and associated rewards}

    \KwResult{$(s\pi_{joint,s,i} \in S\Pi_{joint,s,i}, os_{s,i} \in \mathcal{OS}_{s,i})$, the sub-optimal policies and associated org. specs.}
    ...
    $os_{i,s} = PRAHOM-hos(s\pi_{joint,s,i})$
...
\begin{figure*}
    \centering
    \input{academic_contributions/Organizational_MARL/ECAI_paper/figures/gosi_illustrative_view}
