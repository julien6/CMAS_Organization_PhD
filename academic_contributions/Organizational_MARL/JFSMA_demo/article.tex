...
Si un historique n'est pas déjà associé à une spécification organisationelle, \emph{PRAHOM Wrapper} cherche à généraliser des roles non pré-établis par mesure de similarité entre les historiques des agents. Cela est présentement fait de plusieurs façons complémentaires : par clustering de séquence auquel un dendogramme associé est sauvergardé en accompagnement ; par recherche des K-voisins les plus proches des historiques auquel une ACP des historiques est proposé en accompagnement ; par analyse statistiques (fréquences, variances des couples observation-action) auquel des diagrammes circulaires et des visulations \textquote{en pixel} sont proposés en accompagnement ; etc.

De même des sous-objectifs non pré-établies peuvent être généralisés de plusieurs façons complémentaires : par analyse de la fréquence des observations finales commune des agents jouant un même role auquel un digramme de transition d'état est généré en accompagnement ; par analyse des états seuils récurrents déclenchant une amélioration notable de l'évolution de la récompense.
