...
%
(\emph{Autonomous Intelligent Cyberdefense Agent}\footnote{La recherche sur les AICA a été initiée le cadre du groupe \emph{NATO IST-152} puis de l'\emph{AICA International Work Group} : \url{https://www.aica-iwg.org/}.})
%
...
Un agent AICA peut être vu comme un SMA de Cyberdefense~\cite{Singh2015} où la couverture de Cyberdéfense est distribuée sur un ensemble d'agents de Cyberdéfense collaborant entre eux de façon autonome. La flexibilité d'un SMA de Cyberdéfense renforce sa résilience en lui permettant de s'adapter continuellement aux contraintes de l'environment tout en respectant des politiques de sûreté. Les aspects organisationels ont été considerés dans l'ingénierie d'un tel SMA au sein des travaux de recherche.

%% Elargir le sujet des SMAC/AICA au contexte des SMA en général
Au delà du contexte de Cyberdéfense, nous désignons l'\textbf{organisation} comme un concept regroupant deux sens.
D'une part, l'\textbf{entité de l'organisation} qui désigne l'ensemble des logiques internes des agents qui peut être considerée du point de vue individuel jusqu'au point de vue collectif. D'autre part, le \textbf{support organisationel} qui explicite comment les agents coordonnent leurs activités pour atteindre de manière collaborative un objectif commun~\cite{Picard2009}.

Les méthodes de conception telles que GAIA~\cite{Wooldridge2000,Cernuzzi2014}, ADELFE~\cite{Mefteh2015}, MaSE~\cite{Deloach2001}, DIAMOND~\cite{Jamont2015} ou KB-ORG~\cite{Sims2008} cherchent à tirer profit de connaissances empiriques pour converger itérativement vers une organisation adéquate pour un environnement et un objectif donné.

Néanmoins, 
Cependant, la complexité et le manque de connaissance des systèmes cibles peut rendre coûteux l'application de ces méthodes et ne garantit pas de converger vers une SMA répondant aux exigences de performance~\cite{Mefteh2013}.
...
Cependant, il n'y aucun moyen agnostique de l'environnement cible permettant d'automatiser totalement la recherche des logiques internes des agents (que nous appelons également \textbf{politiques}) satisfaisant les exigences données tout en explicitant les mécanismes organisationnels. %émergents. -> JS : j'enlève "émergent" car les mecanismes organisationels ne sont pas tous émérgents. Certains sont intentionellement voulus.
