...
\noindent Then, we identified two main issues preventing any implementation of these features in both $(\mathbf{G_A})$ and $(\mathbf{G_E})$ gaps:
%
\begin{itemize}[wide, labelsep = 1em]
...
To address these issues, the premise of our contribution is to leverage history as an explicit core concept in MARL to represent organizational specifications in MARL. This idea led to the following additional theoretical tools we propose as prior requirements.

\

\noindent \textbf{Mapping from role/mission to histories} \quad We introduce the $sfh$ bijection that maps a role or goal (thus a mission) to a history subset.
...
\noindent $rh$ and $gh$ the bijections that show how an agent playing a role and committed to a mission should have its policy constrained or enticed to generate histories belonging to an expected history subset. Thus, $mh: \mathcal{M} \rightarrow \mathcal{P}(H) = \{(m,\bigcup gh[mo(m)]), m \in \mathcal{M}\}$ gives histories from missions. The $sfh$ inverse is denoted $hsf = sfh^{-1}$.

\

\noindent \textbf{Mapping from permission/obligation to agents} \quad We introduce the $da$ bijection that maps a permission/obligation to an agent subset.
...
We selected the \textquote{Predator-prey with communication}~\cite{Lowe2017} Atari-like environment that consists of predators monitored by a leader to catch faster prey hence requiring communication for hunting strategies
%
