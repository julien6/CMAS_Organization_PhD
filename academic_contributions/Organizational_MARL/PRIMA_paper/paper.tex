...
To work around issue $(I1)$, we propose to approximate an agent policy $\pi \in \Pi, \pi: \Omega \rightarrow A$ as a history subset $H$ so that all histories generated by the given policy $h = \langle (\omega,\pi(\omega)), \omega \in \Omega \rangle$ belong to that history subset as $h \in H$.
