...
In Annexes, we provide \autoref{proof:jpc_to_ac}, which outlines why constraining the action decision-making process dynamically during training at each step implies that the resulting joint-policy will necessarily be constrained.
...
%
\footnote{Screenshots are provided in Annexes \ref{sec:screenshots}, as well as a video showing the running \textit{manual\_model} in \url{https://shorturl.at/zIYMj}}
%
. This visual representation shows alignment with expected collective strategies ($\mathbf{C1}$). The \textit{manual\_model} also exhibited consistent circle behavior further demonstrating the robustness of predefined policies.
