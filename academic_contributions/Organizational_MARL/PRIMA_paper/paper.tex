...
\begin{abstract}
    ...
    This paper addresses the challenge of leveraging on Multi-Agent Reinforcement Learning (MARL) for MAS design by making it explicitly usable for both constraining the agents' training regarding some specifications and explaining trained agents in \textquote{human-readable} manner. While previous studies have focused on a single agent for guiding its training or explicating its behavior, a multi-agent context shows the need to elucidate the implicit cooperation among multiple agents in MARL systems during and after training. We propose a novel algorithmic approach leveraging the $\mathcal{M}OISE^+$ Organizational Model, called \emph{Partial Relation between Agents' History and Organizational Model} (PRAHOM). Our algorithm's underlying principle consists in linking organizational specifications, such as roles or missions, to their respective agents' histories characterizing their behaviors. PRAHOM serves dual purposes: constraining the learning process based on organizational constraints and inferring organizational specifications from trained agents' histories. Evaluations conducted in a cooperative \textquote{Predator-prey} environment validate the effectiveness of our algorithm, showing alignment with hand-crafted expectations and superior performance in some scenarios.
    ...
    \keywords{Multi-Agent Reinforcement Learning \and Explainability \and Organizational Models \and Assisted engineering}
...
% Problem
For systems using MARL, a set of agents learn to achieve goals that may imply implicit cooperation and coordination. Very few works have attempted to analyze the trained agents' policies to make this cooperation explicit and attempted to handle it directly~\cite{albrecht2018survey,perolat2017pool}. Thid lack of explainability is core to two main gaps we intend to cover:
%
\begin{itemize}
    \item[$(G1)$] Understanding the organizational schemes such as social or collective links that emerge between the trained agents in a bottom-up manner;
    \item[$(G2)$] Applying hand-crafted organizational specifications to agents to guide their training in a top-down manner, hence providing safety guarantees.
\end{itemize}
%
...
Addressing $G1$ and $G2$ requires linking joint-policies and a set of organizational specifications whose triplet \textquote{role-permission/obligation-mission} is core. We defined this linking as eventually establishing the following features:
%
\begin{itemize}
    \item[$G1.F1$] If an agent is constrained to a given role, then its policy should necessarly belong to a policy subset associated with the given role throughut the training;
    \item[$G1.F2$] If an agent is constrained to a mission, then it should be enticed to achieve the mission's goals according to the associated plan throughut and after the training.
    \item[$G2.F1$] For any given joint-policy obtained after/during the training, it should be possible to determine structural specifications containing at least a set of likely matching roles for each agent even if no agents have been explicitely constrained to roles;
    \item[$G2.F2$] For any given joint-policy obtained after/during the training, it should be possible to determine functional specifications containing at least a set of likely matching missions for each agent even if no agents have been explicitely constrained to missions;
    \item[$G2.F3$] For any given joint-policy obtained after/during the training, it should be possible to determine deontic specifications containing at least a set of likely matching permissions/obligations for each agent even if no agents have been explicitely constrained to a role and permitted/obligated to missions;
\end{itemize}

We identified two main issues for implementing these features:
...
To work around issue $I1$, we propose to define an agent policy as a history subset so that all histories generated by the given policy should belong to that history subset. We formalize it as: For any policy $\pi \in \Pi, {(\omega,\pi(\omega)), \omega in \Omega} = H$
