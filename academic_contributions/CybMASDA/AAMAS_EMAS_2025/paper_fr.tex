...
Pour une spécification déontique $(\rho,m,t_c)$ appliquée à un seul agent, ce dernier contraint à un rôle $\rho \in mathcal{R}$ et permis/autorisé  à s'engager à une mission, la fonction de valeur $V^\pi$ a la forme suivante :
\begin{figure*}
...
Avec $rag(\omega_t) = A_{t+1} \times \mathbb{R} = ach, \ ach(a_{t+1}) = ch_{t+1} ; \text{ and } rn: \emptyset \to [0,1]$, a uniform random function ; rsm: 
\end{figure*}
