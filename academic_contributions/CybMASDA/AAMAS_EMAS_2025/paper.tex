...
One of the strengths of SAMMASD, as implemented within CybMASDE, is its adaptability. The method allowed for quick reconfiguration of roles, objectives, and environmental constraints in response to changing scenarios. The tool’s integration with PettingZoo and Gymnasium enabled seamless adaptation to different environments, while the modular design facilitated the re-deployment of agents with updated policies. However, while the method proved adaptable to various simulated environments, further improvements are needed to enhance real-time adaptability in dynamic real-world contexts.

\subsection{Contribution and Future Work}

In conclusion, SAMMASD, with the support of CybMASDE, addresses several critical gaps in the current methodologies for MAS design. The method’s structured approach to incorporating organizational specifications within MARL frameworks enhances the design’s robustness and explainability, which are often limited in purely MARL-based approaches. Additionally, SAMMASD’s semi-automated nature reduces the dependency on expert input, making it accessible to a broader range of users.

Despite these advances, there remain areas for future improvement. While the method facilitates the creation of explainable and safety-compliant policies, real-time adaptability in complex, dynamic environments requires further research. Full automation in the deployment phase is another area for enhancement, as it currently necessitates manual adjustments to ensure compliance with performance and safety requirements.

Future work will focus on refining the CybMASDE tool to include automated deployment pipelines and enhanced real-time adaptability features. The integration of advanced learning algorithms, such as those suited for online learning in non-stationary environments, is also planned. Additionally, work is underway to extend the SAMMASD framework’s applicability to critical domains, such as cybersecurity and healthcare, where MAS must operate under stringent safety and resilience requirements. By addressing these gaps, we aim to make SAMMASD a more versatile and powerful solution for MAS design in increasingly complex and dynamic environments.
\section*{References}
