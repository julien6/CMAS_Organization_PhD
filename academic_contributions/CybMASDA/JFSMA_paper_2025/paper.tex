...
\begin{resume}
  L'apprentissage par renforcement multi-agent (MARL) peut favoriser des comportements collaboratifs s'apparentant à une organisation. Nous proposons le cadre MOISE+MARL, qui fait évoluer explicitement les agents en tant qu'organisation. En assignant des rôles et des objectifs aux agents, MOISE+MARL permet de contrôler l'apprentissage en ajustant les actions et les récompenses. De plus, il propose une analyse post-entraînement pour inférer des spécifications organisationnelles implicites, apportant des éclaircissements sur les comportements émergents. MOISE+MARL a été évalué dans divers environnements et algorithmes de MARL.
\end{resume}
...
\begin{abstract}
  Multi-Agent Reinforcement Learning (MARL) can foster collaborative behaviors that resemble an organization. We propose the MOISE+MARL framework, which explicitly make agents evolve as an organization. By assigning roles and goals to agents, MOISE+MARL enables control over learning by adjusting actions and rewards. Moreover, it offers a post-training analysis to infer implicit organizational specifications, providing insights into emergent behaviors. MOISE+MARL has been evaluated in various MARL environments and algorithms.
\end{abstract}
...
% Contexte  
L'apprentissage par renforcement multi-agent (MARL) permet de découvrir une politique conjointe qui régit à la fois les actions individuelles et les interactions entre agents, sans nécessiter d'organisation prédéfinie. Dans des environnements à forte interaction sociale, les agents adoptent souvent des comportements récurrents, traduisant des rôles et objectifs implicites, qui forment une organisation structurelle et fonctionnelle, telle que dans $\mathcal{M}OISE^+$~\cite{Hubner2007}.


% Problématique  
Nous définissons l'\textbf{adéquation organisationnelle} comme la mesure de la similitude entre le comportement réel des agents et une organisation. Deux aspects clés sont alors à explorer :
i) \textbf{L'évaluation de l'adéquation organisationnelle} consiste à mesurer l'alignement d'une politique conjointe avec une organisation idéale, en tenant compte des contraintes environnementales et des objectifs. La littérature, souvent centrée sur les rôles ou objectifs~\cite{Isakov2024, Wen2024, Xie2024}, manque d'approches systématiques dotées d'outils quantitatifs et qualitatifs.
 ; \quad
ii) \textbf{Le contrôle de l'adéquation organisationnelle} consiste à orienter les agents vers des politiques conformes à une organisation via des contraintes ou incitations définies par l'utilisateur. Les défis incluent la réduction de l'espace de recherche, l'amélioration de la convergence et le respect des contraintes de sécurité, sans recourir à l'apprentissage par renforcement hiérarchique (HRL).


% Contribution  
\noindent Cet article présente le cadre \textbf{MOISE+MARL}~\cite{soule2025moisemarl}. MOISE+MARL apporte une contribution synthétique aux premiers travaux sur l'intégration d'un modèle organisationnel dans le MARL~\cite{soule2024paper-jfsma, soule2024aomea}. Il combine le Dec-POMDP et le modèle organisationnel $\mathcal{M}OISE^+$~\cite{Hubner2007} via des relations définies, permettant aux utilisateurs de spécifier la logique des rôles et objectifs à partir de motifs de trajectoires. Une fois configuré, le cadre attribue des rôles aux agents en imposant des contraintes qui ajustent dynamiquement l'espace d'actions et la fonction de récompense. Il intègre également la méthode \textit{Trajectory-based Evaluation in MOISE+MARL} (TEMM), qui, par apprentissage non supervisé, infère les rôles et missions implicites à partir de trajectoires observées, offrant ainsi une évaluation quantitative de l'adéquation organisationnelle. Contrairement au HRL, qui décompose les tâches~\cite{Qi2024, Matsuyama2025, SaoMai2024}, notre approche guide les agents par des rôles et des missions explicites.

% Évaluation et résultats  
Nous avons évalué MOISE+MARL dans trois scénarios :
i) Quatre environnements différents, chacun entraînant des politiques présentant des organisations implicites variées, afin d'évaluer la généralisabilité du cadre.
 ; \quad
ii) Quatre algorithmes MARL de familles distinctes, pour mesurer leur adéquation avec MOISE+MARL lors de l'entraînement et de l'analyse postérieure.
 ; \quad
iii) Quatre ensembles de spécifications organisationnelles, un par environnement, pour contraindre les agents et permettre une évaluation manuelle et quantitative.
%
Dans tous les cas, les agents adoptant des rôles se comportent comme prévu, en accord avec une mesure quantitative d'adéquation organisationnelle obtenue via TEMM. Les rôles et missions inférés s'alignent étroitement sur les spécifications prédéfinies, démontrant la cohérence interne du cadre. Par ailleurs, les algorithmes basés sur la politique et les actor-critic produisent des politiques stables, tandis que ceux basés sur la valeur présentent une variabilité plus importante.
...
Cette section explore des travaux liés à l'adéquation organisationnelle, telle qu'elle est définie par les deux problématiques fondamentales introduites.
...
\noindent \textbf{Spécifications structurelles (SS):}\$0.2em]
...
\vspace{0.5em}
\noindent \textbf{Spécifications fonctionnelles (FS):}\$0.2em]
...
\vspace{0.5em}
\noindent \textbf{Spécifications déontiques (DS):}\$0.2em]
...
Nous avons identifié les modèles organisationnels \textit{AGR}~\cite{ferber2003} (Agent Group Role) et \(\mathcal{M}OISE^+\)~\cite{Hubner2007}. Alors qu'AGR est un cadre informel qui introduit des rôles par groupes, \(\mathcal{M}OISE^+\) offre une description plus détaillée et flexible des structures et fonctions d'un système multi-agent (MAS), facilitant ainsi la formalisation des politiques en MARL.
...
i) un bonus (via les Guides de Récompense d'Objectif), pondéré par \(\frac{1}{1-p+\epsilon}\), et
ii) une pénalité (via les Guides de Récompense de Rôle), pondérée par la rigidité \(ch_t\).
...
Nous avons développé une API Python, \textquote{MMA}~\hyperref[fn:github]{\footnotemark[1]}, pour implémenter MOISE+MARL. Basée sur une approche orientée objet, cette API structure le modèle \(\mathcal{M}OISE^+\) en classes de données imbriquées (la classe « Moise » étant à la racine) afin de définir les spécifications organisationnelles (rôles, objectifs, permissions).

Pour les environnements Dec-POMDP, nous utilisons la bibliothèque \textit{PettingZoo}~\cite{terry2020pettingzoo} – similaire à Gymnasium~\cite{kwiatkowski2024} – et intégrons un dictionnaire personnalisable pour le mappage des étiquettes d'observation/action (\(l\)), ainsi que le support des Trajectory Patterns (TP) pour définir et faire correspondre les motifs.

Chaque \textbf{Guide de Contrainte} (\(rag\), \(rrg\) et \(grg\)) est implémenté en tant que classe distincte. Les utilisateurs peuvent les définir via des fonctions personnalisées ou des règles JSON (par exemple, \(rag\) associe un couple \(\langle\)TP, dernière observation\(\rangle\) à des actions attendues, et \(grg\) applique des bonus selon des TP spécifiques). La classe globale MMA intègre ces guides et relie les agents aux rôles via des relations telles que \(ar\), intégrant ainsi les spécifications de \(\mathcal{M}OISE^+\).
...
Nous testons MOISE+MARL dans quatre environnements MARL, modélisés comme des scénarios Dec-POMDP et choisis pour leur diversité en collaboration et gestion des ressources :
...
Toutes les expériences ont été menées sur un cluster académique haute performance, avec des nœuds GPU (NVIDIA A100, V100 et AMD MI210). Chaque paire algorithme-environnement a été exécutée 5 fois en parallèle pour assurer des résultats fiables.
...
Pour mesurer l'efficacité des politiques et l'impact des spécifications organisationnelles, nous avons défini les métriques suivantes :
%
...
Cependant, reposant sur des spécifications prédéfinies, MOISE+MARL peut peiner à prendre en compte le surcoût computationnel (notamment lié à TEMM), posant des problèmes de scalabilité.
%
Nous poursuivons trois axes de recherche :
