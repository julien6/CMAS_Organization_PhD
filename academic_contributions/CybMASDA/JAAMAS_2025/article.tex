...
\begin{figure}[h!]
    ...
    \caption{Normalized time to converge to a minimal cumulated reward over minimal representativeness}
...
\begin{algorithm}[H]
    ...
    \For{$t \in \{obs,trans\}$}{
        \ForEach{distance metric $\delta_t \in \{0.0, 0.1, ..., 1.0\}$}{
            \ForEach{minimal cluster distance $\tau_t \in \{0.0, 0.1, ..., 1.0\}$}{
    ...
    \tcp*[l]{4. Sweep on repr. to evaluate convergence times as func. $ct_t$}
    \For{$t \in \{obs,trans\}$}{
        \ForEach{representativeness $\rho_t \in \{0.0, 0.1, ..., 1.0\}$}{
        ...
        \tcp*[l]{5. Select representativeness corresponding to threshold $\eta$ $(=3.5\%)$}
        $\rho_t^* \gets \arg_{ct_t }(\eta)$ \tcp*[r]{Elbow point or exact threshold}

    }

    \tcp*[l]{6. Final role/goal inference using $\rho^*$}
