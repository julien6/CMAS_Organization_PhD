...
Designing \textbf{MAS} for complex real-world applications, such as cybersecurity, logistics, autonomous robotics, or intelligent transportation, requires methodologies that ensure both structured and goal-oriented agent behaviors~\cite{Jamont2O15}. Traditionally, the \textbf{AOSE} paradigm has provided systematic frameworks for specifying and developing MAS, emphasizing the design of agents, roles, and interactions~\cite{Pavon2003, Bernon2005}. AOSE methodologies typically rely on explicit expert's knowledge and predefined organizational models to structure agent behavior, ensuring predictability, reliability and safety~\cite{Hindriks2014}.
...
\[
    \texttt{model}: \left( \mathcal{E}, \mathcal{G}_{inf}, \mathcal{C}_{inf} \right) \longrightarrow (\mathcal{T}_i, R, \mathcal{MM}_i)
\]
where $d_i = (\mathcal{T}_i, R, \mathcal{MM}_i)$ denotes the extended Dec-POMDP that model the problem to solve. This involves three main outputs:

\begin{itemize}
    \item An \textbf{Observation Prediction Model} $\mathcal{T}^{j}(h_{t-1}^{j}, \omega_t^{j}, a_t^{j}) = \hat{\omega}_{t+1}^{j}$, which approximates the next joint-observation in response to joint-action, previous joint-observation, and a given a joint-history. This function plays the role of both the observation and transition functions in a Dec-POMDP acting as a simulation-basis for experimenting design, which is a common practice in AOSE methods~\cite{Jamont2O15};
...
The modeling process follows three main steps described in \autoref{alg:model}:
\begin{enumerate}
...
\begin{algorithm}[H]
    \caption{Modeling phase: definition of \texttt{model} relation}
    \label{alg:model}
    \DontPrintSemicolon
    \KwIn{Environment $\mathcal{E}_i$, Informal goal $\mathcal{G}_{inf}$, Informal constraints $\mathcal{C}_{inf}$}
    \KwOut{Observable dynamics $\mathcal{T}_i$, reward function $R$, organizational specification $\mathcal{MM}_i$}
    
    \textbf{Step 1: Environment Modeling} \\
    \eIf{Manual modeling}{
        Design simulator $\mathcal{T}_i$ manually based on known rules
    }{
        Collect trajectories $\mathcal{H}_{data}$ from $\mathcal{E}_i$ \\
        Train Observation Prediction Model $\mathcal{T}_i$ using Recurrent Latent Dynamics:
        \[
            \mathcal{T}_i(h_{t-1}, \omega_t, a_t) := Dec(g(f(h_{t-1}, Enc(\omega_t), a_t)))
        \]
    }
    
    \textbf{Step 2: Reward Function Definition} \\
    \eIf{Manual reward definition}{
    Define reward function $R: \Omega^j \times A^j \to \mathbb{R}^{|\mathcal{A}|}$
    }{
    Use Inverse Reinforcement Learning (IRL) over $\mathcal{H}_{data}$ to estimate $R$
    }
    
    \textbf{Step 3: Organizational Specifications Integration (optional)} \\
    \eIf{Domain knowledge available}{
        Define organizational specification $\mathcal{MM}_i = \langle \mathcal{OS}, ar, rcg, gcg, rag, rrg, grg \rangle$
    }{
        Leave $\mathcal{MM}_i$ undefined for later inference
    }
    
    \Return $(\mathcal{T}_i, R, \mathcal{MM}_i)$
\end{algorithm}
...
\subsubsection{General workflow}
The training phase follows these key steps described in \autoref{alg:train}:
\begin{enumerate}
...
\begin{algorithm}[H]
    \caption{Training phase: definition of \texttt{train} relation}
    \label{alg:train}
    \DontPrintSemicolon
    \KwIn{Design problem $d_i = \langle \Omega^j, A^j, \mathcal{T}_i, R, \gamma \rangle$, Organizational specification $\mathcal{MM}_i$}
    \KwOut{Trained joint policy $\pi^j_i$}
    
    \textbf{Step 1: Apply constraint guides (if defined)} \\
    Embed $rag$, $rrg$, $grg$ into simulation loop to modulate actions and rewards.
    
    \textbf{Step 2: Hyperparameter Search Space Generation} \\
    Extract environment features $F_{env}$ and resources $F_{res}$ \\
    Define HPO search space $\mathcal{H}$ using expert rules on $F_{env}, F_{res}$
    
    \textbf{Step 3: Hyperparameter Optimization (HPO)} \\
    \ForEach{hyperparameter config $h \in \mathcal{H}$}{
        Train MARL algorithm with config $h$ \\
        Evaluate policy $\pi^j_h$ on performance metrics (reward, stability, constraints)
    }
    
    \textbf{Step 4: Selection of best configuration} \\
    Select $\pi^j_i = \arg\max_{\pi^j_h} \text{Performance}(\pi^j_h)$
...
\subsubsection{General workflow}
The analysis phase follows these key steps described in \autoref{alg:analyze}:
\begin{enumerate}
...
\begin{algorithm}[H]
    \caption{Analyzing phase: definition of \texttt{analyze} relation (TEMM)}
    \label{alg:analyze}
    \DontPrintSemicolon
    \KwIn{Design problem $d_i$, Organizational specification $\mathcal{MM}_i$, Trained policy $\pi^j_i$}
    \KwOut{Inferred organizational specification $\mathcal{MM}_{implicit}$, Organizational Fit score $\mathit{OrgFit}$}
    
    Collect test trajectories $\mathcal{H}_{test}$ by executing $\pi^j_i$
    
    \textbf{Step 1: Role Inference via Cluster-Centric Role Abstraction} \\
    Perform hierarchical clustering over agent action sequences \\
    Extract Common Longest Sequences (CLS) per cluster $\rightarrow$ infer roles $\mathcal{R}_{implicit}$ \\
    Compute role inheritance relations via CLS containment
    
    \textbf{Step 2: Goal and Mission Inference} \\
    Construct Joint-Observation Transition Graphs from $\mathcal{H}_{test}$ \\
    Apply K-means clustering over joint-observations to identify abstract goals $\mathcal{G}_{implicit}$ \\
    Aggregate goals into missions $\mathcal{M}_{implicit}$
    
    \textbf{Step 3: Permission and Obligation Inference} \\
    For each agent-role association, determine valid mission sets \\
    Infer permissions and obligations based on exclusivity and consistency
    
    \textbf{Step 4: Organizational Fit Evaluation} \\
    Compute Structural Fit: alignment of CLS vs actual behaviors \\
    Compute Functional Fit: alignment of goal clusters vs observed interactions \\
    Set $\mathit{OrgFit} = \frac{1}{2} \times (StructuralFit + FunctionalFit)$
    
    \Return $(\mathcal{MM}_{implicit}, \mathit{OrgFit})$
\end{algorithm}
...
%
\footnotetext[2]{ \label{fn:github} Additional details, developed code, datasets containing all the hyperparameters and details of the organizational specifications are available at \url{https://github.com/julien6/MOISE-MARL}}
...
Based on these definitions, TEMM uses a \textquote{hierarchical clustering} technique to find the CLSs among agent histories. The results can be represented as a dendrogram, allowing inferring implicit roles and inheritance relationships, their respective relationships with histories.
We measure the gap between current agents' sequence and inferred implicit roles' sequences, as the \textquote{structural organizational fit}.
...
By measuring the distance between inferred implicit goals which joint-observations with current agents' joint-observation, we compute the \textquote{structural organizational fit}.
...
\subsubsection{General workflow}
This phase follows a systematic deployment strategy, also described in \autoref{alg:transfer}, consisting of:
\begin{enumerate}
...
\begin{algorithm}[H]
    \caption{Transferring phase: definition of \texttt{transfer} relation}
    \label{alg:transfer}
    \DontPrintSemicolon
    \KwIn{Current environment $\mathcal{E}_i$, Trained policy $\pi^j_i$}
    \KwOut{Deployed system $\mathcal{E}_{i+1}$}
    
    \textbf{Step 1: Safe Validation in Emulated Environment} \\
    Deploy $\pi^j_i$ in high-fidelity emulation \\
    Evaluate:
    \begin{itemize}
        \item Behavioral Consistency w.r.t. inferred organizational specifications
        \item Constraint Adherence (safety/performance)
        \item Stability across multiple runs
    \end{itemize}
    
    \textbf{If validation fails:} return to Phase 2
    
    \textbf{Step 2: Choose Deployment Mode} \\
    \eIf{Safety-critical}{
        Use Remote Deployment
    }{
        Use Local Deployment (embedded policies)
    }
    
    \textbf{Step 3: Incremental Real Deployment} \\
    Gradually integrate agents into real system \\
    Perform real-time monitoring of organizational compliance
    
    \Return $\mathcal{E}_{i+1}$
\end{algorithm}
