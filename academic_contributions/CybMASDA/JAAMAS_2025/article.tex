...
We propose the \textbf{MAMAD method}, which extends the proposed \textbf{MOISE+MARL} framework~\cite{soule2025moisemarl}\footnote{This article has been accepted for AAMAS 2025 and is reachable at \url{https://arxiv.org/abs/2503.23615}}, to structure MARL-driven learning. MAMAD enables \textbf{controlled policy learning}, aligning agents with predefined organizational specifications while extracting refined insights from emergent behaviors. It wraps the MOISE+MARL framework into a fully automated, four-phase design process driven by three key inputs: the environment, the user-defined design requirements, and the global goal. Each of these phases follows the optimization under the problem view.
...
This section reviews existing research likely to bridge each of the targeted gaps. A summary is presented as a table in \autoref{tab:related_works}.

\subsection{Gap 1: Integrating AOSE and MARL}

\subsection{Gap 2: Controlling or Guiding MARL}

\subsection{Gap 3: Understanding and Explaining MARL Behaviors}

\subsection{Gap 4: Automating MAS Design}



This section reviews existing research in MAS design, focusing on the intersection between AOSE and MARL. We structure this analysis into three topics. \autoref{sub-sec:aose_rel} presents AOSE methodologies and their limitations in addressing the targeted gaps; \autoref{sub-sec:marl_rel} presents early MARL-driven approaches for MAS design; and \autoref{sub-sec:mm_rel} shows the underlying MOISE+MARL framework to bridge AOSE and MARL, though still lacking full design automation.
...
%
% todo: remettre les noms des gaps avec les gaps initiaux
\begin{itemize}
...
% Common MARL learning paradigms.
...
% Model-based RL, latent dynamics models, VAE + RNN.
% Relevance for creating simulated environments from real data.
...
\subsubsection{Formalizing Organizational Inference in TEMM}
...
\begin{enumerate}
    \item \textbf{Inferring roles and their inheritance} using hierarchical clustering.
    \item \textbf{Inferring goals, plans, and missions} using trajectory clustering and observation-state analysis.
    \item \textbf{Inferring obligations and permissions} by analyzing temporal constraints and goal consistency.
\end{enumerate}
...
\subsubsection{Inferring Roles and Their Inheritance}
...
\noindent \textbf{Measuring Structural Organizational Fit:}  
...
\subsubsection{Inferring Goals, Plans, and Missions}
...
\noindent \textbf{Final Organizational Fit Calculation:}  
\begin{equation}
