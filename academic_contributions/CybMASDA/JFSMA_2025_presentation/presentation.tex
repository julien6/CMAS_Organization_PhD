...
\begin{frame}{De motifs émergents à l'adéquation organisationnelle}
  ...
  \begin{block}{\textbf{Adéquation organisationnelle} : un concept pratique proposé comme\dots}
    La \textquote{distance} entre comportements des agents entraînés et ceux d'une organisation \textquote{régulière}.
  \end{block}
...
\begin{frame}{Le cadre MOISE+MARL}{Solution 1 : Guides de contrainte via scripts personnalisés}
  ...
  \begin{itemize}
    ...
    \item \textbf{Observation :} L’agent tient un bol, un pot est prêt, pas de mur devant
    \item \textbf{Trajectoire :} L’agent a précédemment pris un oignon
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item \textbf{RRG :} Bonus de +2 si `interact` est utilisé à proximité d’un pot prêt
  \end{itemize}
...
\begin{frame}{Le cadre MOISE+MARL}{Solution 2 : Guides de contrainte via \textit{Trajectory-based Pattern} (TPs)}
  ...
  \begin{itemize}
    \item \textbf{Patron :} \\ \textit{[`see\_onion`, `interact`, [\#any](*), `has\_onion`, [\#any](*), `see\_pot`, `interact`](1,1)}
    \item \textbf{Correspondance TP :} L’agent a rempli un pot avec un oignon
...
\begin{frame}{Le cadre MOISE+MARL}{Présentation de la méthode TEMM}
  ...
  \vspace{-0.5cm}
  {
    ...
    \textit{Ainsi:}
    \begin{itemize}
      \item \textit{Adéquation organisationnelle structurelle (SOF) = normalized deviation score} ($trajectory_{trans}$, $centroids_{trans}$)
      \item \textit{Adéquation organisationnelle fonctionnelle (FOF) = normalized deviation score} ($trajectory_{obs}$, $centroids_{obs}$)
  ...
  \begin{itemize}
    ...
    \item \textbf{Performance \& Stabilité} :
          \begin{itemize}
            ...
            \item Aucune violation de contrainte avec dureté de contrainte de 1
          \end{itemize}
