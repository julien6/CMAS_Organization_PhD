...
%% par les auteurs.
\documentclass[francais,ROIA,Unicode]{cedram}
...
\address{Thales Land and Air Systems, BU IAS, 35000, Rennes, France}
\email{louis-marie.traonouez@thalesgroup.com}
\author{\firstname{Paul} \lastname{Théron}} 
\address{AICA IWG, 06600, Antibes, France}
...
% Contribution
\noindent Nous présentons le cadre \textbf{MOISE+MARL}~\cite{soule2024moise_marl,soule2025jfsma} qui articule principalement deux axes de contribution :
%
\begin{itemize}
...
\begin{table*}[h!]
    ...
    \begin{tabular}{
        ...
        Berenji \& Vengerov (2000)~\cite{Kapetanakis2005} & $\times$                              & \checkmark                                   & $\times$                             & $\times$                          & $\sim$                                        \\
...
\begin{itemize}
    ...
    \item $RG \subseteq GR$~: l'ensemble des groupes racines, $GR = \langle \mathcal{R}, \mathcal{SG}, \mathcal{L}^{intra}, \mathcal{L}^{inter}, \allowbreak \mathcal{C}^{intra}, \mathcal{C}^{inter}, np, ng \rangle$, l'ensemble des groupes, où~:
          \begin{itemize}
              ...
              \item $\mathcal{L}^{inter} \subseteq \mathcal{L}$~: ensemble des liens inter-groupe. Par exemple, $link(\rho_{\text{coach}}, \allowbreak \rho_{\text{joueur}}, aut)$ signifie qu'un joueur jouant le rôle de gardien peut exercer une autorité sur un joueur indépendamment du groupe~;
...
%
TEMM repose sur l’apprentissage non supervisée pour généraliser ces spécifications à partir d'un nombre représentatif de trajectoires collectées comme illustrée en \autoref{fig:temm_overview}. En quantifiant l’écart entre les spécifications inférées et les comportements observés, TEMM propose une mesure quantitative de l'adéquation organisationnelle.
...
Pour chaque environnement, nous avons défini un ensemble de spécifications organisationnelles. Ces spécifications comprennent les rôles, les missions, ainsi que les permissions et obligations. Voici une description informelle de ces spécifications (disponible en totalité dans le code source~\hyperref[fn:github]{\footnotemark[1]}) :
%
...
%
Enfin, l’augmentation du nombre de contraintes organisationnelles accroît quasi linéairement la durée d’entraînement, d’après nos premiers résultats.
