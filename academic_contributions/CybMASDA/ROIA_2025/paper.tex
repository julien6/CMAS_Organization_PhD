...
\alttitle{An Organizationally-Oriented Approach to Enhancing Explainability and Control in Multi-Agent Reinforcement Learning}
...
%% R{\'e}sum{\'e} anglais
\begin{altabstract}
Trained agents can exhibit collective behaviors, from which implicit roles and goals can be extrapolated by analogy with a structured and functional organization. We propose MOISE+MARL, a framework that constrains agents to evolve according to such an organization to improve control and explainability in multi-agent reinforcement learning.
MOISE+MARL guides agents toward roles and missions by dynamically adjusting their actions and rewards. It includes post-training analysis to infer implicit organizational specifications. Experimented on various environments, it demonstrates alignment between agent behaviors, defined specifications, and inferred ones.
\end{altabstract}
