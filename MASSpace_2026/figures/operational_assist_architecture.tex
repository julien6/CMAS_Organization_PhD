\begin{figure}[h!]
  ...
  \caption{Operational-assist architecture based on an ORBITAL-oriented MOISE$^{+}$MARL. Human supervision sets priorities and safety posture; decentralized policies are constrained by organizational guides; ORBITAL-TEMM analyzes trajectories and returns audit-oriented feedback for supervisory updates and corrective override.}
  \Description{A three-layer control architecture. The top layer is human-in-the-loop supervision with mission priorities, guide weights, safety thresholds, and fallback settings. The middle layer is the decentralized ORBITAL-oriented MOISE+MARL decision layer with per-satellite policies constrained by organizational guides. The bottom layer is ORBITAL-TEMM trajectory analysis, which processes logs and returns organizational-fit diagnostics and feedback to operators.}
