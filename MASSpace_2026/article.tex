...
All experiments were conducted on an academic high-performance computing (HPC) cluster using heterogeneous GPU nodes. \autoref{tab:exp-hardware} summarizes the reference hardware profile.
...
The environment is implemented in Python and exposed through PettingZoo-compatible APIs. \autoref{tab:exp-software} summarizes the software stack used to ensure reproducibility.
...
To validate causal contributions of the proposed framework, we define the ablations in \autoref{tab:exp-ablations}.
...
We first compare the three organizational-control regimes introduced in \autoref{sec:method}: strong constraints, weak constraints, and medium constraints. \autoref{tab:results-control-regimes} summarizes aggregate results over matched evaluation budgets.

Three trends are directly visible in \autoref{tab:results-control-regimes}. Strong constraints achieve the best early acceleration, with early return AUC at $205.3 \pm 9.8$ and the fastest convergence at $118 \pm 14$ episodes, while maintaining low violation at $1.8\% \pm 0.9$. This acceleration comes with reduced robustness ($0.71 \pm 0.05$) and lower final return than the medium regime. Weak constraints show the opposite profile: slower learning (AUC $143.5 \pm 11.6$, convergence $213 \pm 22$), higher violation ($8.7\% \pm 1.8$), but stronger robustness ($0.87 \pm 0.04$). Medium constraints provide the best overall balance, with the highest final return ($379.8 \pm 16.1$), the best mission success rate ($0.91 \pm 0.03$), the highest organizational fit ($0.92 \pm 0.02$), and substantially improved convergence over weak control ($146 \pm 17$ vs. $213 \pm 22$). This quantitatively supports the expected effect of organizational guidance: constrain enough to compress trivial search dimensions, but not so much that robustness and emergent strategy diversity collapse.

\subsection{Baselines against handcrafted and learning conditions}
\autoref{tab:results-baselines} compares the proposed method against handcrafted and learning baselines. As expected, handcrafted policies do not rely on organizational constraints and therefore violation metrics are marked \texttt{n/a}.
...
\autoref{tab:results-explainability} reports trajectory-analysis indicators. The proposed method improves both structural and functional alignment, and produces clearer role/mission separation in trajectory space.
...
Trajectory inspection confirms the trend. Observer-like trajectories show persistent acquisition motifs followed by relay transitions at buffering thresholds. Relay-like trajectories exhibit draining patterns aligned with communication opportunities, while safety-guard trajectories increase \texttt{LowPower}/\texttt{CyberScan} usage during stress intervals. These motifs are less separable in unconstrained runs, consistent with the lower role-cluster separability score in \autoref{tab:results-explainability} ($0.41 \pm 0.06$ vs. $0.67 \pm 0.04$ for the proposed method).

\paragraph{Operational relevance}
From an operator viewpoint, TEMM outputs are valuable as they provide traceability of role-consistent behavior, diagnostics for repeated deontic constraint violations, and guidance on tuning role/mission guides. This is reflected in improved mission alignment and human-audit agreement ratings, which increase from \texttt{low} in unconstrained settings to \texttt{very high} in the proposed medium-constraint setting, indicating a more coherent and usable inferred organizational structure for post-hoc analysis.\\


\subsection{General gap-by-gap discussion}
The gap G1 highlights the lack of a benchmark integrating mission pressure, resource pressure, communication uncertainty, and cyber uncertainty. ORBITAL effectively discriminates policy quality across these axes, confirming its value as a testbed for fleet assistance. Gap G2 addresses the limited controllability in unconstrained MARL; the proposed method reduces violation rates and enhances convergence while improving mission success. Ablations show that these benefits are not solely due to the backbone. Gap G3 reveals the trade-off between convergence speed and robustness, with medium constraints identified as the optimal balance. Finally, Gap G4 emphasizes the need for operationally useful explainability, with TEMM indicators and trajectory motifs providing interpretable organizational evidence, supported by improved cluster separability and audit agreement.

Even in these experiments, validity risks persist as in full campaigns. External validity is limited by ORBITAL's abstraction; statistical power relies on seed count and variance control; and explainability hinges on trajectory feature design in TEMM. These limitations call for broader scenario sweeps and enhanced environment realism in future work.
