...
\acmConference[MASSpace'26]{Appears at the International Workshop on Autonomous Agents and Multi-Agent Systems for Space Applications (MASSpace-26). Held as part of the Workshops at the 25th International Conference on Autonomous Agents and Multiagent Systems.}{May 2026}{Paphos, Cyprus}{S.~Chien, G.~Picard, I.~Zilberstein (Chairs)}
\acmISBN{}
\copyrightyear{2026}
\acmYear{2026}
\acmDOI{}
...
\ccsdesc[500]{Computing methodologies~Machine learning~Learning paradigms~Reinforcement learning~Multi-agent reinforcement learning}
...
Earth-observation satellite fleets are increasingly difficult to operate with limited mission-control staff in a rapidly evolving space environment~\cite{schetter2003satellite_autonomy,rocha2025iaeossp,govoni2026multi_sat_task_allocation}. Operational teams must coordinate under uncertainty and time pressure while handling communication disruptions, cyber threats, resource limits, and collision risks. This creates a strong need for automated operational-assist systems that support decision-making without removing human oversight.

Throughout this paper, we use several key concepts. \emph{Operational-assist} refers to decision-support systems that augment human operators without removing their authority. \emph{Organizational constraints} are explicit roles and missions that guide agent behavior beyond reward optimization. \emph{Role specialization} captures differentiation of agent behaviors to improve coordination efficiency. \emph{Mission-phase discipline} describes structured progression between operational phases (acquire, deliver, stabilize) rather than greedy behavior.
...
From a multi-agent perspective, each satellite can be modeled as an autonomous agent interacting with teammates in a partially observable setting. This naturally motivates Dec-POMDP formulations and Multi-Agent Reinforcement Learning (MARL) methods~\cite{Oliehoek2016,lowe2017multi,rashid2018qmix,yu2021mappo}. However, raw MARL performance is insufficient for deployment, since operational practice also requires explicit controllability, traceable safety compliance for operator audit~\cite{garcia2015comprehensive,achiam2017cpo,alshiekh2018safe}.

To address this global challenge, we propose two complementary contributions. First, we introduce \textit{Orbital Resilient Benchmark for Interactive Task-aware Autonomous Learning} (ORBITAL), a tunable environment for controlled, reproducible fleet-level experimentation. Second, we propose a decision-making architecture combining decentralized autonomy, organizational constraints, and human oversight. The core control layer uses organizational reinforcement learning grounded in $\mathcal{M}OISE^+$ and MOISE+MARL~\cite{Hubner2002,Hubner2007,soule2024moise_marl} to improve coordination while preserving safety, controllability, and explainability through trajectory-level analysis.
...
\autoref{sec:related} reviews related work; \autoref{sec:background} presents technical background; \autoref{sec:method} introduces ORBITAL and the decision-making architecture; \autoref{sec:experiments} details the experimental protocol; \autoref{sec:results} reports results; and \autoref{sec:conclusion} concludes with a deployment path.
...
Organizational MAS modeling (roles, groups, missions, deontic constraints) is well established through Agent-Group-Role (AGR) and $\mathcal{M}OISE^+$~\cite{ferber2003,Hubner2002,Hubner2007}. Building on this foundation, MOISE+MARL integrates organizational constraints into MARL and uses trajectory-based post-analysis to assess organizational alignment~\cite{soule2024moise_marl}. This direction is promising for controllability and explainability, but it has not yet been fully studied in the context of satellite-fleet operational-assist settings with an explicit benchmarking perspective.
...
\begin{enumerate}
  ...
  \item \textbf{Deontic specifications}: permissions and obligations linking roles to missions under conditions.
        % \end{enumerate*}
\end{enumerate}
This layered representation is important because it separates \emph{what} should be done (functional), \emph{by whom} (structural), and \emph{under which normative constraints} (deontic), instead of collapsing everything into scalar reward coefficients.
...
\]
The result is a hybrid control paradigm in which learning remains data-driven while search is guided by explicit priors. This can reduce unsafe exploration regions and improve policy controllability, particularly in partially observable cooperative settings~\cite{soule2024moise_marl,Chai2025survey}.

\subsection{Trajectory-based analysis for explainability}
Even if policies are trained with organizational constraints, one still needs post-hoc evidence of the actual learned behavior. The \textbf{Trajectory-based Evaluation in MOISE+MARL (TEMM)} method~\cite{soule2024moise_marl} addresses this gap.
%
...
This process yields quantitative interpretable artifacts through:
...
\begin{figure*}[h!]
  ...
  \Description{A two-panel ORBITAL mission screenshot. The left panel shows Earth-centered orbital rings with eight satellites, observation tasks, debris-risk zones, ground stations, and communication links. The right panel shows mission indicators and the reward breakdown for the current step.}
...
\begin{figure}[h!]
  ...
  \Description{A 3D rendering of the ORBITAL environment showing Earth, several orbital shells, satellites, ground stations, debris regions, and communication links with the same operational semantics as the 2D view.}
...
In the ORBITAL setting, static rule-based scheduling systems remain easy to inspect and certify. However, they tend to be brittle when facing evolving observation demand, fluctuating inter-satellite connectivity, and stochastic cyber or operational disturbances, as commonly observed in large-scale Earth observation constellations~\cite{zilberstein2026largescalecontinualschedulingexecution}. Distributed Constraint Optimization Problem (DCOP) formulations provide an explicit framework for decentralized coordination and property-preserving optimization in such scenarios~\cite{Krigman2024}, enabling safety guarantees.

However, extending DCOP-based approaches to support long-horizon adaptation under partial observability and non-stationary uncertainties typically requires non-trivial model extensions and proactive mechanisms. Adaptation is therefore less direct than in learning-based control~\cite{hoang2022proactive}.
Vanilla MARL is highly adaptive to dynamic environments because it learns directly from interaction data.
However, it lacks explicit mechanisms to enforce structured behaviors such as:
(i) role specialization (e.g., some satellites focusing on observation while others relay data), and
(ii) mission-phase discipline (e.g., prioritizing delivery when buffers are saturated).
In such cases, reward shaping alone is often insufficient to ensure consistent and safe behavior~\cite{DevlinKudenko2016}.
...
Therefore, building on the MOISE+MARL~\cite{soule2024moise_marl} framework, we propose an ORBITAL-oriented organizational architecture~\hyperref[fn:github]{\footnotemark[1]} aligned with mission-control practice (\autoref{fig:operational-assist-architecture}).
...
\end{enumerate*}
We report organizational fit as $\mathrm{OF}=\frac{1}{2} \cdot (\mathrm{SF} + \mathrm{FF})$.
...
To validate causal contributions of the proposed framework, we define the ablations in \autoref{tab:exp-ablations}. We also stress-test each setting along controlled perturbation axes:
\begin{enumerate*}[label={\roman*) },itemjoin={; \quad}]
...
\autoref{tab:results-ablations} shows that each component contributes to at least one critical dimension. Removing RAG strongly reduces return and robustness, and increases violations. Removing GRG yields the largest mission-performance drop ($-10.4\%$ return). Removing RRG most strongly harms compliance, with the largest violation increase. Reducing TEMM features barely affects training performance but degrades interpretability quality, consistent with TEMM.
...
\autoref{tab:results-explainability} shows that the proposed method achieves higher role-cluster separability ($0.67 \pm 0.04$) compared to unconstrained learning ($0.41 \pm 0.06$), indicating that organizational constraints enhance behavioral specialization. Audit-agreement ratings improve from \texttt{low} to \texttt{very high}, reflecting that TEMM patterns align closely with intended specifications.
...
However, ORBITAL remains a simplified abstraction that omits high-fidelity orbital mechanics, full communication realism, and complete operational complexity. The organizational specifications are manually designed, which may introduce bias and limit scalability. TEMM analysis quality depends on trajectory features and clustering choices, requiring careful calibration. While this simplification improves experimental control and reproducibility, it may overlook second-order effects such as communication latency and orbital perturbations. Future work will integrate higher-fidelity simulators or digital twins to bridge this gap.

We identify three key directions for future work: enhancing environment realism to better simulate real-world conditions, integrating model-based reinforcement learning and multi-agent world models to improve long-horizon robustness, and reinforcing neuro-symbolic integration to ensure that learned behaviors align with safety constraints. Our aim is to pursue a staged deployment approach, starting with validation within the ORBITAL benchmark, progressing to higher-fidelity digital-twin integration, and ultimately developing offline advisory support systems.
