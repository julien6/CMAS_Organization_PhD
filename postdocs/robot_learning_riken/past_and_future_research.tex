...
This formal yet pragmatic perspective naturally bridges \textbf{simulation-based MARL} and \textbf{embodied robot learning}, aligning with the Robot Learning Team’s mission to combine reinforcement learning, imitation learning, and motion planning to enable robots to learn complex and adaptive behaviors in the physical world.

\vspace{0.6em}
\section*{1. Research in the Past}

During my doctoral studies at \textit{Université Grenoble Alpes} (UGA) and \textit{Thales~LAS} (France), I developed a \textbf{methodological framework for assisting the design and training of Multi-Agent Systems (MAS)}, implemented in the open-source \href{https://github.com/julien6/CybMASDE}{\textbf{CybMASDE platform}}.
This framework supports the entire lifecycle of MAS development---from \textit{formal specification} to \textit{learning}, \textit{analysis}, and \textit{deployment}---by combining \textit{Dec-POMDP}-based modeling, \textbf{multi-agent reinforcement learning (MARL)} using environments such as \textit{Gym} and \textit{PettingZoo}, and scalable training backends (\textit{MARLlib}, \textit{JAX}/\textit{JaxMARL}).
...
Agents learn under high-level constraints (roles, goals, safety), yielding interpretable, constraint-aware behaviors.
...
\begin{itemize}
  \item \textbf{MARL-assisted system design.}
        I proposed a methodological framework that formulates multi-agent system (MAS) design as a \textit{constrained policy-optimization problem}, integrating symbolic task structures with reinforcement learning dynamics.
  ...
  \item \textbf{World-model-based multi-agent learning.}
        Building on \textbf{World Models} \href{https://link.springer.com/chapter/10.1007/978-3-031-63223-5_24}{(Ha \textit{et al.}, 2018)}, I extended this idea to multi-agent and multi-robot settings by learning latent generative models that capture both environmental and inter-agent dynamics.
  ...
  \item \textbf{Behavior analysis and explainability.}
        I developed trajectory-analysis methods that extract implicit roles, subgoals, and coordination patterns from agent behaviors.
...
I will develop compact generative and contrastive models that fuse visual, proprioceptive, and task signals, combined with relational graph encoders and transformer dynamics to capture inter-agent structure.
Trained on cooperative tasks (dual-arm manipulation, collaborative transport, handover), these models will enable sample-efficient model-based RL and safe sim-to-real transfer.

\textbf{Objective 2 – Integrating Imitation and Reinforcement Learning.}
To further accelerate learning and improve safety, I will integrate \textbf{imitation learning (IL)} and \textbf{reinforcement learning (RL)} within the world-model framework.
...
\begin{itemize}
  \item A reproducible \textbf{world-model-based framework} for multi-robot cooperation, integrating predictive modeling, imitation, and reinforcement learning.
  ...
  \item At least \textbf{one international conference publication} (e.g., CoRL, ICRA, RSS) and one \textbf{journal article} (IEEE~T-RO or RAL) demonstrating the benefits of predictive and imitation-guided multi-robot learning.
  \item A \textbf{robotic demonstration at RIKEN} showcasing cooperative manipulation, adaptive coordination, and safety-aware planning.
\end{itemize}
...
Following this postdoctoral project, my long-term objective is to pursue an \textbf{academic career} focusing on the integration of \textbf{multi-agent learning}, \textbf{robotics}, and \textbf{representation learning}.
I aim to contribute to the theoretical and methodological foundations that make robotic systems both \textbf{autonomous} and \textbf{interpretable}.
This research agenda seeks to deepen the connection between symbolic reasoning, world-model-based prediction, and embodied learning—laying the groundwork for robots capable of safe, explainable, and adaptive behavior in complex real-world environments.
