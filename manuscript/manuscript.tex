...
\begin{figure}[h]
    ...
    \fbox{
        \begin{minipage}[c][5cm][c]{0.8\textwidth}
            \centering
            \textit{Illustration de la distinction entre cybersécurité (prévention, protection passive) et cyberdéfense (réaction, adaptation tactique et opérationnelle).}
        \end{minipage}
    }
    \caption{Différences conceptuelles entre cybersécurité et cyberdéfense}
...
\begin{figure}[h]
    ...
    \fbox{
        \begin{minipage}[c][6cm][c]{0.8\textwidth}
            \centering
            \textit{Carte conceptuelle des axes de recherche en cyberdéfense : détection, résilience, automatisation et intégration de l'IA.}
        \end{minipage}
    }
...
\begin{figure}[h]
    \centering
    \fbox{
        \begin{minipage}[c][5.5cm][c]{0.8\textwidth}
            \centering
            \textit{Schéma comparatif entre architectures de cyberdéfense centralisées et décentralisées dans des contextes dynamiques et hostiles.}
        \end{minipage}
    }
...
\subsection*{Positionnement de la thèse}

Cette thèse propose d’explorer une approche distribuée et auto-organisée de la cyberdéfense, en s’appuyant sur des \emph{systèmes multi-agents (SMA)} intégrant des techniques d’apprentissage (RL/MARL), des mécanismes organisationnels (e.g., MOISE+) et une capacité d’adaptation en ligne. L’objectif est de combler un vide critique dans la littérature : concevoir un SMA de cyberdéfense explicable, performant et générique, capable de s’auto-organiser selon le contexte et les contraintes opérationnelles.

Cette approche sera développée de manière progressive dans les sections suivantes, à travers un cadre de conception structuré appelé \textbf{MAMAD} (MOISE+MARL-based Automated Design).
...
\begin{figure}[h]
    ...
    \fbox{\begin{minipage}[c][5.5cm][c]{0.8\textwidth}
        \centering
        \textit{Schéma illustratif de l’évolution des menaces : des attaques scriptées et localisées vers des campagnes distribuées et assistées par IA}
    \end{minipage}}
...
Dans ce contexte, une attention croissante a été portée aux \textbf{autonomous cyber defense agents}, capables d'opérer sans intervention humaine directe, tout en respectant des objectifs globaux. Ce paradigme a notamment été formalisé dans le concept d’\textbf{AICA (Autonomous Intelligent Cyber Agents)} proposé par le groupe de travail IST-168 de l’OTAN~\cite{AICAGuide2022}.
...
\begin{figure}[h]
    \centering
    \fbox{\begin{minipage}[c][5.8cm][c]{0.8\textwidth}
        \centering
        \textit{Diagramme fonctionnel d’un agent AICA typique, illustrant le cycle perception--décision--action autonome}
    \end{minipage}}
...
\begin{figure}[h]
    ...
    \fbox{\begin{minipage}[c][5.5cm][c]{0.8\textwidth}
        \centering
        \textit{Illustration d’une architecture de défense distribuée composée d’agents autonomes communicants}
    \end{minipage}}
...
\begin{figure}[h]
    ...
    \fbox{\begin{minipage}[c][6cm][c]{0.8\textwidth}
        \centering
        \textit{Illustration d’un système multi-agent typique : plusieurs agents spécialisés (surveillance, analyse, réaction) interagissant dans un environnement dynamique}
    \end{minipage}}
...
\begin{figure}[h]
    ...
    \fbox{\begin{minipage}[c][5.5cm][c]{0.8\textwidth}
        \centering
        \textit{Comparaison entre auto-organisation (bottom-up) et organisation imposée (top-down)}
    \end{minipage}}
...
\begin{figure}[h]
    ...
    \fbox{\begin{minipage}[c][6cm][c]{0.8\textwidth}
        \centering
        \textit{Carte conceptuelle des axes de recherche dans le domaine des SMA et leurs relations}
    \end{minipage}}
...
Plus récemment, la DARPA a impulsé le programme \emph{GARD} (Guaranteed AI Robustness against Deception) et promu l’idée d’agents de cyberdéfense proactifs dans le cadre du programme \emph{AI Next Campaign}. Dans un contexte OTAN, l’initiative AICA (Autonomous Intelligent Cyber Agent)~\cite{AICAReport2021, AICAGuide2022} propose une vision modulaire et cognitive de l’agent, intégrant des capacités de perception, de planification, d’apprentissage et d’action distribuée.
...
\label{sec:revue-conception-aica}
