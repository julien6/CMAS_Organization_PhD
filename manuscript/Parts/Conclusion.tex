...
Contrairement aux approches de type \textit{Explainable AI} (\acn{XAI}) centrées sur l'interprétation locale des décisions de modèles neuronaux (e.g., importance des features, attribution de gradients), \acn{TEMM} et \acn{Auto-TEMM} proposent une \textbf{explicabilité organisationnelle}.
...
\begin{itemize}
  ...
  \item \textbf{Coût computationnel} : certaines phases (apprentissage \acn{MARL}, analyse organisationnelle \acn{Auto-TEMM}) demeurent exigeantes en ressources. Ce point ouvre des perspectives en optimisation parallèle et en passage à l'échelle (\acn{GPU} multi-nœuds, exécution distribuée).
