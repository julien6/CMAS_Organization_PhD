...
% Graphes d’attaque, Arbres AD, Réseaux de Petri, Modèles de jeux
...
% Rappel bref des POMDP, POSG et autres modèles Markoviens
% Insister sur le formalisme du Dec-POMDP, pas la peine de détailler les autres formalismes 
...
% World Models (VAE + RNN + Policy Network)
...
% Rappeler les bases du MARL et expliciter en utilisant le formalisme des Dec-POMDP précédement défini
...
% Le but de cette sous-section est de commencer à introduire les concepts liés à l'apprentissage par renforcement sous contraintes que l'on poursuivra plus tard en liant le modèle organisationel avec le processus d'apprentissage.
% Présenter brièvement les :
%  - Constrained Policy Optimization (CPO), Deep Constrained Q-Learning (DCQL)
%  - Garanties partielles de sûreté
%  - Reward shaping, Shielding, Feedback humain
...
% Distinction et exemples, explicabilité locale vs globale
% Importance en MARL

\subsection{Méthodes post-hoc}
% SHAP, LRP, Patching, Attribution de concepts

\subsection{Inférence organisationnelle}
% Clustering de trajectoires, ROMA, Approches bayésiennes
% Extraction de rôles et missions émergents
% Idée de relier extraction organisationnelle et modèles symboliques

\section{Transfert simulation $\rightarrow$ réel et cohérence (TRF)}

\subsection{Domain adaptation et Sim2Real}
% Domain randomization, Domain invariance
% Définitions mathématiques (minimisation divergence)

\subsection{Robust Reinforcement Learning}
% Formulation robuste (incertitude sur la dynamique)
% Garanties de robustesse

\subsection{Adaptation en ligne}
% Online system identification, PILCO, feedback loops
% Approches incrémentales

\subsection{Synchronisation manuelle}
% Recalibrage ponctuel (ex. CybORG, CyberBattleSim)
% Limites pratiques
