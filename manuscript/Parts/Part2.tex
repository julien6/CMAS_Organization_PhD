...
\begin{figure}[h!]
  ...
  \resizebox{0.8\textwidth}{!}{%
    \input{figures/organisation_manuscrit_partie_2}
  }
...
\noindent
\textbf{Repères de lecture.}
Afin de rendre la lecture plus fluide malgré la densité des acronymes et tableaux croisés, chaque section suit la même structure~: (i) recontextualisation du sous-problème, (ii) hypothèse de restriction, (iii) couverture des critères, puis (iv) analyse des verrous. Les sous-problèmes \textbf{MOD}, \textbf{TRN}, \textbf{ANL} et \textbf{TRF} sont systématiquement reliés aux critères \textbf{C1} à \textbf{C5} pour garder un fil directeur unique.
...
\begin{table}[h!]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    \hline
    Modèles formels génériques (\acparen{MDP}, \acparen{POMDP}, \acparen{Dec-POMDP}, \acparen{POSG}, graphes d'attaque, arbres \acparen{AD}, réseaux de Petri, modèles de jeu) & \cmark{}          & \cmark{}              & \xmark{}                & \cmark{}                  & \xmark{}                        & \cmark{}             \\
    Frameworks génériques et configurables (\acparen{CyberBattleSim}, \acparen{NASim}, \acparen{NASimEmu}, \acparen{DETERLab}, \acparen{CyberVAN}, \acparen{CYST})             & \cmark{}          & \cmark{}              & \xmark{}                & \cmark{}                  & \cmark{}                        & \cmark{}             \\
    Simulateurs spécialisés de Cyberdéfense (\acparen{CybORG}, \acparen{CybORG}++, CyberWheel, \acparen{SCYTHE}, \acparen{CTF})                                                & \cmark{}          & \xmark{}              & \xmark{}                & \cmark{}                  & \cmark{}                        & \cmark{}             \\
...
Au niveau le plus abstrait, la modélisation des environnements de Cyberdéfense repose sur des formalismes mathématiques issus de la théorie de la décision séquentielle. Le cadre de base est le \acn{MDP}~\cite{puterman1994mdp}, qui décrit un environnement comme un ensemble d'états, d'actions et de transitions probabilistes. Lorsque l'information disponible est partielle, les \acn{POMDP}~\cite{kaelbling1998pomdp} offrent un cadre adapté. Pour les \acplu{SMA} coopératifs, les variantes décentralisées (\acparen{Dec-POMDP})~\cite{Oliehoek2016} sont utilisées, tandis que les environnements compétitifs (attaquant/défenseur) sont modélisés par les \acn{POSG}~\cite{hansen2004posg}. D'autres extensions, comme les \acn{MDP} factorisés~\cite{guestrin2003factored}, facilitent la modélisation de systèmes complexes, et la théorie des jeux de sécurité~\cite{manshaei2013game} permet de formaliser explicitement les stratégies adversariales.
...
\begin{itemize}
  \item \textbf{L'intégration de contraintes explicites}~: travaux où des contraintes de sûreté, de structure ou de mission sont formellement ajoutées au processus d'apprentissage (par exemple via des Constrained \acparen{MDP}, \acparen{CPO}, Deep Constrained Q-Learning, etc.) ;
  ...
  \item \textbf{L'incorporation de modèles organisationnels symboliques}~: tentatives d'intégrer des spécifications organisationnelles issues de l'ingénierie des \acn{SMA} (principalement \acparen{AGR}~\cite{Ferber2004} et $\mathcal{M}OISE^+$~\cite{hubner2002moise}) dans le processus d'apprentissage, afin de garantir le respect de structures, de rôles ou de missions collectives.
\end{itemize}
...
\begin{table}[h!]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    Intégration de modèles organisationnels ($\mathcal{M}OISE^+$~\cite{hubner2007using}, \acparen{AGR}~\cite{Ferber2004}), hybridation apprentissage/organisation~\cite{bordini2006jade,chernova2014robot} & Forte (potentielle)                     & Organisation                  & Non (verrou)                 & Non (verrou)                         & Forte                  \\
    \hline
  \end{tabularx}
\end{table}
...
L'extraction des spécifications organisationnelles émergentes constitue le troisième sous-problème central de la démarche de la thèse. Alors que les modèles organisationnels symboliques (i.e. $\mathcal{M}OISE^+$) permettent de prescrire explicitement des structures, rôles et missions lors de la conception d'un \acn{SMA}, ces spécifications sont généralement absentes ou perdues dans les approches fondées sur le \acn{MARL}. Les politiques apprises sont souvent représentées sous forme de réseaux de neurones opaques, rendant difficile l'analyse, la vérification ou l'explicabilité des comportements collectifs obtenus.
...
\begin{itemize}
  ...
  \item assurer le transfert robuste des politiques apprises en simulation vers le réel (policy transfer, transfer learning, robust \acparen{RL}) ;
...
\begin{itemize}
  ...
  \item \textit{Manual resynchronization} dans les simulateurs industriels ou cyber (ex.~: \acparen{CybORG}~\cite{Standen2021}, \acparen{CyberBattleSim}~\cite{cyberbattlesim}).
\end{itemize}
...
Ce chapitre peut être lu en suivant la chaîne \textbf{MOD $\rightarrow$ TRN $\rightarrow$ ANL $\rightarrow$ TRF}.
Pour chaque bloc, nous explicitons d'abord les concepts mobilisés, puis leur rôle dans la méthode \acn{MAMAD}.
Ce choix vise à limiter la surcharge cognitive liée aux acronymes, tout en conservant la précision de la revue.
...
\noindent
Certaines approches fondées sur la théorie des jeux s'inscrivent dans le cadre des \textquote{jeux stochastiques partiellement observables} (\acparen{POSG}) ou, plus précisément, dans celui des \textquote{processus de décision markoviens décentralisés partiellement observables} (\acparen{Dec-POMDP}). Les \acn{POSG} et les \acn{Dec-POMDP} sont tous deux des cadres de modélisation mathématique des problèmes de prise de décision dans lesquels des agents interagissent entre eux et dans un environnement stochastique~\cite{beynier2010}. Dans un \acn{POSG}, un groupe d'agents interagit avec un environnement stochastique et partiellement observable. Chaque agent agit en fonction de ses propres observations et d'une politique locale. Les agents peuvent avoir des objectifs différents, car chaque agent a sa propre fonction de récompense et le jeu est généralement supposé être non coopératif~\cite{terry2020pettingzoo}. Dans un \acn{Dec-POMDP}, plusieurs agents peuvent avoir une fonction de récompense commune et peuvent coordonner leurs actions pour atteindre un objectif commun, notamment en étant capables de communiquer~\cite{bernstein2013}.

\subsection{Positionnement vis-à-vis des Stackelberg Security Games}

Les \textit{Stackelberg Security Games} (SSG) constituent un cadre majeur de la cybersécurité décisionnelle~\cite{manshaei2013game}. Ils formalisent une interaction leader/suiveur~: le défenseur choisit une stratégie de protection, puis l'attaquant réagit.

Ce cadre partage un objectif commun avec notre formulation~: optimiser une politique de défense sous contraintes de ressources et face à un adversaire. Néanmoins, il existe trois différences structurantes avec notre périmètre :
\begin{itemize}
  \item les SSG reposent souvent sur une vision stratégique statique ou quasi-statique, alors que nous traitons un processus séquentiel et partiellement observable ;
  \item notre cadre vise des \acplu{SMA} avec coordination explicite entre plusieurs défenseurs (et éventuellement plusieurs attaquants), plutôt qu'un couple leader/suiveur agrégé ;
  \item les contraintes organisationnelles (rôles, missions, obligations/permissions) sont représentées explicitement via $\mathcal{M}OISE^+$ puis injectées dans l'apprentissage, ce qui dépasse la seule allocation stratégique.
\end{itemize}

Notre choix de formulation par \acn{Dec-POMDP}/\acn{ODec-POMDP} et \acn{MARL} contraint permet ainsi d'articuler, dans une même boucle, les quatre activités \textbf{MOD}--\textbf{TRN}--\textbf{ANL}--\textbf{TRF}. Les SSG restent néanmoins complémentaires~: ils peuvent servir de cadre de positionnement théorique et de point d'entrée pour des allocations défensives tactiques en amont de l'apprentissage.



\subsection{Le modèle Dec-POMDP}\label{sec:dec-podmp}

Pour appliquer des techniques \acn{MARL}, il est nécessaire de s'appuyer sur un cadre Markovien pour formaliser les observations, actions, récompense, etc. Nous nous basons sur le cadre du \acn{Dec-POMDP}~\cite{Oliehoek2016}. Les \acn{Dec-POMDP} permettent de modéliser la coordination décentralisée entre agents dans des contextes à observabilité partielle, ce qui les rend particulièrement adaptés à l'intégration de contraintes organisationnelles. Contrairement aux \acn{POSG}, le \acn{Dec-POMDP} utilise une fonction de récompense commune, favorisant ainsi la collaboration~\cite{Beynier2013}.
...
\begin{itemize}
  \item \textbf{\acn{CPO}}~\cite{achiam2017constrained},
  ...
  \item \textbf{\acn{DCQL}}~\cite{kalweit2020deep},
...
\subsection{Pourquoi le choix de $\mathcal{M}OISE^+$ ?}

Le choix de $\mathcal{M}OISE^+$ repose sur un compromis entre expressivité, opérationalisation et traçabilité. D'autres cadres existent dans la littérature (\acn{AGR}, méthodologies orientées conception, cadres fortement normatifs), mais ils couvrent moins bien simultanément les besoins de la thèse.

\begin{itemize}
  \item \textbf{Expressivité organisationnelle}~: $\mathcal{M}OISE^+$ combine structure (rôles/groupes), fonction (missions/plans) et déontique (permissions/obligations), ce qui permet de relier directement les contraintes de conception aux critères C4 (contrôle) et C5 (explicabilité).
  \item \textbf{Compatibilité avec l'optimisation sous contraintes}~: les éléments organisationnels peuvent être traduits en guides de contraintes exploitables par le \acn{MARL} (masquage d'actions, pondération de récompense, objectifs), ce qui soutient C2 (performance) et C3 (adaptation).
  \item \textbf{Traçabilité pour le cycle MAMAD}~: la structure explicite facilite l'audit des décisions et le raffinement itératif dans la boucle \textbf{TRN}--\textbf{ANL}--\textbf{TRF}, au lieu d'un simple guidage implicite difficile à interpréter.
\end{itemize}

En contexte Cyberdéfense, ce choix est particulièrement pertinent car il permet de formaliser des responsabilités différenciées (détection, mitigation, supervision), d'imposer des contraintes critiques, et de conserver une base explicative exploitable lors du transfert vers des environnements opérationnels.
...
d'explicabilité en apprentissage automatique (\acparen{SHAP}, \acparen{LIME}, attribution de gradient),
...
par exemple via des réseaux neuronaux à base de concepts (\acparen{CAV})
...
\begin{itemize}
  ...
  \item \textbf{Domain invariance}~\cite{ganin2016domain}~: ici, on apprend
        ...
        \]
        où $D$ est une divergence statistique (\acparen{KL}, \acparen{MMD}, adversariale).
\end{itemize}
