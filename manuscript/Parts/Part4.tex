...
\textbf{2. Modéliser environnement}, qui dépend de l'étape de configuration et se décline en deux variantes : \textit{Modélisation automatique} (par World Model comme montré en \autoref{fig:cybmasde_screenshot_configuration}) et \textit{Modélisation manuelle} (via le \acn{Dec-POMDP} pré-spécialisé donnant une instance \acn{MCAS})~;
...
\textbf{3. Entraîner politiques}, qui repose sur l'existence d'un environnement simulé et permet d'apprendre des politiques multi-agents compatibles avec les contraintes organisationnelles. Dans cette activité, il est possible d'ajouter des spécifications organisationnelles avant de lancer le processus d'entraînement comme montré en \autoref{fig:cybmasde_screenshot_training}~;
