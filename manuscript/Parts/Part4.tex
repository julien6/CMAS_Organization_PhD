...
\begin{figure}[h!]
  ...
  \caption{Structure de la Partie \acn{IV}~: Cadre expérimental et analyse des résultats}
...
\begin{description}
  \item[Utilisateur :] Renseigne au moins une des options :
    \begin{enumerate}
      \item \textbf{\acn{API} réelle} (\texttt{transferring/configuration.json}) si elle existe déjà.
...
\paragraph{4. Modelling (\acn{MOD})}
\begin{description}
  \item[Utilisateur :] Lance la génération d’un environnement simulé.
  \item[CybMASDE :]
    \begin{itemize}
      \item Si des traces sont présentes : entraîne un \textbf{Joint-Observation Prediction Model (\acn{JOPM})} basé sur PyTorch (\acn{VAE}, \acn{MLP}, \acn{LSTM} optimisé par Adam).
...
\paragraph{5. Training (\acn{TRN})}
\begin{description}
  \item[Utilisateur :] Démarre l’entraînement des politiques multi-agents.
  \item[CybMASDE :] Utilise MARLlib (\acn{MAPPO}, \acn{MADDPG}, QMix, etc.) et Ray RLlib. Applique les contraintes \acn{MOISE+MARL} via masquage d’actions par rôle et shaping de récompenses. Les hyperparamètres (\acn{LR}, $\gamma$, taille de batch, etc.) sont optimisés via Optuna. Sauvegarde checkpoints et statistiques dans \texttt{training/}.
\end{description}
...
\paragraph{9. Transferring (\acn{TRF})}
\begin{description}
...
\begin{itemize}
  \item \texttt{PyTorch} — entraînement des modèles \acn{VAE}, \acn{LSTM}.
  \item \texttt{MARLlib} + \texttt{Ray RLlib} — apprentissage multi-agents avec une grande variété d'algorithmes comme \acn{MAPPO}, QMix, \acn{MADDPG}, etc.
  \item \texttt{Optuna} — optimisation des hyperparamètres (learning rate, discount factor, \acn{PPO} clip value, etc.).
...
\begin{figure}
  ...
  \caption{Diagramme de composants \acn{UML} illustrant l'architecture logicielle de CybMASDE}
  \label{fig:cybmasde_uml}
\end{figure}

\section{Intégration des différentes contributions}

\subsection{Implémentation du modèle Dec-\acn{POMDP} pré-spécialisé pour la Cyberdéfense}
...
\subsection{Implémentation du framework \acn{MOISE}+MARL}
...
\footnotetext[2]{ \label{fn:github} L'implémentation \textquote{MOISE+MARL API} (\acn{MMA}), les hyperparamètres et spécifications utillisés sont disponibles à \url{https://github.com/julien6/\acn{MOISE}-MARL}.}

Pour prendre en charge les environnements Dec-\acn{POMDP}, nous avons utilisé la bibliothèque \textit{PettingZoo} \cite{terry2020pettingzoo}, qui fournit une \acn{API} standard pour les systèmes multi-agents et garantit l'interopérabilité entre différents environnements, à l'instar du framework Gymnasium \cite{kwiatkowski2024}. \acn{MMA} intègre un dictionnaire pour le mappage des étiquettes d'observation/action ($l$), que les utilisateurs peuvent personnaliser, et prend également en charge les modèles de trajectoire \acn{TP} pour faciliter la définition et la correspondance des modèles.
...
\begin{itemize}
  ...
  \item \textbf{Company infrastructure}~\cite{cyberbattlesim}: une simulation d’attaques et de défenses sur un réseau, inspirée de \acn{MITRE} \acn{ATT}\&\acn{CK}.
...
\begin{itemize}
  ...
  \item \textbf{Frameworks DL}~: PyTorch~\cite{Paszke2019} et TensorFlow~\cite{Abadi2016} (implémentations MARLlib/\acn{MAPPO}, etc.).
...
\begin{itemize}
  ...
  \item \textbf{Environnement} : Un environnement avec variantes des scénarios (ex. Overcooked-\acn{AI}~\cite{overcookedai}, Proie-Prédateur~\cite{lowe2017multi}, Drone Swarm~\cite{cage_challenge_3_announcement}).
...
Elle est calculée comme une moyenne pondérée des scores structurels (\acn{SOF}) et fonctionnels (\acn{FOF}) :
\[
...
Réalisation d’au moins une \textbf{étude d’ablation} : chaque ablation consiste prendre la \textbf{baseline avancée} et à changer un ou plusieurs paramètres notamment à remplacer une ou plusieurs activités \acn{MAMAD} par des variantes moins avancées (par exemple, \texttt{\acn{ANL}-MAN} en utilisant \acn{TEMM} plutôt que Auto-\acn{TEMM}, ou \texttt{\acn{TRN}-UNC} sans contraintes organisationnelles) mais aussi à supprimer les contraintes organisationnelles, à changer d’algorithme, etc. Chaque ablation doit être justifiée en fonction des questions spécifiques de chaque scénario.
...
\begin{figure}[h!]
  ...
  \caption{Capture d'écran de l'environnement Overcooked-\acn{AI} : deux agents (chefs cuisiniers) doivent collaborer pour préparer et servir efficacement des soupes à l'oignon. Le processus consiste à prélever trois oignons (un à la fois) dans le distributeur, à les placer dans une marmite, à attendre que la soupe cuise, à récupérer un plat propre, à dresser la soupe et à la livrer au comptoir de service. La disposition de la cuisine comprend des obstacles et des passages étroits, ce qui oblige les agents à coordonner leurs mouvements pour éviter les collisions et optimiser l'accomplissement des tâches.}
  \label{fig:overcooked}
\end{figure}

\subsubsection*{Proie-Prédateur}

L'environnement \textbf{Proie-Prédateur} est un benchmark \acn{MARL} bien connu~\cite{lowe2017multi}, conçu pour évaluer la coordination entre des poursuivants coopératifs (prédateurs) qui tentent de capturer un agent insaisissable (proie). Cet environnement est illustré dans \autoref{fig:predator_prey}.
%
\begin{enumerate*}[label={\roman*)}, itemjoin={; \quad}]
...
\begin{table*}[h!]
  ...
  \begin{tabularx}{\textwidth}{p{3.8cm}p{2.5cm}p{2.8cm}p{4.5cm}}
    ...
    \midrule
    % --- Profil A (Défaut) ---
    \multirow{3}{*}{\parbox{3.8cm}{\textbf{Profil A — Défaut}                                                                                                                                                               \\\acn{MOD}-\acn{AUT};\;\acn{TRN}-\acn{CON};\;\acn{ANL}-\acn{AUT};\;\acn{TRF}-AUT}}
    ...
    \multirow{3}{*}{\parbox{3.8cm}{\textbf{Profil B — Analyse manuelle}                                                                                                                                                     \\\acn{MOD}-\acn{AUT};\;\acn{TRN}-\acn{CON};\;\acn{ANL}-\acn{MAN};\;\acn{TRF}-AUT}}
    ...
    \multirow{3}{*}{\parbox{3.8cm}{\textbf{Profil C — Cycle principalement manuel}                                                                                                                                          \\\acn{MOD}-\acn{MAN} ; \acn{TRN}-\acn{CON} ; \acn{ANL}-\acn{MAN} ; \acn{TRF}-MAN}}
...
L'environnement \textbf{Company infrastructure} est une simulation inspirée du simulateur CyberbatlleSim~\cite{cyberbattlesim} et de \acn{MITRE} \acn{ATT}\&\acn{CK}~\cite{MITREATTACKWebiste}. Cet environnement a été developpé avec le Dec-\acn{POMDP} pré-spécialisé pour la Cyberdéfense et avec \acn{MCAS} intégré à CybMASDE. Il simule un réseau d’entreprise découpé en sous-réseaux (\acn{EXT}, \acn{DMZ}, \acn{ACC}, \acn{MAR}, \acn{SRV}) où des \emph{agents cyber-attaquants} et \emph{agents cyber-défenseurs} interagissent via des actions à pré/post-conditions, selon un formalisme \emph{Dec-POMDP}. La topologie synthétique est illustrée en \autoref{fig:scenario_network_topology}, et les chemins d’attaque/défense sont structurés via un arbre Attaque–Défense (\autoref{fig:ADTree}).
...
\begin{enumerate*}[label={\roman*)}, itemjoin={; \quad}]
  ...
  \begin{enumerate*}[label={\alph*)}, itemjoin={; \ }]
    ...
    \item \textbf{DMZ} : \texttt{\acn{WS}, \acn{ES}, \acn{VPN}, FTP}
    \item \textbf{ACC} : \texttt{E1, E2, CTO}
    \item \textbf{MAR} : \texttt{\acn{PS}, E3, \acn{TAB}}
    \item \textbf{SRV} : \texttt{\acn{API}, \acn{DB}, DC}.
  \end{enumerate*}
  ...
  \begin{enumerate*}[label={\alph*)}, itemjoin={; \ }]
    \item \textbf{Attaquants} : \texttt{Reconnaissance réseau/compte}, \texttt{Exploiter vulnérabilité service}, \texttt{Élévation de privilèges}, \texttt{Mouvement latéral}, \texttt{Persistance (ex.~backdoor)}, \texttt{Exfiltration données (\acn{DB})}, \texttt{Installation spyware (\acn{PS})}
    \item \textbf{Défenseurs} : \texttt{Détection journaux (\acn{WS})}, \texttt{Gestion comptes privilégiés (\acn{PAM})}, \texttt{Surveillance commandes/arguments (\acn{DB})}, \texttt{Blocage trafic/règles FW}, \texttt{Suppression sessions malveillantes}, \texttt{Restauration services}.
  \end{enumerate*}
  ...
  \begin{enumerate*}[label={\alph*)}, itemjoin={; \ }]
    \item \textbf{Attaquants} : bonus de progression le long de l’\acn{AD}-tree ($+r_{\small step}$), objectifs ultimes : \texttt{Exfiltration DB} et \texttt{Spyware PS} ($+R_{\small goal}$), pénalités si détectés/neutralisés ($-\lambda_{\small det}$)
...
\medskip
\textbf{Spécifications organisationnelles :} \emph{(Baseline)} aucune contrainte organisationnelle n’est imposée (\texttt{\acn{TRN}-UNC}), afin de fournir une référence \emph{brute} pour les scénarios orientés cyberdéfense guidés. \emph{(Variante optionnelle, pour analyses ablatives)} :
\begin{enumerate*}[label={\roman*)}, itemjoin={; \quad}]
  ...
  \item \textbf{Missions} : chaîner les techniques \acn{MITRE} par sous-objectif (recon \textrightarrow{} exploitation \textrightarrow{} élévation \textrightarrow{} mouvement latéral \textrightarrow{} action sur l’objectif) côté rouge ; détection \textrightarrow{} confinement \textrightarrow{} éradication \textrightarrow{} reprise côté bleu
...
\begin{figure}[h!]
  ...
  \caption{Topologie réseau synthétique : \acn{EXT}, \acn{DMZ}, \acn{ACC}, \acn{MAR}, \acn{SRV}. Les sous-réseaux sont interconnectés via routeurs/pare-feu implicites.}
...
\begin{figure}[h!]
  ...
  \caption{Aperçu de l’arbre Attaque–Défense (\acn{AD}) structurant les chemins d’attaque (tactiques/techniques \acn{MITRE}) et les contre-mesures associées.}
  \label{fig:ADTree}
\end{figure}
...
Nous utilisons le modèle \emph{Dec-POMDP} pré-spécialisé \textbf{Cyberdéfense} et le simulateur \textbf{MCAS} intégrés à \texttt{CybMASDE}, exposés via un adaptateur \acn{REST} conforme à l’\acn{API} d’I/O \texttt{CybMASDE} (observations jointes, masques d’actions, \emph{step}/\emph{reset}). L’environnement \emph{Company infrastructure} est donc traité, côté pipeline, exactement comme les environnements jouets non-orientés cyberdéfense : (i) collecte de traces pour la modélisation automatique (\texttt{\acn{MOD}-AUT}) ; (ii) entraînement \acn{MARL} avec ou sans contraintes organisationnelles ; (iii) analyse organisationnelle (\texttt{ANL}) ; (iv) transfert automatique (\texttt{\acn{TRF}-AUT}) vers l’exécuteur simulé.
\textbf{Justification :} aucun \texttt{\acn{TRF}-MAN} n’est requis ici, l’environnement n’étant pas une instrumentation d’un système réel mais un simulateur nativement compatible (pas de « passerelle terrain » à maintenir).
...
\begin{table*}[h!]
  ...
  \begin{tabularx}{\textwidth}{p{3.8cm}p{2.6cm}p{2.8cm}p{4.6cm}}
    ...
    \multirow{3}{*}{\parbox{3.8cm}{\textbf{Profil A — Défaut}                                                                                                                                                                                                      \\
        \texttt{\acn{MOD}-AUT};\;\texttt{\acn{TRN}-CON};\;\texttt{\acn{ANL}-AUT};\;\texttt{\acn{TRF}-AUT}}}
    ...
    \multirow{3}{*}{\parbox{3.8cm}{\textbf{Profil C — Cycle semi-manuel}                                                                                                                                                                                           \\
        \texttt{\acn{MOD}-MAN} (actions/props étendues);\;\texttt{\acn{TRN}-CON} (hp manuels);\;\texttt{\acn{ANL}-MAN};\;\texttt{\acn{TRF}-AUT}}}
...
\begin{enumerate*}[label={\roman*)}, itemjoin={;\quad}]
  ...
  \item \textbf{Espace d'observation :} observation \emph{réelle} partielle (Dec-\acn{POMDP}) obtenue via l’\acn{API}~Kubernetes/collecte métriques. Elle est \emph{spécifique au rôle} :
...
\begin{enumerate*}[label={--}, itemjoin={\quad}]
  \item \textsc{\acn{MOD}/TRN} sur jumeau numérique dérivé de traces du cluster réel;
...
\begin{figure}[h!]
  ...
  \caption{Cluster \emph{réel} «~Services en chaîne~» (4 services) et leviers d’action exposés à CybMASDE/\acn{MAMAD} via l’\acn{API} Kubernetes.}
...
Nous opérons \emph{sur le cluster réel} (pas de simulation en ligne de commande jouant le rôle d’« environnement réel »). Le cluster (\textbf{\acn{VM} 8~vCPU, 32~Go \acn{RAM}, 1~Gbps}) héberge l’application e-commerce (4 microservices en chaîne). La télémétrie est collectée par \textit{Prometheus} et visualisée via \textit{Grafana}. \texttt{CybMASDE}/\texttt{KARMA} s’interface avec l’\acn{API} Kubernetes pour \textit{observer} l’état et \textit{agir} (scaling, isolement, redémarrage). Un \textbf{jumeau numérique} est construit à partir des traces pour entraîner les politiques hors-ligne puis \textbf{transférées} et \textit{fermées en boucle} sur le cluster (apprentissage itératif par rafraîchissement des traces).

\begin{figure}[h!]
  \centering
  \input{figures/karma_architecture/karma_architecture.tex}
  \caption{Schéma d’ensemble de \acn{KARMA} avec un cluster Kubernetes : collecte Prometheus, modélisation (jumeau numérique), entraînement guidé par rôles/missions (\acn{AOMEA}), analyse, transfert vers le cluster et boucle de réapprentissage.}
...
\begin{enumerate}[label=\textbf{\arabic*)}, leftmargin=3.5mm, itemsep=2pt, topsep=2pt]
  ...
  \item \textbf{Entraînement} : apprentissage \acn{MARL} avec \textit{rôles} (contraintes d’actions) et \textit{missions} (sous-objectifs) selon \acn{AOMEA}~\cite{soule2024aomea}.
...
\begin{table*}[h!]
  ...
  \begin{tabularx}{\textwidth}{p{4.1cm}p{3.4cm}p{2.7cm}X}
    ...
    \midrule
    \multirow{3}{*}{\parbox{4.1cm}{\textbf{Profil A — Défaut}                                                                                                                                                                                                                                                                                                                                                              \\\acn{MOD}-\acn{AUT} ; \acn{TRN}-\acn{CON} ; \acn{ANL}-\acn{AUT} ; \acn{TRF}-AUT}}
    ...
    \midrule
    \multirow{3}{*}{\parbox{4.1cm}{\textbf{Profil B — Analyse manuelle}                                                                                                                                                                                                                                                                                                                                                    \\\acn{MOD}-\acn{AUT} ; \acn{TRN}-\acn{CON} ; \acn{ANL}-\acn{MAN} ; \acn{TRF}-AUT}}
    ...
    \midrule
    \multirow{3}{*}{\parbox{4.1cm}{\textbf{Profil C — Cycle principalement manuel}                                                                                                                                                                                                                                                                                                                                         \\\acn{MOD}-\acn{MAN} ; \acn{TRN}-\acn{CON} ; \acn{ANL}-\acn{MAN} ; \acn{TRF}-MAN}}
...
Les trois environnements jouets (Overcooked-\acn{AI}, Proie-Prédateur, Warehouse Management) permettent d’évaluer la méthode \acn{MAMAD} dans des contextes coopératifs/compétitifs variés.
La \autoref{fig:noncyber_learning_curves} présente les courbes d’apprentissage (récompenses normalisées).
Dans l’ensemble, les profils \textbf{avec contraintes organisationnelles} convergent plus rapidement que les ablations \texttt{\acn{TRN}-UNC}.
Par exemple, dans Overcooked-\acn{AI}, \acn{MAPPO} avec contraintes fortes converge en moyenne après $1.8\times 10^4$ épisodes contre $2.6\times 10^4$ sans contraintes.
Dans Proie-Prédateur, \acn{QMIX} converge en $2.2\times 10^4$ épisodes (fortes) contre $3.4\times 10^4$ (sans).
...
Par exemple, dans Overcooked-\acn{AI}, les collisions de rôle (deux agents prenant simultanément la même tâche) surviennent dans $24.7\%$ des épisodes sans contraintes, contre $2.8\%$ seulement en mode doux.
...
Dans Overcooked-\acn{AI}, Auto-\acn{TEMM} infère correctement les rôles \texttt{Chef} et \texttt{Serveur} mais confond parfois l’\texttt{Assistant} et le \texttt{Chef}, expliquant un score légèrement plus bas ($87\%$).
...
La \textbf{baseline avancée} (Profil~A, contraintes fortes, \acn{MAPPO}) converge en moyenne après $3.2 \times 10^4$ épisodes, contre $4.5 \times 10^4$ pour l'ablation sans contraintes (\acn{TRN}-\acn{UNC}).
...
La récompense cumulée moyenne (moyenne sur 5 runs indépendants) atteint $+2450 \pm 120$ pour \acn{MAPPO}, contre $+1930 \pm 150$ pour \acn{TRN}-\acn{UNC}, indiquant un \textbf{gain de $+27\%$} grâce aux contraintes organisationnelles.
...
Sous scénarios perturbés (attaques simultanées, mouvement latéral intensif, faux positifs injectés), le \textbf{score de robustesse} (ratio performance perturbée/nominale) atteint $0.81$ pour \acn{MAPPO} avec contraintes fortes, contre $0.63$ pour \acn{TRN}-\acn{UNC}.
...
L’analyse Auto-\acn{TEMM} sur les trajectoires montre une \textbf{adéquation organisationnelle} ($\acn{OF}$) de $0.84$ pour \acn{MAPPO} avec contraintes fortes, contre $0.67$ pour \acn{TRN}-\acn{UNC}.
...
Le \textbf{Profil~A (contraintes fortes, \acn{MAPPO})} converge en $2.6\times 10^4$ épisodes (détection de plateau \emph{change-point}), contre $3.9\times 10^4$ pour l’ablation \texttt{\acn{TRN}-UNC}.
...
Sous DDoS, le \textbf{temps de reprise} (retour sous $L_{\text{avg}}<200$~ms) est de $3.7 \pm 0.6$~min pour \acn{MAPPO} contre $5.2 \pm 0.8$~min (douces) et $7.9 \pm 1.1$~min (\acn{TRN}-\acn{UNC}).
...
L’\textbf{écart-type inter-runs} sur la récompense est réduit avec contraintes ($\sigma=0.028$ fortes, $0.031$ douces) vs $0.049$ (\acn{TRN}-\acn{UNC}), soulignant une stabilité accrue.
...
Auto-\acn{TEMM} appliqué aux trajectoires (post-entraînement) produit un \textbf{score d’adéquation organisationnelle} $\acn{OF}=0.86$ (contraintes fortes), $0.83$ (douces) et $0.71$ (\acn{TRN}-\acn{UNC}).
La \textbf{qualité des spécifications inférées} (similitude Jaccard sur rôles/missions et déclencheurs) atteint $93\%$ (fortes), $90\%$ (douces), $76\%$ (\acn{TRN}-\acn{UNC}).
...
L’ablation \texttt{\acn{TRN}-UNC} sous-performe et présente une variabilité accrue, confirmant l’apport du guidage organisationnel dans un contexte opérationnel.
...
Le \textbf{Profil A (contraintes fortes, \acn{MAPPO})} converge en $3.1\times 10^4$ épisodes, contre $4.7\times 10^4$ pour l’ablation \texttt{\acn{TRN}-UNC}.
...
Auto-\acn{TEMM} infère des clusters de comportements distincts : \texttt{Analyste}, \texttt{Pare-feu}, \texttt{Opérateur}.
...
\begin{table}[h!]
  \centering
  \caption{Synthèse multi-environnements : couverture des critères C1--C5 par \acn{MAMAD}.}
...
\begin{itemize}
  ...
  \item L’\textbf{autonomie (C1)} atteint des scores élevés dans les environnements opérationnels (Company Infrastructure, Microservices, Drone Swarm), grâce à la boucle complète \acn{MOD}–\acn{TRN}–\acn{ANL}–\acn{TRF}, démontrant la capacité de \acn{MAMAD} à réduire l’intervention humaine.
\end{itemize}
