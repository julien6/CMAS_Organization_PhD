...
\begin{description}
  ...
  \item[CybMASDE :]
    \begin{itemize}
      \item Si des traces sont présentes : entraîne un \textbf{Joint-Observation Prediction Model (\acn{JOPM})} basé sur PyTorch (\acn{VAE}, \acn{MLP}, \acn{LSTM} optimisé par \texttt{Adam}).
...
\begin{description}
  ...
  \item[CybMASDE :] Utilise \acn{MARLlib} (\acn{MAPPO}, \acn{MADDPG}, QMix, etc.) et Ray RLlib. Applique les contraintes MOISE+MARL via masquage d'actions par rôle et shaping de récompenses. Les hyperparamètres (\acn{LR}, $\gamma$, taille de batch, etc.) sont optimisés via \texttt{Optuna}, une librairie facilitant l'optimisation des hyperparamètres. Sauvegarde checkpoints et statistiques dans \texttt{training/}.
\end{description}
