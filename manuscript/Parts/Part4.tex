...
\acn{CybMASDE}\footnotemark[1] est une plateforme modulaire et extensible que nous proposons pour supporter la méthode \acn{MAMAD}. Elle a pour objectif de satisfaire aux différentes attentes du concepteurs qu'il joue le rôle de:
%
\begin{itemize}
...
\acn{CybMASDE} est utilisable principalement en ligne de commande (voir \autoref{appendix:cybmasde-manual}) mais propose également une interface graphique pour effectuer les principales tâches de configuration.
Cette section décrit le parcours type d'un utilisateur de \acn{CybMASDE}, depuis l'environnement jusqu'à l'obtention d'une politique conjointe satisfaisante accompagnée de mesures de performance, stabilité et spécifications organisationnelles explicatives. Chaque étape indique à la fois les actions de l'utilisateur et les processus internes de la plateforme. Une première vue d'ensemble de \acn{CybMASDE} est proposée en \autoref{fig:cybmasde_usecase} sous forme de diagramme de cas d'utilisation puis un diagramme de séquence (c.f. \autoref{fig:cybmasde_sequence}) et de composant (c.f. \autoref{fig:cybmasde_uml}) montrent davantage le fonctionnement interne de \acn{CybMASDE}.

\subsection{Diagramme de cas d'utilisation de \texttt{CybMASDE}}

Le diagramme de cas d'utilisation présenté à la \autoref{fig:cybmasde_usecase} illustre les interactions possibles entre l'utilisateur et les différents acteurs et les principales fonctionnalités offertes par la plateforme \acn{CybMASDE}.
En général, l'\textbf{Utilisateur CLI} peut activer n'importe quel fonctionnalité tant que les dépendances entres les fonctionnalités sont respectées. Par exemple, il est possible de lancer l'entraînement sans avoir préalablement modélisé l'environnement si une instance \acn{MCAS} est fournie par l'utilisateur.
...
Bien que l'\textbf{Utilisateur CLI} représente le rôle \textquote{général}, il est également possible de décomposer ce rôle en d'autres sous-rôles (même abstraits) dédiés :
\begin{itemize}
  \item l'\textbf{Modélisateur de l'environnement}, chargé de définir ou compléter la description de l'environnement simulé, soit automatiquement à l'aide d'un \textbf{World Model}, soit manuellement par le biais du modèle \acn{MCAS}~;
  \item l'\textbf{\textquote{Entraîneur} d'agents}, qui conduit les phases d'apprentissage des politiques multi-agents en tenant compte des spécifications organisationnelles~;
  \item l'\textbf{Analyste organisationnel}, qui exploite la méthode \acn{TEMM} ou Auto-\acn{TEMM} afin d'inférer des rôles et objectifs organisationnels à partir des trajectoires produites~;
...
\acn{CybMASDE} fournit également une interface graphique permettant d'éditer le fichier de configuration de façon plus conviviale. Cette interface est illustrée dans les figures \autoref{fig:cybmasde_screenshot_configuration}, \autoref{fig:cybmasde_screenshot_training} et \autoref{fig:cybmasde_refining_screenshot}.
...
Le diagramme de séquence présenté en \autoref{fig:cybmasde_sequence} illustre le fonctionnement interne général de \acn{CybMASDE}.
...
\begin{description}
  \item[Utilisateur :] Met en place l’API environnementale (permettant à \acn{CybMASDE} de communiquer avec l’environnement cible) et renseigne les paramètres du projet dans le fichier de configuration central (\texttt{project\_configuration.json}).
...
\paragraph{4. Boucle de raffinement itératif}
\begin{description}
...
\medskip
\acn{CybMASDE} articule deux processus parallèles : le \textit{Transferring}, responsable du déploiement continu et de la collecte de données, et le \textit{MTA}, déclenché périodiquement pour modéliser, entraîner, analyser et raffiner les politiques.
...
\subsection{Cycle d'utilisation sur Overcooked-AI}
...
\paragraph{1. Mise en place / approche de l’environnement cible}
\begin{description}
...
\begin{description}
  ...
  \item[CybMASDE :] Utilise MARLlib et Ray RLlib pour entraîner deux agents.
    ...
    Les spécifications organisationnelles MOISE+MARL guident l’apprentissage (rôle \textquote{cuisinier}, rôle \textquote{serveur} et rôle \textquote{polyvalent}). L'intégration de ces rôles dans l'entraînement est illustrée dans le code \autoref{lst:org_spec_oa} qui décrit le modèle organisationnel utilisé comme contrainte au cours de l'apprentissage avec MOISE+MARL.
...
\begin{description}
  ...
  \item[CybMASDE :] Applique \acn{Auto-TEMM} :
    \begin{itemize}
      \item regroupe les trajectoires en clusters (ex.~: séquences \textquote{ramasser → déposer oignon}),
      ...
      \item calcule des métriques d’adéquation organisationnelle (ex.~: 80\% des actions de l’agent A sont conformes au rôle de préparateur). Un exemple de sortie textuelle de \acn{TEMM} est présenté en \autoref{lst:cybmasde_auto_temm_output}.
    \end{itemize}
...
\begin{description}
  \item[Utilisateur :] Choisit de continuer avec \texttt{cybmasde refine --max 5}.
    Il peut aussi corriger manuellement les spécifications organisationnelles (ex.~forcer les deux agents à échanger leurs rôles à mi-parcours). Un exemple de spécifications organisationnelles inférées est présenté en \autoref{lst:cybmasde_auto_temm_output}.
  \item[CybMASDE :] Relance des cycles \textit{Entraînement $\rightarrow$ Analyse} jusqu’à atteindre le seuil ou l’itération maximale.
\end{description}

\begin{lstlisting}[language=json,basicstyle=\tiny, label={lst:cybmasde_auto_temm_output}, caption={Extrait de spécifications organisationnelles après inferrés à analyser pour être raffinées manuellement}]
{
    "role_2": {
        "rules": [
            {
...
\begin{description}
  \item[Utilisateur :] Dispose désormais d’une politique conjointe robuste, validée, assortie de métriques de performance et de stabilité, ainsi que de spécifications organisationnelles explicatives.
    Exemple : \textquote{L’agent A assure en moyenne 80\% du ramassage, l’agent B sert 70\% des soupes}.
  \item[CybMASDE :] Archive l’ensemble du projet (modèles, politiques, logs, figures) dans l’arborescence générée.
\end{description}

\medskip
Dans cet exemple basé sur l'environnement Overcooked-AI, \acn{CybMASDE} illustre comment, en quelques commandes, il est possible de passer d’un environnement inconnu comme Overcooked-AI à un \acn{SMA} performant.
...
Le diagramme de composant C4 (voir \autoref{fig:cybmasde_uml}) illustre les modules de \acn{CybMASDE} et leur enchaînement lors d’une utilisation typique.
...
L’un des apports essentiels de \acn{CybMASDE} est d’intégrer le cadre organisationnel MOISE+ aux méthodes d’apprentissage multi-agents. Pour cela, nous avons développé une implémentation Python appelée \textbf{MMA} (\textit{MOISE+MARL API})\footnotemark[2].
...
Après l’entraînement, CybMASDE applique la méthode \acn{TEMM} (ou Auto-\acn{TEMM}) pour :
\begin{itemize}
...
En résumé, \textbf{MCAS} offre une modélisation manuelle spécialisée pour la Cyberdéfense, tandis que \textbf{MMA} fournit un cadre organisationnel opérationnel intégré au cycle d’apprentissage. Leur combinaison dans \acn{CybMASDE} permet d’allier rigueur organisationnelle et flexibilité d’entraînement multi-agent.
