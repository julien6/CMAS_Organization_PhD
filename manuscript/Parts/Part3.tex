...
Un \acn{TP} implique que toute observation ou action réelle considérée est connue et associée à une étiquette \(l \in L\) (via \(l: \Omega \cup A \to L\)) afin d'être gérée de manière pratique. Un \acn{TP} \(p \in P\) est défini comme suit : \(p\) est soit une \textquote{séquence feuille} notée comme un couple historique-cardinalité \(s_l = \langle h, \{c_{min}, c_{max}\} \rangle\) (où \(h \in H\), \(c_{min} \in \mathbb{N}\), \(c_{max} \in \mathbb{N} \cup \{``*"\}\)) ; soit une \textquote{séquence nœud} notée comme un couple composé d'un tuple de séquences et d'une cardinalité \(s_n = \langle \langle s_{l_1}, s_{l_2}, \dots \rangle, \{c_{min}, c_{max}\} \rangle\). Par exemple, le pattern
$
...
\begin{itemize}
  ...
  \item $\texttt{Render}^j_H : H \rightarrow \emptyset$, une fonction de rendu visuelle optionnelle basée historique~;
...
Bien qu'il n'est pas indiqué, en pratique la fonction d'arrêt $S^j_H$ est utilisée à la place du facteur d'actualisation afin de mettre un terme à la boucle des étapes. Le plus souvent cette fonction d'arrêt consiste à renvoyer vrai quand un nombre seuil d'étape a été franchi.
%
De même, si la fonction de rendu basée historique $\texttt{Render}$ est fournie, celle-ci est utilisée pour donner une visualisation des observations de chaque agent. Cette fonction est utilisée surtout à des fins d'explicabilité et de supervision.
...
\acn{TEMM} permet un contrôle expert et une analyse qualitative, tandis que \textbf{Auto-\acn{TEMM}} automatise l'analyse et optimise les hyperparamètres pour une utilisation à grande échelle.
