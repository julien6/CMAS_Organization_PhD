...
\begin{algorithm}[h!]
  \caption{Coeur algorithmique de la méthode MAMAD}
  \label{alg:mamad}
  \DontPrintSemicolon
  ...
  \While{\acn{SMA} en cours de conception}{
    ...
    $\mathcal{D}_{H^j}, \texttt{need\_update} \gets \text{transfer}(\pi^i_{\text{latest}}, \mathcal{D}_{H^j}, \texttt{mode})$ \tcp*[r]{appel asynchrone}
...
\begin{algorithm}[htb]
  \caption{Algorithme de l'activité d'analyse (Auto-TEMM)}
  \label{alg:auto_temm}
  \DontPrintSemicolon
...
\vspace{-0.3em}
\begin{algorithm}[h!]
  \caption{Algorithme de l'activité de transfert}
  \label{alg:transferring}
  \DontPrintSemicolon
  \KwIn{Politique actuelle $\pi^j_{\text{latest}}$, environnement réel $\mathcal{E}$, base de trajectoires $\mathcal{D}_{H^j}$, mode de transfert $\texttt{mode} \in \{\texttt{DIRECT}, \texttt{REMOTE}\}$}
