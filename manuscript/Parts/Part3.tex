...
\begin{figure}[h!]
  ...
  \caption{Structure de la Partie \acn{III}~: La méthode MAMAD}
...
\begin{figure}[h!]
  ...
  \caption{Cycle de vie d'un \acn{SMA} conçu avec MAMAD}
  \label{fig:cycle}
\end{figure}
...
\begin{algorithm}[H]
  \caption{Conception de \acn{SMA} assistée par MOISE+MARL}
  \label{alg:mamad}
  \DontPrintSemicolon
...
\paragraph{Processus \acn{MTA}~: optimisation des politiques et raffinement organisationnel}
...
En résumé, les approches symboliques (basées sur des modèles explicites comme Dec-\acn{POMDP}) apportent de l’explicabilité et du contrôle, tandis que les approches connexionnistes (basées sur les World Models) favorisent l’adaptation et la performance.
...
\begin{figure}[h]
  ...
  \caption{Schéma de l'architecture d'un \acn{JOPM} incluant le \acn{RDLM} et l'Auto-encodeur}
...
\subsubsection{Modélisation générale Dec-\acn{POMDP} pré-spécialisé}
...
\noindent
Le modèle Dec-\acn{POMDP} exprime l’état comme l’ensemble des propriétés des nœuds. Les actions sont définies par des pré/post-conditions sur ces propriétés. Les transitions et observations sont conditionnées par ces propriétés, et les récompenses sont calculées à partir de métriques sur l’état.
...
En utilisant la description formelle d'un Dec-\acn{POMDP}~\cite{OliehoekA16}, nous proposons le modèle Dec-\acn{POMDP} pré-spécialisé suivant :
...
Pour intégrer des scénarios réalistes, on s’appuie sur \acn{MITRE} \acn{ATT}\&\acn{CK} qui constitue une base de connaissances exhaustive des tactiques, techniques et procédures (\acn{TTP}) utilisées par les cyberattaquants.
Chaque \acn{TTP} est modélisée comme une action dans le Dec-\acn{POMDP}, avec des préconditions basées sur les propriétés requises et des post-conditions reflétant les effets de l’action.
...
\begin{enumerate*}[label=\arabic*),itemjoin={;\quad}]
  ...
  \item Ajouter les mesures de détection/atténuation issues de \acn{MITRE} \acn{ATT}\&\acn{CK}, également modélisées par des actions.
\end{enumerate*}
...
\paragraph{Étape 4 : Entraînement du modèle de dynamique récurrent (\acn{RLDM}).}
...
\paragraph{Étape 5 : Construction du \acn{JOPM}.}
...
\begin{algorithm}[H]
  ...
  \For{$h_z^j \in \mathcal{B}$}{
    \For{$(z_t, a^j_t, z_{t+1}) \in h^j$}{
      Entraîner le \acn{RLDM} $\mathcal{T}^{z}$ en minimisant l'erreur quadratique moyenne (\acn{MSE}) entre la prédiction $\hat{z}_{t+1}$ et la valeur réelle $z_{t+1}$.
    }
  }
...
\begin{itemize}
  \item formaliser l’intégration des spécifications organisationnelles \acn{MOISE}+ dans le \acn{MARL} via des guides de contraintes, permettant de restreindre ou d’orienter l’espace des politiques apprises ;
  \item proposer un nouveau formalisme, l’ODec-\acn{POMDP}, compatible avec les environnements simulés appris (World Models), pour permettre l’entraînement à partir de données observables uniquement ;
...
\noindent Enfin, nous introduisons les \textbf{Linkers} pour lier les spécifications organisationnelles $\mathcal{M}\acn{OISE}^+$ aux guides de contraintes et aux agents :
%
\begin{itemize}
  % \begin{enumerate*}[label={\roman*) },itemjoin={; \quad}]

  \item \textbf{Agent vers Rôle} \quad $ar: \mathcal{A} \to \mathcal{R}$, la relation bijective reliant un agent à un rôle ;
  \item \textbf{Guide Rôle vers Contrainte} \quad $rcg: \mathcal{R} \rightarrow rag \cup rrg$, la relation associant chaque rôle $\mathcal{M}\acn{OISE}^+$ à une relation $rag$ ou $rrg$, forçant/encourageant l'agent à suivre les actions attendues pour le rôle $\rho \in \mathcal{R}$ ;
...
\autoref{fig:mm_synthesis} illustre comment les spécifications \acn{MOISE}+ sont intégrées à la résolution Dec-\acn{POMDP} via le cadre MOISE+MARL.
...
\noindent Dans des environnements réalistes, on ne dispose que des transitions issues des historiques d'actions et d'observations reçues. Pour mieux représenter ce contexte, nous introduisons un nouveau formalisme appelé \textbf{Dec-\acn{POMDP} basé sur les observations} (\acparen{ODec-POMDP}). Un \acn{ODec-POMDP} $d_\Omega \in OD_\Omega$ (avec $OD_\Omega$, l'ensemble des ODec-POMDPs) est défini comme un quintuplet~:
%
...
\subsubsection*{Résolution d'un ODec-\acn{POMDP} avec \acn{MOISE}+MARL}
...
\paragraph{Étape 5 : Transition et mise à jour du \acn{JOPM}.}
...
\begin{itemize}
  ...
  \item un ODec-\acn{POMDP} $d_\Omega$ représentant l’environnement simulé appris ;
...
À ce jour, aucune méthode n’extrait automatiquement des spécifications organisationnelles complètes (rôles, missions, permissions, obligations) à partir des trajectoires, ni ne relie systématiquement les comportements émergents à un cadre symbolique tel que $\mathcal{M}\acn{OISE}^+$.
...
Dans ce cadre, un \textbf{rôle} $\rho$ est défini comme une politique dont les agents partagent une \textit{séquence commune la plus longue} (\acn{CLS}) dans leurs historiques.
Un rôle $\rho_2$ hérite de $\rho_1$ si $\text{CLS}(\rho_2) \subseteq \text{CLS}(\rho_1)$.
Le clustering hiérarchique permet d’extraire ces \acn{CLS} et de construire une hiérarchie des rôles.
...
\subsection{Auto-\acn{TEMM} : la méthode \acn{TEMM} étendue avec optimisation des hyperparamètres}
...
\begin{itemize}
  ...
  \item sur un entraînement contraint par des spécifications organisationnelles intégrées au sein du cadre \textit{\acn{MOISE}+MARL},
