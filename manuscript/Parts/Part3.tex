...
\begin{enumerate}
  ...
  \item \textbf{Analyse}~: extraire des spécifications structurelles et fonctionnelles émergentes à partir des trajectoires des agents entraînés~;
  \item \textbf{Transfert}~: mettre à jour régulièrement les politiques des agents déployés dans l'environnement réel à partir des politiques des agents entraînés en simulation et éventuellement mettre à jour ou améliorer l'environnement simulé.
\end{enumerate}

\noindent
Ces quatre activités peuvent être vues comme exécutées de façon itérative pour produire des \acplu{SMA} adaptés à leur environnement, alignés sur des contraintes organisationnelles, explicables et robustes.
...
\textbf{Guide de lecture du chapitre.}
Ce chapitre présente la logique globale de \acn{MAMAD} avant le détail activité par activité.
Pour une lecture rapide, le fil directeur est le suivant :
\begin{enumerate}
  \item comprendre la vue d'ensemble du cycle (voir \autoref{fig:cycle}) ;
  \item lire la formalisation algorithmique complète (voir \autoref{alg:mamad}) ;
  \item distinguer le rôle du processus \textit{Transferring} et du processus \acn{MTA} ;
  \item retenir les points de contrôle du raffinement (\(\texttt{org\_fit}_{min}\), \(\overline{r}_{min}\), \(\sigma_{max}\)).
\end{enumerate}
...
Le cœur de la méthode \acn{MAMAD} est d'envisager la conception d'un \acn{SMA} comme un processus itératif d'optimisation sous contraintes. Nous proposons une description formalisée de la méthode \acn{MAMAD} dans l'\autoref{alg:mamad} qui met en perspective les activités évoquées précédemment. Les données en entrée sont~:
\begin{itemize}
  \item $\mathcal{E}_0$~: l'environnement initial dans lequel les agents peuvent agir~;
  \item $d \in D \cup OD$~: un environnement fourni par l'utilisateur de façon optionnelle. Ce dernier peut avoir préalablement modélisé manuellement comme un \acn{Dec-POMDP} (voir \autoref{sec:dec-podmp}) ou modélisé automatiquement comme un \acn{ODec-POMDP} (voir \autoref{sec:wm_extension})~;
  ...
  \item $\texttt{org\_fit}_{min}$, $\overline{r}_{min}$, $\sigma_{max}$~: respectivement la valeur minimale exigée pour le score d'\textbf{adéquation organisationnelle} obtenu sur les $w_{\text{episodes}}$ derniers épisodes, la valeur minimale de la moyenne des récompenses cumulées sur les $w_{\text{episodes}}$ derniers épisodes, et la valeur maximale de l'écart-type sur les $w_{\text{episodes}}$ derniers épisodes. Ces seuils servent à valider une politique conjointe. Ces seuils sont généralement déterminés empiriquement, à partir d'une analyse préliminaire des résultats obtenus lors d'une première série d'épisodes~;
  \item $\texttt{mode} \in \{\texttt{DIRECT}, \texttt{REMOTE}\}$~: le mode de transfert des politiques des agents, soit direct (les agents déployés dans l'environnement embarquent les politiques pour les exécuter eux-mêmes) ou distant (la politique conjointe est exécutée sur le nœud d'exécution de \acparen{CybMASDE} et les actions sont communiquées aux agents qui ne font que les appliquer et renvoyer leurs observations respectives à \acparen{CybMASDE})~;
  \item $n_{refine}$~: le nombre maximal de cycles de raffinement alternant entre entraînement (pour entraîner les politiques avec spécifications organisationnelles) et analyse (pour déterminer des spécifications organisationnelles plus pertinentes en termes de performance ou explicabilité).
\end{itemize}

L'\textbf{adéquation organisationnelle} est introduite de façon théorique comme un indicateur quantitatif que nous théorisons et qui est compris entre 0 et 1 pour évaluer dans quelle mesure les comportements des agents sont structurés et conformes à des spécifications organisationnelles (qu'elles soient implicites ou explicites). Une valeur proche de 1 indique que les agents adoptent des comportements réguliers, stables et fortement alignés avec une organisation définie. À l'inverse, une valeur proche de 0 signifie que les comportements sont très irréguliers et qu'aucun schéma organisationnel cohérent n'émerge. En résumé, l'adéquation organisationnelle évalue la conformité d'une politique conjointe à une organisation structurée et fonctionnelle.
...
\paragraph{Schéma opératoire (vue synthétique).}
La lecture de l'\autoref{alg:mamad} peut être résumée en quatre transitions de flux :
\begin{enumerate}
  \item \textbf{TRF} déploie \(\pi^j_{\text{latest}}\) et accumule des trajectoires réelles ;
  \item un \textbf{déclencheur événementiel} active \textbf{MTA} quand le volume de données est suffisant ;
  \item \textbf{MTA} enchaîne \textbf{MOD} \(\rightarrow\) \textbf{TRN} \(\rightarrow\) \textbf{ANL} avec mise à jour de \(\mathcal{MM}\) et \(\pi^j_{\text{latest}}\) ;
  \item le système revient en \textbf{TRF}, ce qui ferme la boucle sim2real.
\end{enumerate}
...
\begin{algorithm}[h!]
  \caption{Vue algorithmique de la méthode MAMAD}
  \label{alg:mamad}
  \DontPrintSemicolon

  \KwIn{
  Environnement initial $\mathcal{E}$,
  Environnement simulé fourni $d \in OD \cup D$,
  objectif informel $\mathcal{G}_{\text{inf}}$,
  contraintes de conception informelles $\mathcal{C}_{\text{inf}}$,
  facteur d'actualisation $\gamma$,
  espace d'observation $\Omega$,
  espace d'action $A$,
  fenêtre glissante d'épisodes de validation $w_{\text{episodes}}$,
  valeur minimale d'adéquation organisationnelle $\texttt{org\_fit}_{min}$,
  valeur minimale de récompense cumulée moyenne sur un épisode $\overline{r}_{min}$,
  valeur maximale de l'écart-type des récompenses $\sigma_{max}$,
  mode de transfert $\texttt{mode} \in \{\texttt{DIRECT}, \texttt{REMOTE}\}$,
  nombre maximal de cycles de raffinement $n_{refine}$}
  ...
  Initialiser~: $\mathcal{D}_{H^j} \gets \emptyset$, $\pi^j_{\text{latest}} \gets \pi^j_{\text{init}}$, $\texttt{running\_MTA} \gets False, \texttt{running\_refinement} \gets True, \mathcal{MM} \gets \emptyset$ \;

  \vspace{0.3em}

  \While{\acn{SMA} en cours de conception}{
    \tcp*[l]{Transfert~: récupération des trajectoires \& déploiement de la politique}
    $(\mathcal{D}_{H^j}, \texttt{need\_update}) \gets \texttt{transfer}(\pi^j_{\text{latest}}, \mathcal{E}, \mathcal{D}_{H^j}, \texttt{mode})$ \tcp*[r]{appel asynchrone}
  ...
  \tcp*[l]{Si env. simulé non fourni, lancer modélisation}
  \If{$d = \emptyset$}{
  ...
  \While{$i < n_{refine} \ \text{ou} \ (\texttt{org\_fit} < \texttt{org\_fit}_{min} \ ou \ \overline{r} < \overline{r}_{min} \ ou \  \sigma > \sigma_{max}) \ ou \ \texttt{running\_refinement}$}{
...
Ce processus modélise le problème de conception actuel et améliore la politique conjointe des agents ainsi que ses spécifications organisationnelles. Il commence par construire un modèle de prédiction d'observations conjointes (\acparen{JOPM}) $T^j$ à l'aide de \textit{World Models}~\cite{soulej2025cloud} étendus, à partir des trajectoires collectées. Les exigences de conception sont formalisées sous forme de spécifications organisationnelles MOISE+MARL $\mathcal{MM}$ et l'objectif est formalisé par une fonction de récompense basée sur l'historique $R^j_H$.
...
\begin{itemize}
  ...
  \item \textbf{Un choix ciblé de sous-activités} : pour chaque activité retenue, une sous-activité peut être choisie en fonction des objectifs, des ressources disponibles et du degré d'automatisation souhaité (par exemple, \acparen{MOD-AUT} pour la modélisation automatisée, \acparen{TRN-CON} pour l'entraînement avec des contraintes). Cela permet de gérer le coût de conception en fonction du niveau de complexité de l'environnement et des ressources (financières, temps, expérience) disponibles.
...
\begin{itemize}
  ...
  \item \acn{ANL-AUT} : analyse entièrement automatisée via \acn{Auto-TEMM} pour extraire rôles et objectifs implicites, et évaluer l'adéquation organisationnelle.
...
Les chapitres suivants détaillent chaque activité du cadre \acn{MAMAD}, identifient les défis spécifiques rencontrés, et décrivent les contributions proposées pour y répondre.

\section{Bilan}
La méthode \acn{MAMAD} offre un cadre méthodologique structuré pour la conception de \acn{SMA}, en combinant modélisation, apprentissage, analyse et transfert dans un cycle itératif. Elle permet d'intégrer explicitement des spécifications organisationnelles dans le processus d'apprentissage, afin de contraindre les agents à adopter des comportements conformes aux exigences (par exemple, garantir la sécurité ou restreindre l'espace de recherche des politiques). \acn{MAMAD} facilite également la découverte, de manière semi-automatisée, de spécifications organisationnelles adaptées à l'atteinte des objectifs du \acn{SMA}. L'alternance entre entraînement et analyse permet de cibler progressivement les zones les plus pertinentes de l'espace des politiques, réduisant ainsi le besoin de connaissances ou d'expertise préalable sur les interactions agents-environnement.
...
\begin{itemize}
  \item un modèle de l'environnement simulé $d$ formalisé soit comme un \acn{Dec-POMDP} (modélisation manuelle) soit comme un \acn{ODec-POMDP} (modélisation automatisée) ;
\end{itemize}
...
\end{displaymath}
%
où $d$ est le modèle de l'environnement simulé.
Dans le cas d'une modélisation manuelle, le modèle $d$ est formalisé comme un \acn{Dec-POMDP} : $d = (S,\{A_i\},T,R,\{\Omega_i\},O,\gamma)$.
%
Dans le cas d'une modélisation automatisée, le modèle $d$ est formalisé comme un \acn{ODec-POMDP} : $d = (\mathcal{T}^j, \Omega^{\mathcal{T}^j}_0, R^j_H, S^j_H, \texttt{Render}^j_H)$ avec :
\begin{itemize}
  \item un \acn{JOPM} $\mathcal{T}^j$~;
  \item un ensemble d'états initiaux $\Omega^{\mathcal{T}^j}_0$ ;
  ...
  \item une fonction de rendue optionnelle $Render^j_H$ permettant de visualiser les trajectoires.
\end{itemize}

\section{Travaux mobilisés et verrous identifiés}

La modélisation d'un environnement multi-agent pour la Cyberdéfense repose sur plusieurs piliers théoriques.
...
En résumé, les approches symboliques (basées sur des modèles explicites comme \phantom{XXXX} \acparen{Dec-POMDP}) apportent de l'explicabilité et du contrôle, tandis que les approches connexionnistes (basées sur les World Models) favorisent l'adaptation et la performance.
...
\subsection{Spécificité Cyberdéfense : éléments génériques vs spécialisés}

Un point important de positionnement est de distinguer clairement ce qui relève d'un cadre \textbf{générique} de conception de \acplu{SMA} et ce qui constitue une \textbf{spécialisation Cyberdéfense}.

\begin{itemize}
  \item \textbf{Composants génériques}~: la structuration \textbf{MOD}--\textbf{TRN}--\textbf{ANL}--\textbf{TRF}, le formalisme \acn{Dec-POMDP}/\acn{ODec-POMDP}, la boucle de raffinement, et les mécanismes d'apprentissage/évaluation sont réutilisables dans d'autres domaines multi-agents.
  \item \textbf{Composants spécialisés Cyberdéfense}~: le modèle pré-spécialisé orienté réseau (nœuds, propriétés, privilèges), l'intégration des scénarios attaque/défense via MITRE ATT\&CK et arbres \acn{AD}, les métriques de sécurité (détection, violation, robustesse) et les environnements cibles (Company Infrastructure, Kubernetes, Drone Swarm).
  \item \textbf{Impact sur les critères}~: la spécialisation sert principalement C2/C3 (performance/adaptation en contexte opérationnel) et C4/C5 (contrôle/explicabilité) en rendant explicites les contraintes de sécurité, tandis que la structure générique de la méthode soutient C1 (autonomie) et la transférabilité globale.
\end{itemize}

Ainsi, la méthode n'est pas limitée à la Cyberdéfense, mais sa déclinaison présentée dans ce manuscrit intègre explicitement les artefacts du domaine pour assurer un alignement opérationnel réel.


\subsection{Les \textit{World Models} Multi-Agents pour la génération automatique du modèle simulé}

Dans cette approche automatisée, on commence par générer un environnement simulé de haute fidélité en construisant un \acn{JOPM} $\mathcal{T}^j~: H^j \times \Omega^j \times A^j \rightarrow \mathcal{H} \times \hat{\Omega}^j$ à partir des traces d'interactions réelles $\mathcal{D}_{H^j}$ (avec $h^j \in \mathcal{D}_{H^j}$, $h^j = (h^1, h^2 \dots h^{|\mathcal{A}|})$ et pour $i \in \{0\dots|\mathcal{A}|\}$, $h^i = \langle (\omega_t, a_t) \rangle\phantom{X}_{t \in \llbracket 0, n_{step} \rrbracket}$). À un pas de temps $t \in \llbracket 0, n_{step} \rrbracket$, pour un état caché récurrent $\tilde{h}_{t-1} \in \mathcal{H}$ représentant l'historique conjoint jusqu'à $t-1$ (avec $\tilde{h}_{-1} = \mathbf{0}$), l'observation conjointe reçue $\omega_t^j \in H^j$ et l'action conjointe $a_t^j \in A^j$, le \acn{JOPM} $\mathcal{T}^j$ renvoie le nouvel état caché $\tilde{h}_t \in \mathcal{H}$ ainsi que la prédiction de la prochaine observation conjointe $\hat{\omega}^j \in \hat{\Omega}^j$. Cette architecture, illustrée en \autoref{fig:jopm_architecture}, permet à \acn{MAMAD} d'apprendre la dynamique des observations de l'environnement pour former une simulation depuis zéro\index{Joint-Observation Prediction Model (JOPM)}.
...
On utilise généralement des \acn{MLP}s ou des architectures à base d'attention pour l'auto-encodeur (ensemble encodeur-décodeur), afin d'agréger les informations multi-agents en vecteurs de caractéristiques de taille fixe, tout en capturant les dépendances critiques entre agents.

Une fois l'encodage effectué sur l'ensemble des observations conjointes $\omega^j \in \Omega^{j}$ pour obtenir les représentations latentes $z_t \in Z_t$, le \textit{World Model} multi-agent fonctionne comme en contexte mono-agent, en utilisant les observations encodées $z_t$ dans les historiques transmis au \acn{RLDM} $\mathcal{T}^{z}$ on peut également utiliser un \acn{RLDM}. Pour chacun des épisodes collectés $h^j \in \mathcal{D}_{H^j}$, pour chaque étape $t \in \llbracket 0, n_{step} \rrbracket$, la représentation latente de l'observation conjointe $z_t$ est concaténée avec le vecteur d'état caché récurrent $\tilde{h}_{t-1}$ ainsi que le vecteur de l'action conjointe aplatie $\tilde{a}_t$. Le vecteur résultant de cette concaténation est passé au travers du \acn{RNN} (\acparen{LSTM}) pour mettre à jour le vecteur d'état caché récurrent $\tilde{h}_{t}$ puis est passé au travers un \acn{MLP} pour déterminer la représentation latente de l'observation conjointe approximée à l'instant suivant $z_{t+1}$. Ensuite, le décodeur $Dec$, permet d'obtenir la reconstruction de l'observation conjointe approximée aplatie $\tilde{\hat{\omega}}_{t+1}^{j}$. Enfin, cette observation conjointe approximée aplatie est recomposée en une observation conjointe prédite pour l'état suivant $\hat{\omega}_{t+1}^{j}$. Dans le cadre de \acn{MAMAD}, les \textit{World Models} constituent un des cœurs de la simulation mise en œuvre par l'activité de modélisation, agissant comme des jumeaux numériques de haute fidélité de l'environnement cible.

\subsection{Un modèle de simulation pour la génération manuelle du modèle simulé}

Dans l'approche manuelle, une description informelle de l'environnement, de l'objectif global et des contraintes est traduite en un modèle formel générique, extensible à divers contextes~\cite{soulej2023sim}.

\subsubsection{Modélisation générale Dec-POMDP pré-spécialisé}

Nous considérons un environnement réseau composé de nœuds, chacun décrit par un ensemble de propriétés $(id, v)$. Des agents (cyberattaquants/défenseurs) interagissent avec ces nœuds via des actions conditionnées par leurs observations et privilèges. Une action modifie l'état de l'environnement si ses préconditions (sur les propriétés) sont satisfaites, produisant un nouvel état et de nouvelles observations pour l'agent. Les agents agissent séquentiellement (mode \acparen{AEC}), chaque tour consistant à choisir une action, appliquer la transition, et recevoir l'observation/récompense.
...
Ce modèle, illustré en \autoref{fig:model_example_illustration}, permet de générer un simulateur multi-agents fidèle, où chaque action et observation est explicitement définie, facilitant l'extension à divers contextes de Cyberdéfense.

\

\noindent
\textbf{Traduction opérationnelle des arbres \acn{AD} vers le modèle séquentiel.}
La chaîne de traduction utilisée dans ce manuscrit est la suivante :
\begin{itemize}
  \item les nœuds et branches \acn{AD} sont convertis en actions d'attaque/défense avec préconditions et postconditions ;
  \item ces actions alimentent les composantes du modèle \acn{Dec-POMDP} (\(A\), \(T\), \(O\), \(R\)) via les propriétés des nœuds réseau ;
  \item les objectifs de défense associés sont ensuite exprimés au niveau organisationnel (missions/rôles) puis injectés dans MOISE+MARL via les guides de contraintes.
\end{itemize}

\noindent
Dans l'état actuel, cette traduction reste semi-manuelle et pilotée par le concepteur. L'automatisation complète de la projection \acn{AD} $\rightarrow$ spécifications $\mathcal{M}OISE^+$ est identifiée comme une perspective.
...
\section{Description et mise en œuvre dans l'activité}
...
\begin{algorithm}[h!]
  \caption{Vue algorithmique de l'activité de modélisation}
  \label{alg:modeling}
  \DontPrintSemicolon

  \KwIn{
  Environnement initial $\mathcal{E}$,
  historiques conjoints $\mathcal{D}_{H^j}$~;
  objectif informel $\mathcal{G}_{\text{inf}}$~;
  facteur d'actualisation $\gamma$~;
  espace des actions $A$~;
  espace des observations $\Omega$
  }
  \KwOut{l'environnement simulé $d \in D \cup OD$}
  ...
  \vspace{0.5em}
  \If{\textit{utilisateur souhaite modélisation manuelle}}{
  ...
  Entraîner un auto-encodeur $(Enc, Dec)$ sur $\Omega^j$ en minimisant l'erreur de reconstruction \;

  \vspace{0.5em}
  \tcp{3. Encoder les observations dans les historiques}
  Pour chaque historique $h^j = (\omega_t^j, a_t^j) \in \mathcal{D}_{H^j}$, encoder chaque observation conjointe ${z}_t = Enc(\omega^j_t)$ pour constituer l'ensemble d'entraînement $\mathcal{B} = \{ \{(z_t, a^j_t, z_{t+1})\} = h_z^j, h_z^j \in \mathcal{D}_{H^j}\}$
...
À partir de l'ensemble encodé $\mathcal{B}$, un modèle récurrent de dynamique latente (\acparen{RLDM}) est entraîné.
...
Le \acn{JOPM} $\mathcal{T}^j$ est alors défini en combinant le modèle récurrent $f$, l'encodeur $Enc$, et le décodeur $Dec$.
...
Enfin, l'activité retourne l'ensemble des éléments modélisés : le \acn{JOPM} $\mathcal{T}^j$, l'ensemble des observations initiales $\Omega^{\mathcal{T}^j}_0$, la fonction de récompense $R^j_H$, la fonction d'arrêt $S^j_H$, et la fonction de rendu éventuelle $Render^j_H$.
...
\begin{itemize}
  \item l'environnement simulé $d \in D \cup OD$ produit par l'activité de modélisation~;
        % \item le \acn{JOPM} $\mathcal{T}^j$ produit par la modélisation ;
        % \item les observations initiales $\Omega^{\mathcal{T}^j}_0$ ;
        % \item la fonction de récompense basée historique $R^j_H$
        % \item la fonction d'arrêt basée historique $S^j_H$ ;
        % \item la fonction de rendu basée historique $\texttt{Render}^j_H$ ou vide $\emptyset$ ;
  \item les spécifications organisationnelles $\mathcal{MM}$ possiblement issues des spécifications organisationnelles d'un cycle de raffinement précédent~;
  \item les spécifications organisationnelles informelles $\mathcal{C}_{\text{inf}}$ notamment si premier cycle de raffinement.
        % \item le facteur d'actualisation $\gamma$ ;
        % \item l'espace d'observations $\Omega$ ;
        % \item l'espace d'actions $A$ ;
\end{itemize}

\noindent
La \textbf{sortie attendue} est une politique conjointe entraînée $\pi^j = \{\pi^j_0, \pi^j_1, \dots, \pi^j_{|\mathcal{A}|}\}$.
...
Si $d \in OD, d = (\Omega, A, \mathcal{T}^j, \Omega^{\mathcal{T}^j}_0, R^j_H, S^j_H, \texttt{Render}^j_H, \gamma)$ et si $d \in D, d = (S,\{A_i\},T,R,\{\Omega_i\}, \allowbreak O,\gamma)$.
...
Du côté du \acn{MARL}, les méthodes classiques telles que l'apprentissage indépendant, l'apprentissage centralisé avec exécution décentralisée (\acparen{CTDE}), ou encore les algorithmes de type Q-learning, Policy Gradient et leurs variantes multi-agents, constituent la base pour optimiser des politiques conjointes dans des environnements simulés. Ces approches sont efficaces pour maximiser la performance collective, mais elles n'intègrent pas nativement de contraintes organisationnelles explicites.

Pour pallier ce manque, plusieurs travaux issus du Safe \acn{RL} et des \acn{CMDP} ont été mobilisés. Les méthodes comme \acn{CPO} ou \acn{DCQL} permettent d'intégrer des contraintes numériques (sûreté, consommation, risque) dans le processus d'apprentissage, mais leur expressivité reste limitée à des contraintes locales et numériques, sans prise en compte de structures organisationnelles complexes.
...
\subsection{Détails algorithmiques et passage à l'échelle}

Dans l'implémentation expérimentale, plusieurs familles d'algorithmes \acn{MARL} sont mobilisées (notamment \acn{MAPPO}, \acn{MADDPG}, \acn{QMIX}, \acn{COMA}), avec une prédominance de \acn{MAPPO} comme compromis stabilité/performance sur les cas étudiés.

Le coût d'entraînement dépend principalement :
\begin{itemize}
  \item du nombre d'agents \(|\mathcal{A}|\), de la longueur d'épisode \(T\), du nombre d'itérations de mise à jour et de la taille des réseaux ;
  \item de la dimension des observations/actions conjointes, qui croît rapidement avec la taille du système ;
  \item de la non-stationnarité induite par l'apprentissage simultané de plusieurs politiques.
\end{itemize}

De manière indicative, le coût suit un ordre de grandeur de type
\[
  O\!\big(|\mathcal{A}| \times T \times \text{taille\_modèle} \times \text{updates}\big),
\]
auquel s'ajoute le coût d'application des guides de contraintes
\[
  O\!\big(|\mathcal{A}| \times T \times \text{nombre\_de\_règles}\big).
\]

Les atténuations déjà intégrées dans la thèse sont : (i) compression latente des observations via \acn{JOPM}/auto-encodeurs, (ii) contraintes organisationnelles qui réduisent l'espace d'actions effectif (\textit{action masking} + \textit{reward guides}), (iii) protocole d'évaluation reproductible multi-runs (seeds, baselines, ablations) pour contrôler la variance, et (iv) boucle de raffinement MAMAD qui permet des itérations courtes et validées par seuils (\(\texttt{org\_fit}_{min}\), \(\overline{r}_{min}\), \(\sigma_{max}\)).

Cette discussion complète les résultats sans introduire de nouvelles expérimentations : elle explicite les compromis de scalabilité observés et justifie les choix algorithmiques effectués.

\subsection{Portée des contraintes : multi-objectif, shielding et contraintes temporelles}

Dans cette thèse, la dimension \textbf{multi-objectif} est traitée de façon opérationnelle dans la fonction de récompense via les guides \(grg\) associés aux missions, et non comme une optimisation de Pareto dédiée avec front explicite.
Autrement dit, plusieurs objectifs peuvent être poursuivis simultanément, mais leur arbitrage passe par des pondérations et des priorités définies dans les spécifications organisationnelles et les guides.
Cette formulation est cohérente avec l'objectif de contrôle (C4) et d'explicabilité (C5), tout en conservant des algorithmes \acn{MARL} standards.

Le \textbf{shielding} est implémenté selon deux niveaux complémentaires déjà présents dans MOISE+MARL :
\begin{itemize}
  \item un niveau \textbf{dur} via le \textit{action masking} (\(rag\), \(ch_t=1\)), qui interdit explicitement certaines actions ;
  \item un niveau \textbf{souple} via le \textit{reward shaping} (\(rrg\), \(grg\), \(0 \leq ch_t < 1\)), qui pénalise ou récompense des trajectoires sans blocage strict.
\end{itemize}
Cette combinaison explicite le compromis performance/contrôle : plus le masquage est strict, plus le respect des contraintes est assuré, au prix d'une exploration réduite.

Enfin, les \textbf{contraintes temporelles et de précédence} sont prises en compte à deux niveaux :
\begin{itemize}
  \item au niveau déontique, via \(rds(\rho)=\langle tc, y, m \rangle\) et \(v_m(t)\), qui activent obligations/permissions selon des fenêtres temporelles ;
  \item au niveau de la modélisation de scénario, via des préconditions/postconditions et des enchaînements d'actions issus des arbres \acn{AD}, ensuite traduits en missions et guides.
\end{itemize}
Cette prise en compte est déjà opérationnelle dans le pipeline, mais son automatisation complète (notamment pour la génération systématique des dépendances de précédence) reste un axe d'amélioration identifié.
...
%
se lit comme suit : un historique valide doit contenir au moins une occurrence de la paire \(\langle o_1,a_1\rangle\), suivie de zéro à deux occurrences de \(\langle o_2,a_2\rangle\). Ce TP capture donc une famille entière de comportements, sans avoir à lister tous les historiques.
%
...
\paragraph{Exemple concret.} Considérons l’environnement \textit{Overcooked-AI}~\cite{overcookedai}, où des agents cuisiniers doivent collaborer en se déplaçant dans un monde en grille et interagir avec les ingrédients (oignons) et les instruments (pots, bols) pour faire de la soupe qui est expédiée en interagissant avec la zone d'expédition. Ici, l’on souhaite reconnaître le comportement suivant : \textquote{un agent qui détient un oignon, observe un pot et interagit avec lui pour le remplir}. Ce comportement peut être exprimé par le TP suivant :
\[
  p = [[\#any](*) , \; has\_onion , \; [\#any](*) , \; see\_pot , \; interact \;](1,1)
\]

\noindent
Ce TP peut être exploité dans les guides de contrainte comme montré dans la \autoref{tab:tp_guides_example} pour inciter l'agent à interagir à nouveau pour récupérer la soupe par exemple.
...
\begin{itemize}
  ...
  \item $\mathcal{T}^j$, le \acn{JOPM} estimant la prochaine observation conjointe $\omega'$ à partir de l'historique $\tilde{h} \in \mathcal{H}$, de l'observation conjointe actuelle $\omega$ et de l'action conjointe $a$. Le modèle renvoie également l'état caché récurrent mis à jour $\tilde{h}'$~;
  ...
  \item $S^j_H~: H \rightarrow \{0,1\}$ la fonction d'arrêt basée historique qui indique si l'agent a atteint la fin de son épisode ou réussi son objectif~;
  \item $\texttt{Render}^j_H : H \rightarrow \emptyset$, une fonction de rendue visuelle optionnelle basée historique~;
...
Bien qu'il n'est pas indiqué, en pratique la fonction d'arrêt $S^j_H$ est utilisée à la place du facteur d'actualisation afin de mettre un terme à la boucle des étapes. Le plus souvent cette fonction d'arrêt consiste à renvoyer vrai quand un nombre seuil d'étapes a été franchi.
%
De même, si la fonction de rendue basée historique $\texttt{Render}$ est fournie, celle-ci est utilisée pour donner une visualisation des observations de chaque agent. Cette fonction est utilisée surtout à des fins d'explicabilité et de supervision.
...
Les transitions sont stockées dans $\mathcal{B}$, et la politique $\pi^j$ est optimisée par tout algorithme \acn{MARL} compatible (Q-learning, Policy Gradient, \acparen{CTDE}, etc.).
...
\begin{algorithm}[h!]
  \caption{Vue algorithmique de l'activité d'entraînement}
  \label{alg:training_mamad}
  \DontPrintSemicolon

  \KwIn{
  Environment simulé comme \acn{Dec-POMDP} ou \acn{ODec-POMDP} $d \in D \cup OD$~;
  ...
  \tcp*[l]{Si env. simulé manuel, entraînement classique}
  \If{$d \in D$}{
    \textit{entrainer une politique conjointe $\pi^j$ via MARL contraint selon \hyperref[eq:single_value_function]{Définition 1}} \;
  }
  \Else{
...
\begin{itemize}
  \item un \acn{ODec-POMDP} $d$ représentant l'environnement simulé appris ;
  \item une politique conjointe entraînée $\pi^j$ ;
  \item une spécification organisationnelle initiale $\mathcal{MM}$ (optionnelle) ;
  \item la fenêtre d'épisodes de test $w_{\text{episodes}}$.
\end{itemize}

\noindent
Les \textbf{sorties attendues} sont :
\begin{itemize}
  \item l'ensemble des spécifications implicites inférées $\mathcal{MM}_{\text{inferred}}$ ;
  \item le score d'adéquation organisationnelle $\texttt{org\_fit}$ sur les $w_{\text{episodes}}$ derniers épisodes ;
  \item la moyenne des récompenses $\overline{r}$ sur les $w_{\text{episodes}}$ derniers épisodes ;
  \item l'écart-type des récompenses $\sigma$ sur les $w_{\text{episodes}}$ derniers épisodes ;
  \item le booléen qui indique si l'utilisateur souhaite poursuivre les cycles de raffinement $\texttt{running\_refinement}$.
\end{itemize}
...
Ces limites motivent le développement de la méthode \acn{TEMM} et d'\acn{Auto-TEMM}, pour automatiser l'inférence organisationnelle, quantifier l'adéquation organisationnelle, et relier les comportements émergents à des spécifications formelles exploitables dans la boucle de conception.
...
\textbf{Définition~:} L'\textit{adéquation organisationnelle} est théorisée comme la mesure de la conformité entre les comportements collectifs observés et les spécifications organisationnelles attendues (rôles, objectifs, missions, permissions, obligations). Elle est quantifiée par un indicateur global (\acparen{OF}), combinant cohérence structurelle (\acparen{SOF}) et fonctionnelle (\acparen{FOF}) extraites des trajectoires.
...
\begin{itemize}
  ...
  \item une méthode flexible, utilisable en mode manuel (\acparen{TEMM}) pour l'analyse experte, ou automatisée (\acparen{Auto-TEMM}).
\end{itemize}

\acn{TEMM} permet un contrôle expert et une analyse qualitative, tandis que \textbf{\acn{Auto-TEMM}} automatise l'analyse et optimise les hyperparamètres pour une utilisation à grande échelle.
...
\begin{itemize}
  ...
  \item l'automatisation de l'analyse via \acn{Auto-TEMM} ;
...
La méthode exploite des techniques d'apprentissage non supervisé pour inférer des spécifications organisationnelles à partir des trajectoires observées des agents, et pour calculer l'\textbf{adéquation organisationnelle} (\acparen{OF}) entre les comportements émergents et les rôles, objectifs et missions attendus. Elle se décline en trois volets.

\paragraph{1) Rôles et héritage de rôles}
Les trajectoires $(\omega, a) \in \Omega \times A$ sont regroupées en clusters à l'aide de métriques de distance (par exemple \acparen{LCS}, Smith-Waterman~\cite{smith1981identification}), éventuellement après encodage one-hot des actions.
Dans ce cadre, un \textbf{rôle} $\rho$ est défini comme une politique dont les agents partagent une \acn{SP} dans leurs historiques.
...
Cette séquence capture le \textquote{motif le plus représentatif} partagé entre deux (ou plusieurs) séquences d'agents, en ignorant le bruit et les divergences en début et fin de séquence.
...
Un rôle $\rho_2$ hérite de $\rho_1$ si $\acn{SP}(\rho_2) \subseteq \acn{SP}(\rho_1)$.
Le clustering hiérarchique permet d'extraire ces \acn{SP} et de construire une hiérarchie des rôles.
...
Une obligation implique une exclusivité, tandis qu'une permission implique une optionnalité.
...
\begin{algorithm}[h!]
  \caption{Vue algorithmique de l'activité d'analyse (Auto-TEMM)}
  \label{alg:auto_temm}
  \DontPrintSemicolon
  ...
  \KwOut{
  Spécification organisationnelle inférée $\mathcal{MM}_{\text{inferred}}$~;
  Score de adéquation  organisationnel $\acn{OF}$
  }

  \tcp*[l]{1. Collecte des trajectoires}
  Générer les historiques individuels $\mathcal{D}_{\text{trans}}$ depuis $d$ sous $\pi^j$ \;
  ...
  Entraîner $\pi^j_{\rho_t}$ sur $(d, \mathcal{MM}_{\rho_t})$ jusqu'à atteindre $R_{\min}$ \;
  Enregistrer le temps de convergence $c_{\rho_t}$ tel que $ct_t(\rho_t) = c_{\rho_t}$ \;
  }

  \tcp*[l]{Sélectionner le point de coude}
  $\rho_t^* \gets max(\{\rho_t \ | \ ct_t(\rho_t) < \eta \})$ \tcp*[r]{par défaut $\eta = 3.5\%$}
  }

  \tcp*[l]{5. Inférence des rôles et objectifs}
...
La première étape consiste à exécuter la politique conjointe $\pi^j$ dans l'environnement $d$ afin de générer des historiques complets de transitions $(\omega, a, \omega')$ sur les $w_{\text{episodes}}$ de la fenêtre de validation. Cela permet de calculer la moyenne $\overline{r}$ et l'écart-type $\sigma$ des récompenses sur les épisodes collectés.
De plus, deux ensembles de données sont extraits de ces épisodes :
\begin{itemize}
...
On explore plusieurs métriques de distance $\delta_t$ (ex. \acparen{LCS}, Smith-Waterman, distances vectorielles) et seuils de regroupement $\tau_t$.
...
\begin{itemize}
  \item les clusters de transitions $C_{trans}$ permettent d'inférer les \textbf{rôles} par extraction des séquences communes (\acparen{CLS}) et des règles comportementales associées ;
...
Pour chaque $\rho_t$, une spécification $\mathcal{MM}_{\rho_t}$ est inférée, puis une nouvelle politique $\pi^j_{\rho_t}$ est réentraînée dans $d$.
...
\paragraph{Étape 5 : Inférence des rôles et objectifs.}
...
Comme pour \acn{TEMM}, deux indicateurs partiels sont calculés :
\begin{itemize}
...
\paragraph{Étape 6.5 : Compréhension et raffinement des spécifications.}
À cette étape, les spécifications organisationnelles implicites $\mathcal{MM}_{\text{inferred}}$ sont présentées sous forme de guides de contraintes, qui s'apparentent à des règles reliant des observations à des actions (RAG) ou à des ensembles d'observations (GRG). L'utilisateur peut, s'il le souhaite, examiner et interpréter ces guides de contraintes afin de proposer de nouvelles spécifications organisationnelles plus explicites et compréhensibles. Cette étape est facultative, mais elle permet d'améliorer la robustesse et l'interprétabilité des spécifications organisationnelles. Si l'utilisateur ne modifie pas les spécifications inférées, celles-ci sont conservées et réutilisées lors du cycle de raffinement suivant pour restreindre l'espace de recherche. Si l'utilisateur considère que les résultats sont satisfaisants, il peut mettre fin aux cycles de raffinement en positionnant le booléen \texttt{running\_refinement} à 0.

Nous englobons cette étape optionnelle dans la relation $\texttt{manual\_refine}: \mathcal{MM}_{\text{inferred}} \rightarrow \mathcal{MM}_{\text{explicit}} \times {0,1}$, qui prend en entrée les spécifications implicites et retourne des spécifications explicites (ou les mêmes si l'utilisateur ne souhaite pas les modifier) ainsi qu'un booléen indiquant s'il souhaite poursuivre les cycles de raffinement.

\paragraph{Étape 7 : Sorties de l'activité.}
L'activité retourne :
\begin{itemize}
  \item une spécification implicite $\mathcal{MM}_{\text{inferred}}$ décrivant rôles, missions, permissions et obligations inférés automatiquement ;
  \item le score d'adéquation organisationnelle $\texttt{org\_fit}$ permettant de quantifier la qualité organisationnelle des comportements émergents~;
  \item la moyenne $\overline{r}$ et l'écart-type $\sigma$ des récompenses sur les épisodes analysés ;
  \item un booléen $\texttt{running\_refinement}$ indiquant si l'utilisateur souhaite poursuivre les cycles de raffinement.
\end{itemize}


\section{Bilan}

En synthèse, l'activité d'analyse permet d'établir un lien objectif entre les comportements émergents des agents et les structures organisationnelles attendues. Grâce à la méthode \acn{TEMM} et à son extension \acn{Auto-TEMM}, il devient possible d'inférer automatiquement des rôles, objectifs et missions à partir des trajectoires, et de quantifier leur adéquation organisationnelle par un indicateur robuste. Cette démarche favorise l'explicabilité, la traçabilité et le raffinement itératif des spécifications organisationnelles, tout en fournissant des outils d'évaluation pour comparer différentes politiques ou configurations. Toutefois, la qualité de l'analyse dépend de la diversité des trajectoires collectées et du choix des hyperparamètres de clustering, même si l'automatisation par optimisation conjointe permet de limiter l'intervention humaine. De plus, bien que permissions et obligations soient inférées, leur évaluation reste aujourd'hui agrégée dans l'indicateur global~: une mesure déontique dédiée constitue un axe d'amélioration direct pour renforcer C4 et C5. Cette activité constitue ainsi un pivot essentiel pour la boucle de conception \acn{MAMAD}, en préparant le transfert et l'amélioration continue des politiques dans l'environnement réel.
...
L'activité de transfert et de supervision en environnement réel s'appuie sur plusieurs axes : le transfert de politiques (policy transfer), l'adaptation de domaine (Sim2Real), la calibration dynamique des modèles (online model calibration), et la supervision continue des \acplu{SMA}.
...
\subsection*{Schéma opératoire du transfert}
Pour clarifier la lecture de l'\autoref{alg:transferring}, le flux de contrôle est le suivant :
\begin{enumerate}
  \item exécuter la politique courante en mode \texttt{DIRECT} ou \texttt{REMOTE} ;
  \item stocker les transitions dans un tampon temporaire \(\mathcal{B}\) ;
  \item tester le seuil \(\texttt{batch\_size}\) ;
  \item en cas de seuil atteint, fusionner \(\mathcal{B}\) dans \(\mathcal{D}_{H^j}\) puis déclencher une mise à jour asynchrone ;
  \item reprendre immédiatement la boucle de transfert avec la dernière politique disponible.
\end{enumerate}
Ce schéma distingue explicitement le plan \textbf{d'exécution en ligne} (déploiement) et le plan \textbf{de mise à jour hors-ligne} (relance \acn{MTA}), ce qui réduit l'ambiguïté sur la synchronisation.


\vspace{-0.3em}
\begin{algorithm}[h!]
  \caption{Vue algorithmique de l'activité de transfert}
  \label{alg:transferring}
  \DontPrintSemicolon
  \KwIn{
  Politique actuelle $\pi^j_{\text{latest}}$,
  environnement réel $\mathcal{E}$,
  base de trajectoires $\mathcal{D}_{H^j}$,
  mode de transfert $\texttt{mode} \in \{\texttt{DIRECT}, \texttt{REMOTE}\}$
  }
  ...
  \tcp*[l]{Chaque agent applique localement la politique}
  Les agents observent localement $\omega^j_t$\;
  Calcul local $a^j_t \gets \pi^j_{\text{latest}}(\omega^j_t)$\;
  Application locale des actions et mise à jour de l'état $\omega^j_{t+1}$\;
  Stockage local $(\omega^j_t, a^j_t, \omega^j_{t+1})$ dans un tampon interne\;
  Périodiquement : envoi des tampons locaux à CybMASDE pour mise à jour de $\mathcal{D}_{H^j}$\;
  }
  \uElseIf{$\texttt{mode} = \texttt{REMOTE}$}{
  \tcp*[l]{La politique est exécutée par le processus Transferring}
  ...
  \If{$|\mathcal{B}| \geq \texttt{batch\_size}$}{
    ...
    \If{$\texttt{not running\_update} = \texttt{True}$}{
      \texttt{launch\_update()} \tcp*[r]{Appel asynchrone du processus MTA}
    }
  }
  }
  }
...
%
\begin{itemize}
  \item $\mathcal{B}$ désigne le tampon temporaire (voir \autoref{alg:transferring}),
  \item $\texttt{batch\_size}$ fixe la granularité de déclenchement des mises à jour,
  \item $\texttt{launch\_update}()$ assure la synchronisation avec les autres activités de la méthode.
\end{itemize}
...
%
Cette activité propose une méthode \textbf{autoadaptative}, alternant apprentissage simulé et déploiement réel pour garantir la robustesse des \acn{SMA} en contexte dynamique.
