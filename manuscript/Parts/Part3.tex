...
L'\textit{activité de modélisation} occupe une place centrale dans la méthode MAMAD.
...
En résumé, les approches symboliques (basées sur des modèles explicites comme Dec-POMDP) apportent de l’explicabilité et du contrôle, tandis que les approches connexionnistes (basées sur les World Models) favorisent l’adaptation et la performance.
...
\noindent
Le modèle Dec-POMDP exprime l’état comme l’ensemble des propriétés des nœuds. Les actions sont définies par des pré/post-conditions sur ces propriétés. Les transitions et observations sont conditionnées par ces propriétés, et les récompenses sont calculées à partir de métriques sur l’état.
...
En utilisant la description formelle d'un Dec-POMDP~\cite{OliehoekA16}, nous proposons le modèle Dec-POMDP pré-spécialisé suivant :
...
Pour intégrer des scénarios réalistes, on s’appuie sur MITRE ATT\&CK qui constitue une base de connaissances exhaustive des tactiques, techniques et procédures (TTP) utilisées par les cyberattaquants.
Chaque TTP est modélisée comme une action dans le Dec-POMDP, avec des préconditions basées sur les propriétés requises et des post-conditions reflétant les effets de l’action.
...
\begin{enumerate*}[label=\arabic*),itemjoin={;\quad}]
  \item Identifier tactiques/techniques/procédures pertinentes pour l’APT ciblée.
  \item Décrire le scénario d’attaque/défense et la topologie réseau.
  \item Construire une arborescence AD (objectifs, techniques, procédures), chaque action étant définie par ses pré/post-conditions.
...
À partir de l’ensemble encodé $\mathcal{B}$, un modèle récurrent de dynamique latente (RLDM) est entraîné.
...
Une fois le RLDM entraîné, les observations initiales $\Omega^{\mathcal{T}^j}_0$ sont extraites des historiques et utilisées pour initialiser le simulateur.
...
Ces composants constituent le cœur du jumeau numérique qui sera exploité par les activités d’entraînement, d’analyse et de transfert de la méthode MAMAD.
...
L’activité d’entraînement des politiques sous contraintes s’appuie sur plusieurs familles de travaux issus du domaine du MARL (Multi-Agent Reinforcement Learning) et de l’intégration de contraintes organisationnelles.
...
\begin{itemize}
  \item l’absence de cadre unifié permettant d’intégrer des contraintes organisationnelles symboliques dans l’apprentissage MARL ;
...
\begin{itemize}
  \item formaliser l’intégration des spécifications organisationnelles MOISE+ dans le MARL via des guides de contraintes, permettant de restreindre ou d’orienter l’espace des politiques apprises ;
  \item proposer un nouveau formalisme, l’ODec-POMDP (Dec-POMDP basé sur les observations), compatible avec les environnements simulés appris (World Models), pour permettre l’entraînement à partir de données observables uniquement ;
  \item développer un algorithme d’entraînement générique (voir Algorithme~\ref{alg:training_mamad}) qui articule ces guides de contraintes avec les méthodes MARL existantes, assurant ainsi la compatibilité entre apprentissage connexionniste et respect des contraintes organisationnelles ;
...
\subsubsection*{Résolution du Dec-POMDP avec MOISE+MARL}
...
\subsubsection*{Résolution d'un ODec-POMDP avec MOISE+MARL}
...
Les transitions sont stockées dans $\mathcal{B}$, et la politique $\pi^j$ est optimisée par tout algorithme MARL compatible (Q-learning, Policy Gradient, CTDE, etc.).
...
\begin{itemize}
  \item la scalabilité du MARL contraint à un grand nombre d’agents ;
  \item la dépendance aux données disponibles pour apprendre un JOPM fidèle ;
...
\begin{itemize}
  ...
  \item un ODec-POMDP $d_\Omega$ représentant l’environnement simulé appris ;
...
\textbf{Définition~:} L’\textit{adéquation organisationnelle} est théorisé comme la mesure de la conformité entre les comportements collectifs observés et les spécifications organisationnelles attendues (rôles, objectifs, missions, permissions, obligations). Elle est quantifiée par un indicateur global (\textbf{OF}), combinant cohérence structurelle (SOF) et fonctionnelle (FOF) extraites des trajectoires.
...
\begin{itemize}
  ...
  \item une méthode flexible, utilisable en mode manuel (TEMM) pour l’analyse experte, ou automatisé (Auto-TEMM) pour l’industrialisation.
\end{itemize}
...
\begin{itemize}
  ...
  \item l’automatisation de l’analyse via Auto-TEMM ;
  \item l’intégration de cette analyse dans la boucle de conception MAMAD.
\end{itemize}
...
On explore plusieurs métriques de distance $\delta_t$ (ex. LCS, Smith-Waterman, distances vectorielles) et seuils de regroupement $\tau_t$.
...
\begin{itemize}
  \item les clusters de transitions $C_{trans}$ permettent d’inférer les \textbf{rôles} par extraction des séquences communes (CLS) et des règles comportementales associées ;
...
\begin{itemize}
  \item le \textbf{SOF} (structural organizational fit), mesurant la cohérence des rôles via la variance des transitions intra-cluster ;
  \item le \textbf{FOF} (functional organizational fit), mesurant la cohérence fonctionnelle dans l’atteinte des objectifs intermédiaires.
\end{itemize}
...
\begin{itemize}
  ...
  \item l’environnement réel $\mathcal{E}$, représentant le domaine opérationnel où le SMA agit ;
...
La boucle de transfert s’exécute tant que le SMA est actif dans $\mathcal{E}$.
...
\begin{itemize}
  ...
  \item $\texttt{launch\_update}()$ assure la synchronisation avec les autres activités de la méthode MAMAD.
\end{itemize}
...
\begin{itemize}
  ...
  \item synchronisation robuste avec les autres activités de MAMAD ;
...
Cette activité permet une méthode réellement \textbf{autoadaptative}, alternant apprentissage simulé et déploiement réel pour garantir la robustesse des SMA en contexte dynamique.
