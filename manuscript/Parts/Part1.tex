...
\begin{figure}[h!]
  ...
  \caption{Description de l'architecture modulaire \acn{MASCARA}}
  \label{fig:mascara}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Vue synthétique de l'organisation~: (a) \acn{SMA} émergents~; (b) \acn{SMA} basés sur les coalitions~; (c) Ingénierie orientée agent~; (d) \acn{SMA} orientés organisation (tiré de \cite{Picard2009reorganisation})}
...
%     \item \textbf{Modèles organisationnels}~: $\mathcal{M}\acn{OISE}^+$~\cite{hubner2002moise,Hannoun2000}, OperA~\cite{Dignum2004}, \acn{AGR}~\cite{Ferber2004}.
...
\section{De la conception d'un \acn{SMA} de Cyberdéfense}\label{sec:problematique-sma}
...
\subsection*{Critères pour l'évaluation des \acn{SMA} de cyberdéfense}
...
%     }
%     \caption{Hiérarchie des critères guidant la conception d'un \acn{SMA} de Cyberdéfense (de type \acn{AICA})}
%     \label{fig:criteria}
% \end{figure}

Nous nous appuyons sur un ensemble de critères, résumée en \autoref{tab:critere_aica}, décrivant les propriétés qu'un \acn{SMA} de Cyberdéfense doit satisfaire pour répondre à la question de recherche globale.

\begin{table}[H]
  \centering
  \caption{Grille des critères d'évaluation d'un \acn{SMA} de Cyberdéfense}
...
\chapter{Vers des \acn{SMA} de Cyberdéfense et leur conception}
...
\footnotetext[1]{Cette section s'appuie sur une revue de littérature initialement publiée dans \autocite{soule2023ressithese}.}
...
Dans un premier temps, nous avons conduit une revue de littérature des travaux pouvant se comparer aux \acplu{SMA} de Cyberdéfense~\cite{soule2023ressithese}.
...
Dans un \textbf{\acn{SMA} de Cyberdéfense}, plusieurs agents atteignent un objectif global de Cyberdéfense par un comportement collectif résultant de la réalisation de sous-objectifs individuels et/ou de mécanismes locaux~\cite{jamont2015meeting}.
...
\begin{table*}[t!]
  ...
  \caption{Un aperçu de quelques organisations et des environnement hôtes utilisés dans les \acn{SMA} de Cyberdéfense étudiés}
...
\subsection{Organisations des \acn{SMA} de Cyberdéfense}
...
\begin{table}[htb]
  ...
  \caption{Un aperçu des fonctions de Cyberdéfense prises en charge par les \acn{SMA} de Cyberdéfense étudiés}*
...
\begin{table*}[h!]
  \centering
  \caption{Lecture synthétique des organisation des \acn{SMA} de Cyberdéfense selon les critères C1--C5}
...
\section{Les cadres de conception de \acn{SMA} de Cyberdéfense dans la littérature}\label{sec:sma-cyberdefense-conception}
...
\begin{itemize}
  ...
  \item les frameworks d'entraînement à base d'apprentissage automatique ou par renforcement, parfois multi-agents (e.g., \acparen{CSLE}~\cite{Hammar2023}, \allowbreak AutoPentest-\acn{DRL}~\cite{CROND}, \acn{CANDLES}~\cite{10.1145/2739482.2768429}) ;
  \item les plateformes dédiées à l'optimisation de politiques d'attaque ou de défense (e.g., PenGym~\cite{Nguyen2025}, \acn{CSLE}~\cite{Hammar2023}, CyberBattleSim).
\end{itemize}%ttt
...
Ces environnements présentent des profils variés qui sont résumés dans la \autoref{tab:design-frameworks}. Certains visent à fournir une simulation réaliste pour le test de politiques (e.g., NASimEmu, EmuLab), tandis que d'autres s'orientent vers la génération automatique ou la co-évolution de comportements (e.g., \acn{CLAP}, \acn{CANDLES}). Le degré d'autonomie accordé aux agents, l'intégration de modèles d'attaquants adaptatifs, et le support pour des politiques multi-agents diffèrent selon les cadres.
...
\begin{table*}[t]
  \centering
  \caption{Couverture des cadres de conception de \acn{SMA} de Cyberdéfense au regard des critères C1--C5}
  ...
  \begin{tabularx}{\textwidth}{
    ...
    CyberBattleSim        & Partielle                                             & Bonne (comparaison de politiques \acn{RL})  & Partielle (diversité de graphes vulnérabilités)         & Faible                                 & Faible                                   \\
...
Les approches symboliques, reposant sur des formalismes comme $\mathcal{M}\acn{OISE}^+$~\cite{hubner2002moise} ou \acn{AGR}~\cite{Ferber2004}, offrent un cadre rigoureux pour structurer les rôles, les permissions et les obligations au sein d'un \acn{SMA}. Ces modèles facilitent l'interprétabilité, la vérifiabilité, et la maîtrise de la structure organisationnelle (C4).
...
En fin de compte, les \acplu{SMA} issus d'approches symboliques peuvent pleinement tirer parti de la \textbf{re-organisation}~: ils disposent de structures explicites (rôles, missions, groupes, etc.) qu'ils peuvent manipuler de manière consciente, ce qui favorise à la fois l'\textbf{explicabilité} et le \textbf{contrôle}. En effet, la reconfiguration organisationnelle s'appuie sur des représentations internes accessibles à l'analyse ou à la vérification formelle. De plus, ces \acplu{SMA} peuvent également implémenter des mécanismes d'\textbf{auto-organisation}, notamment en utilisant un raisonnement symbolique local ou distribué. Bien que cette forme d'auto-organisation puisse introduire une certaine complexité dans les comportements, elle reste néanmoins interprétable, car fondée sur des règles ou structures modélisées explicitement.
...
\medskip
\noindent
\textbf{\acn{ANL} -- Analyse et explicabilité des comportements}
...
\medskip
\noindent
\textbf{\acn{TRF} -- Déploiement et transfert vers le réel}
...
\begin{itemize}
  \item \textbf{H-\acn{MOD}~:} Il est possible d'obtenir un environnement simulé réaliste, soit en facilitant la modélisation manuelle, soit par des techniques d'apprentissage machine à partir de données.
  \item \textbf{H-\acn{TRN}~:} Il est possible d'obtenir une politique conjointe via le \acn{MARL} et d'y intégrer les exigences de conception en tant que spécifications organisationnelles dans le processus d'apprentissage pour améliorer la conformité et la sûreté des politiques apprises.
  \item \textbf{H-\acn{ANL}~:} Les rôles et objectifs organisationnels peuvent être inférés automatiquement à partir des trajectoires comportementales d'agents entraînés via des techniques d'apprentissage machine.
  \item \textbf{H-\acn{TRF}~:} Un couplage entre environnement réel et simulé permet un déploiement sûr, adaptatif et maintenable des politiques apprises.
\end{itemize}
...
\begin{table}[H]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    (Optimiser la performance sous contraintes organisationnelles)
                                                                                                                          &
    \acn{TRN} : Résolution sous contraintes
                                                                                                                          &
    \vspace{-1.05cm}H-\acn{TRN} : Intégration des exigences organisationnelles dans l'apprentissage multi-agent
                                                                                                                          &
    ...
    (Expliciter et vérifier les comportements organisationnels)
                                                                                                                          &
    \acn{ANL} : Analyse et explicabilité des comportements
                                                                                                                          &
    \vspace{-1.05cm}H-\acn{ANL} : Extraction automatique de structures organisationnelles à partir des trajectoires d'agents
                                                                                                                          &
    ...
    (Assurer la cohérence et l'adaptation entre simulation et réalité)
                                                                                                                          &
    \acn{TRF} : Déploiement et transfert vers le réel
                                                                                                                          &
    \vspace{-1.05cm}H-\acn{TRF} : Couplage inspiré des jumeaux digitaux entre simulation et environnement réel
                                                                                                                          &
...
\end{itemize}
Cette décomposition conduit à identifier quatre sous-problèmes fondamentaux, correspondant aux objectifs de chaque activité~: (\acn{MOD}) la modélisation réaliste de l’environnement, (\acn{TRN}) la résolution sous contraintes, (\acn{ANL}) l’analyse et l’explicabilité des comportements, et (\acn{TRF}) le déploiement et le transfert vers le réel. Pour chacun de ces sous-problèmes, nous avons formulé des hypothèses de recherche (H-\acn{MOD}, H-\acn{TRN}, H-\acn{ANL}, H-\acn{TRF}) qui délimitent l’espace d’investigation et orientent la suite du manuscrit.
