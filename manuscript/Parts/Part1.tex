...
\begin{itemize}
  ...
  \item \textbf{Exploitent} des techniques d'\textbf{adversarial ML}\index{Adversarial ML}, générant des formes d'échappement aux détections habituelles (poisoning, prompt injection…).
\end{itemize}
...
\noindent
Finalement, c'est dans ce contexte que l'idée d'un \acn{SMA} de Cyberdéfense\index{SMA de Cyberdéfense}, telle qu’illustrée dans \autoref{fig:distributed_sma}, apparaît comme une alternative générique prometteuse par rapport aux approches centralisées existantes. Nous présentons dans la section suivante les bases conceptuelles des \acplu{SMA} en vue de la conception d'un \acn{SMA} de Cyberdéfense.
...
Inspirés par les défis posés par l'\textit{AICA IWG}~\footnotemark[3], nous avons défini un ensemble de critères, résumé en \autoref{tab:critere_aica}, décrivant les propriétés qu'un \acn{SMA} de Cyberdéfense doit satisfaire pour répondre à la question de recherche globale.
...
\begin{table}[h!]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    \hline
    \textbf{C1 -- Autonomie}     & Le \acn{SMA} doit fonctionner avec un minimum d'interventions humaines, que ce soit lors de sa conception, de son implémentation, de son fonctionnement ou de son arrêt. L'objectif est de réduire la dépendance à l'opérateur en favorisant l'auto-organisation et l'autodécision des agents.     \\
...
%
Les travaux identifiés concernant l'idée d'agents autonomes ou même de défense distribuée doivent prendre en compte simultanément tous les critères identifiés précédemment.
...
\begin{table*}[t!]
  ...
  \caption{Un aperçu de quelques organisations et des environnements hôtes utilisés dans les SMAs de Cyberdéfense étudiés}
...
Une synthèse de ce travail est présentée en \autoref{tab:general-overview}.
...
Les organisations de type décentralisé tirent profit d'une approche davantage auto-organisée pour faire face aux cyber-menaces de façon à augmenter l'autonomie du \acn{SMA} de Cyberdéfense comme proposée dans le <<~Artificial Immune System~>>~\cite{morteza2015method} ou la <<~Ant-Based Cyber Security~>>~\cite{haack2011ant} en sont des exemples.
...
\begin{table*}[h!]
  ...
  \begin{tabularx}{\linewidth}{
    ...
    \midrule
    Centralisée                                                                              &
    Réactivité locale limitée ; dépendance forte au centre (\acparen{SPOF}, goulots).        &
...
\begin{itemize}
  \item les environnements de simulation permettant d'émuler des réseaux ou des comportements d'attaques/défenses (e.g., \acparen{CybORG}, \acparen{NASimEmu}, \acparen{EmuLab}) ;
  \item les frameworks d'entraînement à base d'apprentissage automatique ou par renforcement, parfois multi-agents (e.g., \acparen{CSLE}~\cite{Hammar2023}, \allowbreak \acparen{AutoPentest-DRL}~\cite{CROND}, \acparen{CANDLES}~\cite{10.1145/2739482.2768429}) ;
  \item les plateformes dédiées à l'optimisation de politiques d'attaque ou de défense (e.g., \acparen{PenGym}~\cite{Nguyen2025}, \acparen{CSLE}~\cite{Hammar2023}, \acparen{CyberBattleSim}).
\end{itemize}
...
Ces environnements présentent des profils variés qui sont résumés dans la \autoref{tab:design-frameworks}. Certains visent à fournir une simulation réaliste pour le test de politiques (e.g., \acparen{NASimEmu}, \acparen{EmuLab}), tandis que d'autres s'orientent vers la génération automatique ou la co-évolution de comportements (e.g., \acparen{CLAP}, \acparen{CANDLES}). Le degré d'autonomie accordé aux agents, l'intégration de modèles d'attaquants adaptatifs, et le support pour des politiques multi-agents diffèrent selon les cadres.
...
\begin{table*}[t]
  ...
  \begin{tabularx}{\textwidth}{
    ...
    \acn{CSLE}            & Partielle                                              & Bonne (multi-agents \acparen{RL})              & Partielle (adaptation limitée aux environnements fixes) & Partielle (contrôle via paramètres)    & Partielle (visualisation des politiques) \\
    ...
    \acn{CyberBattleSim}  & Partielle                                              & Bonne (comparaison de politiques \acparen{RL}) & Partielle (diversité de graphes vulnérabilités)         & Faible                                 & Faible                                   \\
...
\section{Une tension entre approche symbolique et connexionniste}\label{sec:limits-existing}

La revue de littérature menée précédemment révèle une couverture très partielle des cinq critères de conception (C1 à C5) identifiés au \autoref{sec:problematique-sma}. Plus précisément, aucun travail ne permet aujourd'hui de concevoir un \acn{SMA} de Cyberdéfense pleinement conforme à la vision \acn{AICA}, à la fois robuste, explicable, généralisable et automatisable.
...
%
Les critères C1 à C5 identifiés précédemment rappellent l'ampleur des exigences à satisfaire~: \textbf{autonomie} (C1), \textbf{performance} (C2), \textbf{adaptation} (C3), \textbf{contrôle} (C4) et \textbf{explicabilité} (C5).
%
