...
% Invariants for Neuro-Symbolic Multi-Agent World Models" (AAAI-27
% submission). This document is NOT page-limited and is intended to
% accompany paper.tex as the technical supplement at submission time.
...
\begin{assumption}[Rollout support (H)]
\label{ass:support-full}
The imagined rollout's state distribution at every horizon $h\le H$ remains within the support of $d^\pi$.
\end{assumption}

\begin{proposition}[Chaining to rollout fidelity, conditional]
\label{thm:t3-full}
Under hard injection (projection or residual), disjoint masks, and \autoref{ass:support-full}: the one-step squared prediction error satisfies $\E\|\hat\omega_{t+1}-\omega_{t+1}\|^2 \le \varepsilon_d B^2+e_u$, and under an $L$-Lipschitz rollout operator, $E_H\le\big(\sum_{i=0}^{H-1}L^i\big)(\varepsilon_d B^2+e_u)$.
\end{proposition}
\begin{proof}
...
\end{proof}
This is stated as a proposition rather than a theorem for the same reason as in the main paper: \autoref{ass:support-full} is a conjecture, not an established fact, and separately the bound is vacuous for $H\gtrsim\ln\big(1/(\varepsilon_dB^2+e_u)\big)/\ln L$ whenever $L>1$, since the right-hand side then grows without bound while the true error cannot exceed $B^2$ (bounded observation space). Neither issue is fixed by tightening the constants above; both require either establishing \autoref{ass:support-full} (we could not) or working with a horizon-dependent, empirically-measured error curve directly (as Figure 2 of the main paper does), rather than this closed-form bound, once $H$ leaves the informative range.
...
If rules $r_1,\dots,r_p$ are injected on disjoint feature positions, let $V_i$ be rule $i$'s violation indicator and $\varepsilon_{r_i}=\Pr[V_i=1\mid c_i]$ by definition (\autoref{app:proofs}, Notation). Since $\Pr[V_i=1]=\varepsilon_{r_i}\Pr[c_i]$ (not $\varepsilon_{r_i}$ alone), the union bound over violation events gives
$$\varepsilon_d = \Pr\Big[\bigvee_{i=1}^p V_i=1\Big] \le \sum_{i=1}^p\Pr[V_i=1] = \sum_{i=1}^p\varepsilon_{r_i}\Pr[c_i].$$
Omitting $\Pr[c_i]$ (i.e., bounding $\varepsilon_d\le\sum_i\varepsilon_{r_i}$ directly, as an earlier version of this derivation did) is not conservative in a useful sense but simply loose whenever preconditions are not near-certain: at $\varepsilon_{r_i}=0.01$ and $p=50$ simultaneously active rules, $\sum_i\varepsilon_{r_i}=0.5$ regardless of coverage, while the corrected bound gives $\varepsilon_d\le0.5\cdot\max_i\Pr[c_i]$, e.g.\ $\varepsilon_d\le0.05$ for preconditions with typical empirical frequency $\Pr[c_i]\approx0.1$. The $\Pr[c_i]$ factor is what makes calibration data-driven: to keep rule $i$'s contribution to a total error budget $\varepsilon_{\mathrm{target}}$ below $\varepsilon_{\mathrm{target}}/p$, hard-inject only if $\tau_{\mathrm{hard},i}\ge1-\varepsilon_{\mathrm{target}}/(p\cdot\widehat{\Pr}[c_i])$, where $\widehat{\Pr}[c_i]$ is the rule's own measured trigger frequency (already tracked as $\mathrm{cov}(r)$ in the discovery inventory, \autoref{app:algo}) rather than a value fixed independently of how often the rule actually fires.
\end{remark}

\begin{remark}[Feature coverage vs.\ transition coverage]
$\kappa=\|M_d\|_0/(nn_f)$ counts covered feature \emph{positions}; it does not account for how often each position's precondition $c$ actually holds. Define $\kappa_{\mathrm{eff}}=\E_{d^\pi}\big[\tfrac{1}{|\mathcal{I}_d|}\sum_{(i,k)\in\mathcal{I}_d}\mathbb{1}[c_{i,k}(h^j_{t-1},\omega^j_t,a^j_t)]\big]$, the expected fraction of covered positions whose precondition is true at a given transition. Since each indicator is in $\{0,1\}$, $\kappa_{\mathrm{eff}}\le\kappa$ pointwise, with equality iff every covered rule has $c=\top$. \autoref{thm:t3-full}'s ``disjoint covered/residual masks'' assumption is exact in feature space but is a statement about $\kappa_{\mathrm{eff}}$, not $\kappa$, once any covered rule has a non-trivial precondition --- the residual model still receives (and must handle) transitions where a nominally-covered feature's rule does not fire. This is not currently measured in the main paper's experiments; see \texttt{revision/TODO-EXPERIMENTS.md}.
\end{remark}
...
\subsection{Metrics}
\begin{itemize}
  \item Fraction recovered $=(M-B_1)/(B_2-B_1)$, where $B_1$ = purely neural MAWM, $B_2$ = the symbolically-injected MAWM with hand-written oracle rules, $M$ = the symbolically-injected MAWM with discovered rules.
  \item Precision/recall of discovered rules against the oracle rule set (hand-written for GridCraft, used here as ground truth for the discovery metric only, not injected into $M$).
...
The main paper's discovery-vs-oracle and difficulty-gradient tables specify the exact column structure to fill from seed-level logs; this document specifies protocol, not new results, except for the significance and transfer-environment tables below, moved here from the main paper for space.

\subsubsection{Statistical significance across proposer and strategy comparisons}

\begin{table}[h]
\centering\small
\caption{Unpaired two-sample $t$ (difference / combined SE) for the proposer and strategy comparisons in the main paper's Tables 2 and 4; $|t|\ge1.96$ treated as distinguishable from noise at 5 seeds, $|t|<1.96$ as not.}
...
Computed directly from the means and standard errors already reported in the main paper, treating the 5 per-proposer/per-strategy seeds as unpaired. Only P1's disadvantage against P2 and P3 clears $|t|\ge1.96$; P2 vs.\ P3, the residual-conditioning fix, and projection vs.\ residual do not. Regularization vs.\ residual does clear the threshold; projection vs.\ regularization is borderline ($t=1.77$). If seeds are in fact matched across proposers and strategies (same environment instantiation, only the discovery/injection component varied), a paired-seed bootstrap would use the common-seed noise cancellation and could tighten several of the borderline rows above --- we did not have matched-seed logs to run this recomputation; see \texttt{revision/TODO-EXPERIMENTS.md}, item 1.

\subsubsection{Transfer environments}

\begin{table}[h]
\centering\small
\caption{Proposer comparison on transfer environments (mean $\pm$ SE, 5 seeds); $t$ for P2 vs.\ P3 computed as above.}
\label{tab:transfer}
\begin{tabular}{@{}llccc@{}}
\toprule
...
\multirow{2}{*}{Predator--Prey} & P2 & $0.76\pm0.06$ & $0.24\pm0.02$ & \multirow{2}{*}{$0.13$} \\
 & P3 & $0.75\pm0.05$ & $0.25\pm0.02$ & \\
\multirow{2}{*}{SMACv2} & P2 & $0.58\pm0.07$ & $0.14\pm0.02$ & \multirow{2}{*}{$1.41$} \\
 & P3 & $0.71\pm0.06$ & $0.16\pm0.02$ & \\
\bottomrule
\end{tabular}
\end{table}

On all three transfer environments, P2 and P3 are statistically indistinguishable ($|t|<1.5$ throughout): even the numerically largest gap, on SMACv2 ($0.71$ vs.\ $0.58$, $t=1.41$), does not approach the $1.96$ threshold used elsewhere. We find no evidence in these three environments that either proposer is more robust than the other as features move away from GridCraft's already-factorized structure; SMACv2's numerically larger gap is a reason to run more seeds there specifically (\texttt{revision/TODO-EXPERIMENTS.md}, item 6), not a trend to read into as submitted.

\subsubsection{Injection strategies (tabulated)}

\begin{table}[h]
\centering\small
\caption{Injection strategies with discovered rules; discussed in the main paper's \S7.4. $\mathrm{RVR}_{\mathrm{post}}=0$ for hard strategies is by construction, not a learned result.}
\label{tab:injection}
\begin{tabular}{@{}lcccc@{}}
\toprule
...
\subsubsection{Difficulty gradient (tabulated)}

\begin{table}[h]
\centering\small
\caption{Certification outcome by invariant class and falsification severity on GridCraft; discussed in the main paper's \S7.3. F1: passive holdout only. F1--F4: full protocol.}
\label{tab:gradient}
\begin{tabular}{@{}lccc@{}}
\toprule
Invariant class & Cert., F1 & Cert., F1--F4 & $\hat\varepsilon$ (F1) \\
\midrule
...
Latent env.\ confound & \textbf{yes (false)} & \textbf{no (flagged)} & $0.006\pm0.005$ \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection{Compounding-error curves}

Under open-loop imagination on GridCraft (mean over 5 seeds, across-seed SD $\le0.03$ at $H=100$ for all curves), the compounding one-step error grows with rollout horizon $H\in[0,100]$ as: $B_1$ (purely neural) reaches $0.02\to0.52$; $B_2$ (oracle rules) reaches $0.01\to0.13$; $M$ (discovered rules) reaches $0.01\to0.21$, tracking $B_2$ more closely than $B_1$ throughout, consistent with $\hat\kappa<\kappa$ and the non-zero $\hat\varepsilon$ carried by some discovered rules. This saturation is not read as evidence for Proposition 5.4's chaining bound (main paper, \S5), which is only informative below a horizon this draft does not estimate.

\subsubsection{Ablations (tabulated)}

\begin{table}[h]
\centering\small
\caption{Ablations on GridCraft (P2 proposer, residual injection unless noted); discussed in the main paper's \S7.5.}
\label{tab:ablations}
\begin{tabular}{@{}lc@{}}
\toprule
Ablation & Effect \\
...
Discovery wall-clock: P1 / P2 / P3 & $41.3{\pm}3.7$ / $6.2{\pm}0.8$ / $18.4{\pm}2.6$ min \\
Correlation-masking (fraction recov.\ drop) & $-0.147\pm0.028$ \\
Precondition depth $\beta{=}2{\to}3$ ($\hat M$, frac.\ recov.) & $6.1\times$, $+0.024\pm0.018$ \\
\bottomrule
\end{tabular}
\end{table}
...
Discovery introduces additional artifacts beyond the injection-time reproducibility material already needed for hand-specified symbolic injection: the four data partitions ($\mathcal{D}_{\mathrm{prop}},\mathcal{D}_{\mathrm{fit}},\mathcal{D}_{\mathrm{test}},\mathcal{D}_{\mathrm{shift}}$) and their exact construction per environment; the enumerated catalog and its realized $\hat M$ for P1; the decision-tree hyperparameters for P2; the exact LLM prompts, model identifier, and sampling temperature for P3; and the discovered rule inventory $\{(\varphi,c,\rho,m,q,\mathrm{cov},N_c)\}$ per seed, which should be logged in full (not only aggregated) so that precision/recall against the oracle set can be recomputed independently. Auditing the main paper's Theorem/Corollary bounds specifically additionally requires: the per-gate $N_c(r)$ distribution (min/median/max) across promoted rules; $p$, the number of simultaneously hard-injected rules per environment/proposer; the exact sizes (transitions and episodes) of each of the four partitions; the realized $\tau_{\mathrm{soft}},\tau_{\mathrm{hard}},m_{\max},\beta$ grid cardinalities feeding $\hat M_{\mathrm{eff}}$; and, for the retrogradation rate, the total training steps and number of hard-injected rules at risk that constitute its denominator. None of these are currently reported; see the corresponding \texttt{[MISSING ...]} markers in the main paper and \texttt{revision/TODO-EXPERIMENTS.md} item 8.

\paragraph{Contamination.} GridCraft, Overcooked, and SMACv2 are public, and their hand-written oracle rule sets may overlap with P3's LLM's training data, in which case P3's apparent proposal quality on these environments would not transfer to a genuinely novel one. Anonymization (main paper, \autoref{app:protocol}) strips semantic names but not structure: a feature confined to $\{0,\dots,9\}$ that changes by $\pm1$ per step remains recognizable as a coordinate regardless of what it is called, so name-stripping does not by itself rule out the LLM recognizing the environment \emph{class} from its transition structure. We did not have a genuinely unpublished environment available to test this and recommend one of two mitigations before relying on the P3 numbers: (i) evaluate P3 on a synthetic environment constructed after the LLM's training cutoff, or (ii) rescale and re-permute value \emph{ranges}, not just feature indices, subject to \autoref{app:dsl}'s constraint that this not break DSL-expressibility --- rescaling alone (not the linear mixing already ruled out) is compatible with the catalog and would weaken, though not eliminate, structural recognizability.
